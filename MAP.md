...
⭐⭐⭐ **als-v546 — Ο ΜΕΤΑΦΡΑΣΤΗΣ ΕΝΟΤΗΤΑΣ + ΤΟ SKILL.** Δική του απαίτηση:
*«δεν μπορω να σπαταλησω ενα βεεκλυ λιμιτ απλα για να φτιαχνουμε τα προηγουμενα»*.
`tools/istoria-unit.js <id>` κατεβάζει την ενότητα από το ebooks.edu.gr και την
ενώνει με ό,τι είναι **ΗΔΗ ελεγμένο** στο `istoria-data.js`. **Μηδέν μοντέλο,
μηδέν κόστος, τρέχει όσες φορές θες.** Βγάζει παραγράφους · `sic` · λεξιλόγιο ·
όρους Α1 · χρονολόγιο · σκελετό · υποψήφια Σ/Λ · **και τον χάρτη «ποια**
**παράγραφος»** για τα Β1/Β2. Ό,τι θέλει άνθρωπο είναι σημειωμένο `ΣΥΜΠΛΗΡΩΣΕ`.
**13/13 ενότητες, 68 παράγραφοι, 214 λεξιλόγιο, 63 όροι, 156 Σ/Λ, ΜΗΔΕΝ άγκυρα
εκτός κειμένου.** Το μόνιμο recipe ζει στο `~/.claude/skills/istoria-enotita/`.

⚠️ **ΤΕΣΣΕΡΑ ΠΡΑΓΜΑΤΑ ΤΟΥ EBOOK ΠΟΥ ΔΕΝ ΕΙΝΑΙ ΚΕΙΜΕΝΟ ΤΟΥ ΒΙΒΛΙΟΥ**, και κάθε ένα
σταμάτησε τον μεταφραστή μέχρι να αντιμετωπιστεί ΔΟΜΙΚΑ: λεζάντες εικόνων
(`.image_text`) · πλαϊνά πλαίσια (`.box_cyan`) · **δείκτες υποσημείωσης `<sup>`**
(«χαμηλά επίπεδα1.») · **λέξεις σπασμένες σε αλλαγή γραμμής** («Αμβρακικού-
Παγασητικού»). ⛔ Ο δείκτης υποσημείωσης βγαίνει από το `<sup>`, **ΠΟΤΕ με regex
σε ψηφία** — θα έτρωγε αληθινούς αριθμούς.

⭐⭐ **als-v545 — Ο ΧΑΡΤΗΣ ΕΓΙΝΕ ΠΙΝΑΚΑΣ ΠΕΡΙΕΧΟΜΕΝΩΝ.** Ήταν 105 πανομοιότυπα
κουτιά με **ΕΝΑ** που είχε περιεχόμενο, και η φράση «Δεν έχει φτιαχτεί ακόμα»
τυπωμένη **68 φορές** ενώ τη λέει ήδη το υπόμνημα (σταθ. 53). Τώρα: τυπογραφική
λίστα, και **κουτί φοράει ΜΟΝΟ ό,τι έχει περιεχόμενο**. Μηδέν απώλεια — ίδιοι
τίτλοι, ίδια σειρά. Μαζί: τίτλοι κεφαλαίων σε πεζά με τόνους (5, στο χέρι, ίδιες
λέξεις), `align-items:start` σε 6 πλέγματα (καμία κάρτα δεν κρατάει τον αέρα της
διπλανής), ενιαία γραφή «X / Y», και υπόμνημα που δείχνει **ΤΙ ΒΛΕΠΕΙΣ ΟΝΤΩΣ**.
⚠️ **ΤΡΕΙΣ ΜΕΤΡΗΣΕΙΣ ΑΝΤΙΘΕΣΗΣ, ΟΧΙ ΕΙΚΑΣΙΕΣ:** το `--faint` ήταν **2,69:1**
στην ετικέτα «Εκτός ύλης» και στο **✕ που σβήνει πλαγιότιτλο** (als-v525 ξανά)·
και το `--muted` πέφτει σε **4,34:1 ΜΕΣΑ στην έτοιμη κάρτα**, γιατί το
κεχριμπαρένιο φόντο ανεβάζει το φόντο σε ~rgb(53,48,45). Όλα ανέβηκαν.

⭐⭐⭐ **als-v544 — ΤΟ ΒΟΗΘΗΜΑ ΕΓΙΝΕ Η ΠΟΡΤΑ ΤΗΣ ΙΣΤΟΡΙΑΣ.** Από το School
Studies, η κάρτα «Ιστορία» ανοίγει το **`istoria-voithima.html`**.
⚠️⚠️ **ΔΥΟ ΠΟΡΤΕΣ, ΚΑΙ ΔΕΝ ΓΙΝΟΝΤΑΙ ΜΙΑ:** το `SUBJ.istoria` έχει πλέον
`door` (η κάρτα → βοήθημα) ΚΑΙ `page` (βαθύ link ενότητας → `istoria.html`).
Η μηχανή ανάκλησης διαβάζει `#recall:u:<id>` και κατέχει το `ist:v1`· το
βοήθημα έχει ΜΙΑ ενότητα, άρα κάθε βαθύ link προς τα εκεί θα προσγειωνόταν
σιωπηλά σε αδιέξοδο. **Η `istoria.html` ΔΕΝ πέθανε**: στέκει μόνη της, με
ρητό σύνδεσμο «Η μηχανή ανάκλησης · Ο πλαγιότιτλος» στο τέλος της Αρχικής.

⭐⭐⭐ **als-v543 — ΤΟ ΒΟΗΘΗΜΑ ΕΓΙΝΕ ΟΛΟΚΛΗΡΟ ΤΟ ΒΙΒΛΙΟ.** Δική του απόφαση:
το βοήθημα **αντικαθιστά** την `istoria.html` ως το σπίτι της Ιστορίας, και κάθε
ενότητα φτιάχνεται μία-μία. Τώρα έχει **Αρχική** («Όλη η ύλη, στη θέση της»),
**χάρτη ύλης** με 5 κεφάλαια / 27 υποενότητες / **105 ενότητες**, και δύο
συγκεντρωτικές οθόνες: **Όλοι οι ορισμοί μου** και **Όλες οι χρονολογίες**.
Πλοήγηση με hash (`#/` · `#/k1-g4/text` · `#/orismoi` · `#/xronologies`).

- **Οι τίτλοι ΔΕΝ γράφτηκαν από μνήμη**: curl σε 24 σελίδες του ebooks.edu.gr.
  ⚠️ Δύο παγίδες πληρώθηκαν: το «1. Τα δημογραφικά δεδομένα» ζει σε
  `span.bold` (όχι `div.title`), και το βιβλίο γράφει **«1.Η έξοδος» χωρίς κενό** —
  ένα φίλτρο που απαιτούσε κενό εξαφάνισε **3 ενότητες σιωπηλά**. Επίσης όλος ο
  **20ός αιώνας του Προσφυγικού** ζει σε `index3_7…3_11`, εκτός της πρώτης σάρωσης.
- ⭐ **ΤΑ ID ΕΙΝΑΙ ΣΥΜΒΟΛΑΙΟ** (`k<κεφ>-<λατινικό><αρ>`): κουβαλάνε τις σημειώσεις.
  **Το κεφ. 3 έχει ΔΥΟ μέρη που ξεκινούν και τα δύο από το «Α»** → 11 διπλά id,
  δηλαδή δύο ενότητες με κοινούς πλαγιότιτλους. Ο 20ός πήρε **`k3b-`**.
  Το **`k1-g4` δεν αλλάζει ΠΟΤΕ**.
- **Η εξεταστέα ύλη σημειώνεται, ΔΕΝ φιλτράρει** (Υ.Α. 90176/Δ2/06-07-2026,
  ΦΕΚ Β΄ 4210 — **μία πηγή**). Λάθος πηγή χάνει ετικέτα, ποτέ ενότητα.
- ⭐ Έλεγχος σε **αληθινό 400px μέσω CDP**: το `--window-size` του Chrome
  **κλειδώνει στα 500**, οπότε κάθε «screenshot στα 400» είναι κρόπ ενός 500άρη
  — δείχνει υπερχείλιση που δεν υπάρχει και κρύβει αυτήν που υπάρχει.
...
Σχεδιάγραμμα · Χρονολόγιο · Λεξικό · Fun facts · Κείμενο · Πηγές** — και
**οκτώ έτοιμες ενότητες** (Κεφ. 1 · **Α.1** «Τα δημογραφικά δεδομένα» ·
**Α.2** «Οι παραγωγικές δυνάμεις… και η «Μεγάλη Ιδέα»» · **Β.1** «Το εμπόριο» ·
**Β.2** «Η εμπορική ναυτιλία» · **Β.3** «Η διανομή των εθνικών κτημάτων» ·
**Γ.2** · **Γ.3** · **Γ.4**) από τις 105 του χάρτη. Νέα ενότητα = ένα νέο
αντικείμενο στον πίνακα `CHAPTERS` + ένα `CHAPTERS[n].diagram` στο δεύτερο
`<script>` + **δικό της αρχείο γείωσης** `tests/istoria-voithima-<id>.source.txt`
και δικό της μπλοκ «ζωγραφίζει» στον φρουρό. Το μηχανικό μέρος το τρέχει
δωρεάν το `tools/istoria-unit.js` (skill `istoria-enotita`). ⭐ Ο φρουρός έχει
και **οριζόντιο** έλεγχο (§3γ): δάπεδα ποιότητας και πληρότητα πεδίων σε **όλες**
τις ενότητες μαζί — τον χρειάστηκε, γιατί η Γ.4 είχε δύο πηγές χωρίς «δικό μου
σχόλιο» και κανένας κάθετος έλεγχος δεν μπορούσε να το δει. Ο ίδιος πίνακας
τυπώνεται και με το χέρι: `node tools/istoria-audit.js`.
...
- **Τα τρία τυπογραφικά του βιβλίου μένουν** («μεγάλο δάνεια» · «Το ποσά αυτά» ·
  ...
  εξαγωγείς** — ένα αρχείο γείωσης **ανά ενότητα** κρατάει την πηγή και το sha
  (`istoria-voithima.source.txt` Γ.4 · `-g3` · `-g2` · `-a1` · `-a2` · `-b1` · `-b2` · `-b3`). ⚠️ Η Α.1 ζει σε
  ΑΛΛΗ σελίδα του βιβλίου (`index1_2`): το κεφ. 1 είναι μοιρασμένο σε **τρία
  αρχεία, ένα ανά τμήμα** — `index1_2` = Α, `index1_3` = Β, `index1_4` = Γ.
...
✅⭐⭐ **ΤΑ ΔΙΑΓΩΝΙΣΜΑΤΑ — shipped als-v537, 5 Σεπ 2026.** Πέμπτη πόρτα,
**`#diagonismata`**: γράφει διαγώνισμα μόλις του το πουν (μάθημα · ημερομηνία ·
τίτλος με τα ΔΙΚΑ ΤΟΥ λόγια · η ύλη σε γραμμές), και όταν γυρίσει ο βαθμός τον
γράφει από πάνω. Νέος χάρτης **`hw:v1.exams`** — κανένα νέο κλειδί, κανένα νέο
appKey, τίποτα στο `sync.js`/`BUNDLES`. ⚠️ Είναι ΧΑΡΤΗΣ ΕΓΓΡΑΦΩΝ ΠΟΥ
ΞΑΝΑΓΡΑΦΟΝΤΑΙ (η ύλη διορθώνεται, ο βαθμός έρχεται μέρες μετά), άρα ζει στο
readMaps του `study-stamp.js` (σταθ. 31) και έχει ταφόπλακα `tombExam`
(σταθ. 32). ⛔ **ΔΕΝ είναι εργασία**: μια εργασία ΤΕΛΕΙΩΝΕΙ, ένα διαγώνισμα
ΓΡΑΦΕΤΑΙ και μετά γυρίζει με βαθμό — δύο κύκλοι ζωής σε μία λίστα θα σήμαιναν
ότι το «τελείωσε» λέει δύο πράγματα. Τρεις καταστάσεις, τρεις προτάσεις:
έρχεται · γραμμένο χωρίς βαθμό · γραμμένο με βαθμό (γι᾿ αυτό `grade` είναι
`null`, ποτέ `0`). Ο βαθμός κρατάει την **κλίμακά του** (/20 ή /100) — ένα «17»
χωρίς κλίμακα είναι άριστα ή αποτυχία. ⛔ Κανένας μέσος όρος από δύο κλίμακες.
⭐ **Και έκλεισε νεκρή πόρτα:** το `#programma` ήταν στο `DOORS` με κανόνα CSS
και renderer, αλλά **χωρίς γραμμή `cl.toggle`** από την als-v485 — δηλαδή ΛΕΥΚΗ
ΟΘΟΝΗ πίσω από σύνδεσμο της αρχικής. Το `tests/homework-exams.test.js` απαιτεί
τώρα, για ΚΑΘΕ κλειδί του `DOORS`, να συμφωνούν toggle και κανόνας CSS.
...
- ⭐⭐ **Η ΕΙΣΑΓΩΓΗ LEFT THE RECALL ENGINE (als-v497).** It used to be six units,
  ...
  **wrong material**. The εισαγωγή is now **four standalone study packets**,
  ...
  ⭐⭐ **ΠΕΝΤΕ από την als-v551**: μπήκε το **`arxaia-protagoras.html`** (πλάκα
  **V** — ο σοφιστής και ο διάλογος), ίδιο σχήμα με τη δίκη, CSS byte-για-byte.
  ⛔ **ΕΙΝΑΙ ΔΥΟ ΚΕΦΑΛΑΙΑ ΤΟΥ ΒΙΒΛΙΟΥ, ΟΧΙ ΕΝΑ** — το μέρος «Πρωταγόρας» του
  «Γ. Οι σοφιστές…» (4510,20308) **συν** τα μέρη Β και Γ του κεφαλαίου
  «ΠΡΩΤΑΓΟΡΑΣ» (4511,20312). Μια αναζήτηση σε ΕΝΑ κεφάλαιο δεν το βρίσκει.
  ⚠️⚠️ **ΕΔΩ Η ΗΛΕΚΤΡΟΝΙΚΗ ΕΚΔΟΣΗ ΕΧΕΙ ΑΔΙΚΟ, ΟΧΙ ΤΟ ΤΥΠΩΜΕΝΟ** — αντίστροφα
  από τα Λατινικά. Τρεις αποκλίσεις, δηλωμένες στο test ΚΑΙ στο υποσέλιδο:
  «ΙΙρωταγόρας» με δύο γιώτα, και **δύο βαρείες γραμμένες ως οξείες**
  («ὤν» αντί «ὢν», «ἤ» αντί «ἢ»). Η πηγή μένει ΩΜΗ· η σελίδα τυπώνει το σωστό.
  ⛔ **Όποιο πακέτο προστεθεί, αλλάζουν ΤΡΙΑ σημεία μαζί**: το `PACKS` του
  `tests/arxaia-gnosto.test.js`, η κεφαλίδα «N πακέτα · χωρίς βαθμό», και ο
  μετρητής `.gn-pl` σε **κάθε** test πακέτου (aristotelis + sokratis-ideas).
...
πρόοδό του διαβασμένη ζωντανά. Τα δύο πρώτα πακέτα, `latinika-lectio16.html`
...
και το `tests/latinika-lectio.test.js` τα κλειδώνει με **sha256** (ίδιο σχήμα
με το §9 των Αρχαίων· το `8596fc…` του XVII είναι, byte προς byte, το hash του
αρχείου που έστειλε).

⭐⭐ **ΤΡΙΤΟ ΠΑΚΕΤΟ — `latinika-lectio18.html`, als-v516 (27/08/26).** Lectio
XVIII, «Ο Ηρακλής στην Ιταλία», 3 σκηνές / **11 ενότητες** / 7 παγίδες. Δική
του εντολή: *«δες ακριβώς τι μου έχεις φτιάξει για τα άλλα δύο κείμενα … κάνε
ΑΚΡΙΒΩΣ το ίδιο»* — άρα η μηχανή του XVII αυτούσια (ομάδες `g` που δένουν
λατινική λέξη με ελληνικό κομμάτι, `d` μετάφραση + `x` συντακτικό ανά λέξη,
«✓ το ξέρω» ανά ενότητα), **συν** το μπλοκ «Οι παγίδες» του XVI.

- ⭐⭐ **ΚΛΕΙΔΩΝΕΤΑΙ ΑΛΛΙΩΣ, ΚΑΙ ΑΥΤΟ ΕΙΝΑΙ ΤΟ ΣΗΜΑΝΤΙΚΟ.** Τα XVI/XVII τα
  ΕΣΤΕΙΛΕ έτοιμα → sha256. Το XVIII το **έγραψα εγώ** από φωτογραφία του
  φυλλαδίου· ένα sha256 πάνω σε δικό μου αρχείο θα συμφωνούσε με κάθε λάθος
  μου τέλεια. Πήρε λοιπόν τη **γείωση της Ιστορίας**: το λατινικό κείμενο ζει
  χωριστά στο `tests/latinika-lectio18.source.txt` (**curl στο ebooks.edu.gr**,
  ΜΑΘΗΜΑ XVIII, sha256 του κατεβάσματος καρφωμένο) και το test ενώνει τις 11
  ενότητες και απαιτεί να το δίνουν **αυτολεξεί**.
- ⚠️⚠️ **ΔΥΟ ΔΗΛΩΜΕΝΕΣ ΑΠΟΚΛΙΣΕΙΣ, ΚΑΜΙΑ ΤΡΙΤΗ.** (α) οι αστερίσκοι
  (`adduxisse*`) είναι παραπομπές του βιβλίου, όχι κείμενο· (β) ⭐ **το
  «proximan» είναι ΤΥΠΟΓΡΑΦΙΚΟ ΤΟΥ ΒΙΒΛΙΟΥ** — το `spelunca` είναι θηλυκό σε
  αιτιατική, ο μόνος δυνατός τύπος είναι `proximam`, έτσι το τυπώνει και το
  φυλλάδιο. ⛔ Μη «διορθώσεις» τη σελίδα προς το βιβλίο. Και οι δύο
  επιβάλλονται **θετικά** (το test απαιτεί να ΥΠΑΡΧΟΥΝ στην πηγή), αλλιώς ο
  μετασχηματισμός γίνεται σιωπηλά no-op και θα έκρυβε πραγματική απόκλιση.
- ⚠️ **Η ΜΕΤΑΦΡΑΣΗ ΔΕΝ ΕΙΝΑΙ ΤΟΥ ΒΙΒΛΙΟΥ** — είναι μεταγραμμένη από τη
  φωτογραφία της σελ. 27 του φυλλαδίου (ΦΡΟΝΤΙΣΤΗΡΙΟ ΠΥΘΑΓΟΡΑΣ). Το λέει η
  πηγή, το λέει το υποσέλιδο της σελίδας, το ελέγχει το test. Αν στείλει ποτέ
  το αρχείο, αντικαθίσταται το μπλοκ και ξαναγίνεται πράσινο μόνο του.
- ⛔ Δικό του κλειδί, **`lectio18_known_v1`** — το test αποδεικνύει ότι ΔΕΝ
  δανείζεται την πρόοδο του XVII (μοιράζονται μηχανή· ένα copy-paste κλειδί θα
  έδειχνε την ίδια πρόοδο σε δύο κάρτες και θα φαινόταν αληθινό).
⭐⭐ **ΤΕΤΑΡΤΟ ΠΑΚΕΤΟ — `latinika-lectio19.html`, als-v535 (04/09/26).** Lectio
XIX, «Η συνωμοσία του Κατιλίνα», 3 σκηνές / **10 ενότητες** / 8 παγίδες / 6
σημειώσεις. Ίδια μηχανή με τα XVII/XVIII, ίδιο σχήμα εγγύησης με το XVIII:
δικό μου αρχείο → **γείωση**, όχι sha256.

- **Η ΠΗΓΗ:** `tests/latinika-lectio19.source.txt` — curl στο ebooks.edu.gr
  (ΜΑΘΗΜΑ XIX, `index19.htm`, sha256 του κατεβάσματος καρφωμένο) + η μετάφραση
  του φυλλαδίου. Το test (§2γ) ενώνει τις 10 ενότητες και απαιτεί να δίνουν
  **αυτολεξεί** και το λατινικό και τη μετάφραση.
- ⚠️⚠️ **ΔΥΟ ΔΗΛΩΜΕΝΕΣ ΑΠΟΚΛΙΣΕΙΣ, ΚΑΜΙΑ ΤΡΙΤΗ, ΞΑΝΑ.** (α) οι αστερίσκοι
  (`erant*`, `esse*`) είναι παραπομπές του βιβλίου· (β) ⭐ **«coniuārvit» είναι
  ΤΥΠΟΓΡΑΦΙΚΟ ΤΟΥ ΒΙΒΛΙΟΥ** — μια **αντιμετάθεση δύο χαρακτήρων**: το `ā`
  στέκει πριν από το `r` (`coniu|ā|rvit`) ενώ ο τύπος του `coniūro` είναι
  `coniurāvit`, όπως τον τυπώνει και το φυλλάδιο. Η διόρθωση είναι ελάχιστη —
  κανένας χαρακτήρας δεν μπαίνει και κανένας δεν φεύγει. ⛔ Μη «διορθώσεις» τη
  σελίδα προς το βιβλίο. Δεύτερη φορά που η ψηφιακή έκδοση έχει άδικο
  (`proximan` στο XVIII): **γείωση ≠ αλάθητη πηγή.**
- ⭐ **ΚΑΙ ΤΟ ΛΕΕΙ ΚΑΙ ΣΤΟΝ ΙΔΙΟ.** Το υποσέλιδο της σελίδας εξηγεί γιατί
  διαφωνεί με το βιβλίο σε μία λέξη· μια σιωπηλή διόρθωση σε κείμενο
  Πανελληνίων είναι ακριβώς αυτό που δεν επιτρέπεται να είναι σιωπηλό. Το test
  το απαιτεί.
- ⚠️ **ΟΥΤΕ Η ΜΕΤΑΦΡΑΣΗ ΟΥΤΕ ΤΟ ΣΥΝΤΑΚΤΙΚΟ ΕΙΝΑΙ ΔΙΚΑ ΜΟΥ** — και τα δύο
  μεταγράφηκαν από τη φωτογραφία της σελίδας του φυλλαδίου (ΦΡΟΝΤΙΣΤΗΡΙΟ
  ΠΥΘΑΓΟΡΑΣ) που έστειλε στις 04/09/26: ιδιόμορφη αφαιρετική απόλυτη, γενική
  ιδιότητας, εμπρόθετος συνοδείας, η ανάλυση του `proelio victus`
  (*postquam victus est* / *cum victus esset*) και τα παραθετικά του `gravis`
  που έχει γράψει ο ίδιος κάτω κάτω.
- ⛔ Δικό του κλειδί, **`lectio19_known_v1`**. **ΤΡΙΑ** πακέτα μοιράζονται τώρα
  τη μηχανή, άρα ένα ξεχασμένο κλειδί θα έδειχνε την ίδια πρόοδο σε τρεις
  κάρτες και θα φαινόταν απόλυτα αληθινό· το test το αποδεικνύει και προς τις
  δύο κατευθύνσεις.
- Το hero της βιβλιοθήκης: **«Τέσσερις ενότητες · 48 προτάσεις»** — και τα δύο
  μετρημένα από τα πακέτα (σταθ. 33). ⚠️ Το test απαγορεύει πια τη φράση
  «τρεις ενότητες» **και** απαιτεί να ΕΠΙΖΕΙ το σχόλιο της σταθ. 10 («Τρεις
  απαντήσεις, όχι δύο»), που ένα τυφλό search/replace θα το είχε φάει.
...
δικό μου αρχείο → **γείωση**, όχι sha256. Δική του εντολή: *«φτιάξε για τη
μτφραση των λατινικών όπως όλα τα άλλα τέλεια και για αυτό το κείμενο»*.

- ⚠️⚠️ **ΤΟ ΤΕΥΧΟΣ ΑΛΛΑΞΕ ΚΑΙ ΜΑΖΙ ΤΟΥ Η ΔΙΕΥΘΥΝΣΗ — ΚΑΙ ΤΟ 404 ΕΙΝΑΙ ΣΕΛΙΔΑ.**
  Το Α΄ Τεύχος (`8547/2706/…/indexNN.htm`) **σταματάει στο XX**: το `index21` /
  `index22` εκεί γυρίζουν **HTTP 404 με κανονική σελίδα μενού**, οπότε ένα
  `curl -s` χωρίς έλεγχο κωδικού θα κατέβαζε το μενού του αποθετηρίου και θα το
  περνούσε για βιβλίο. Η XXII ζει στο **Β΄ Τεύχος** (XXI–L):
  `8547/2708/Latinika-B_G-Lykeiou-AnthrSp_html-empl/**indexB_22.html**` — άλλο
  πρόθεμα, άλλη κατάληξη. Ο κατάλογος βρίσκεται στο `<select name="section">`
  του `index.html`, όχι σε href.
- **Η ΠΗΓΗ:** `tests/latinika-lectio22.source.txt` (sha256 του κατεβάσματος
  καρφωμένο) + η μετάφραση του φυλλαδίου. Το test (§2δ) ενώνει τις 11 ενότητες
  και απαιτεί να δίνουν **αυτολεξεί** και το λατινικό και τη μετάφραση.
- ⭐⭐⭐ **ΜΙΑ ΑΠΟΚΛΙΣΗ ΜΟΝΟ — ΚΑΙ ΤΟ «ΔΕΝ ΒΡΗΚΑ ΤΥΠΟΓΡΑΦΙΚΟ» ΕΙΝΑΙ ΕΥΡΗΜΑ.**
  Μετά το `proximan` (XVIII) και το `coniuārvit` (XIX) — δύο στη σειρά, δηλαδή
  μοτίβο — το κείμενο διαβάστηκε **λέξη-λέξη σαν λατινιστή** πριν καρφωθεί
  (αποθετικά, χαρακτηριστικά φωνήεντα υποτακτικής, δ΄ κλίση, `coĕtu`) και
  **δεν βρέθηκε κανένα**. Αυτό γράφεται ΡΗΤΑ στην πηγή, στο test και στο
  υποσέλιδο της σελίδας: ένα «δεν βρήκα» που δεν γράφεται πουθενά μοιάζει με
  «δεν κοίταξα», και η επόμενη ενότητα πρέπει να ξαναψαχτεί από την αρχή.
  Η μόνη απόκλιση είναι οι **ΔΕΚΑ αστερίσκοι** — και είναι δέκα επειδή
  σημαδεύουν **κάθε προτρεπτική υποτακτική** του κειμένου, οπότε το test τους
  **μετράει και τους ονομάζει** αντί να τους σβήνει τυφλά.
- ⭐ **ΔΕΥΤΕΡΟΣ ΜΑΡΤΥΡΑΣ ΓΙΑ ΤΗ ΜΕΤΑΦΡΑΣΗ, ΚΑΙ ΔΙΑΦΩΝΕΙ.** Το ebooks.edu.gr
  έχει τη δική του μετάφραση, αλλά **ως εικόνα** (`…/metafraseis/index_22/22.JPG`).
  Διαβάστηκε και συγκρίθηκε: συμφωνεί παντού εκτός από **δύο** σημεία («τους
  τοποθετώ στη χορεία» ↔ «δίνω μια θέση ανάμεσα», και «ας πιστεύουμε ότι» ↔
  «πως»). Καμία δεν είναι λάθος· η σελίδα κράτησε **του φυλλαδίου**, γιατί από
  αυτό διαβάζει, και το **υποσέλιδο δείχνει και τις δύο** ώστε να μην τρομάξει
  αν δει άλλο στο βιβλίο. Το test το απαιτεί.
- ⚠️ **ΟΥΤΕ Η ΜΕΤΑΦΡΑΣΗ ΟΥΤΕ ΤΟ ΣΥΝΤΑΚΤΙΚΟ ΕΙΝΑΙ ΔΙΚΑ ΜΟΥ** — από τη
  φωτογραφία του φυλλαδίου (25/09/26): το κόκκινο **«quos: αρχή περιόδου =
  κύρια, καθώς αντικαθιστά το eos»**, οι τέσσερις αναφορικές με το ΕΙΔΟΣ της
  καθεμιάς (προσδιοριστική στα αντικείμενα · προσδιοριστική στο `id` ·
  **ουσιαστικές** ως αντικείμενα στα `sperēmus` / `ferāmus`), η δοτική
  χαριστική στο `bonis`, ο εμπρόθετος τόπου και τα δύο ειδικά απαρέμφατα.
- ⛔ Δικό του κλειδί, **`lectio22_known_v1`**. **ΤΕΣΣΕΡΑ** πακέτα μοιράζονται
  πια τη μηχανή — και το XXII γράφτηκε με copy-paste **της μηχανής του XIX**,
  δηλαδή ακριβώς εκεί που ξεχνιέται το κλειδί. Το test το κόβει και προς τις
  **τέσσερις** κατευθύνσεις.
- Το hero της βιβλιοθήκης: **«Πέντε ενότητες · 59 προτάσεις»**, μετρημένα.
  ⭐⭐⭐ **Ο ΦΡΟΥΡΟΣ ΤΗΣ ΛΕΞΗΣ ΒΓΗΚΕ ΑΠΟ ΤΟ ΥΛΙΚΟ.** Ο παλιός έλεγχος έλεγε
  «όχι τρεις, τέσσερις» και **ξανάσκασε μόλις μπήκε η πέμπτη** — το νούμερο
  ήταν καρφωμένο ΜΕΣΑ ΣΤΟΝ ΦΡΟΥΡΟ. Τώρα ο κανόνας είναι «το hero λέει το
  αριθμητικό **των πακέτων που υπάρχουν**, και κανένα άλλο»: πρόσθεσε έκτη
  ενότητα και περνάει μόνο του. Το σχόλιο της σταθ. 10 («Τρεις απαντήσεις, όχι
  δύο») εξακολουθεί να **απαιτείται να επιζεί**.
- ⚠️ **Και το ίδιο το test κατηγόρησε σωστό περιεχόμενο**: `indexOf(v) > 0` σε
  λέξη που κάθεται στη **θέση 0** («Imitēmur», η πρώτη του κειμένου) είπε
  «λείπει» δύο φορές. Η βελόνα ήταν λάθος, όχι το κείμενο.
- ⭐ **ΑΠΟΔΕΙΓΜΕΝΟ ΖΩΝΤΑΝΑ (CDP):** κλικ στο `sempiternam` άναψε το ελληνικό
  του ζευγάρι και το συντακτικό, δεύτερο κλικ το έσβησε, «✓ το ξέρω» έγραψε
  **ΜΟΝΟ** στο `lectio22_known_v1` (μοναδικό κλειδί στο localStorage), «μηδένισε»
  το άδειασε. **390px: μηδέν οριζόντια υπερχείλιση**, και η κάρτα ελέγχθηκε στα
  **560px** — η σειρά `rn → src → t → d → in → pr → go` στέκει.

⭐⭐ **Η ΕΙΣΑΓΩΓΗ — `latinika-eisagogi.html`, als-v503.** Δική του εντολή,
22/08/26: *«μέσα στο λατινικά page εκτός από τις ενότητες έχουμε να μάθουμε και
την εισαγωγή … αυτό θα το κοιτάω και θα το διαβάζω και θα κάνω επανάληψη μέσα
στη χρονιά μέχρι να δώσω πανελλήνιες … να είναι πολύ ωραία και όμορφες οι
σημειώσεις, κάτι που δεν γίνεται από βιβλίο»*. Οκτώ κεφάλαια, από την
ινδοευρωπαϊκή ρίζα ως τον Βιτρούβιο. **ΑΝΑΓΝΩΣΜΑ, ΟΧΙ ΜΗΧΑΝΗ: κανένας
βαθμός** — μόνο «το διάβασα» ανά κεφάλαιο, κλειδί `eisagogi:v1`, χάρτης
`{id:true}`, **τρίτο σχήμα** στη σελίδα δίπλα στα δύο των Lectio και εκτός sync
για τον ίδιο ακριβώς λόγο.
- ⭐⭐ **ΤΟ ΚΕΙΜΕΝΟ ΕΙΝΑΙ ΑΥΤΟΥΣΙΟ, ΚΑΙ ΑΠΟΔΕΙΚΝΥΕΤΑΙ.** Εδώ δεν υπήρχε αρχείο
  να μπει με `cp`+sha256 όπως τα Lectio — η εισαγωγή ήρθε ως κείμενο. Άρα το
  `tests/latinika-eisagogi.test.js` κλειδώνει το ΙΔΙΟ πράγμα με άλλο σχήμα:
  η πηγή ζει χωριστά στο **`tests/latinika-eisagogi.source.txt`** (μία
  παράγραφος ανά γραμμή) και το τεστ απαιτεί **και οι 75 να υπάρχουν αυτολεξεί**
  μέσα στο `DATA[]`. Βγάζει μόνο τις ΤΙΜΕΣ των string literals και τις ενώνει,
  ώστε μια πρόταση μοιρασμένη σε πεδία (χρονογραμμή: `nm`/`yr`/`ex`) να
  ξαναγίνεται συνεχής. **Πρώτη έκδοση ήταν OCR από φωτογραφίες και είχε
  αληθινά λάθη· ο ίδιος έστειλε το σωστό κείμενο και αντικαταστάθηκε ολόκληρο.**
- ⛔ **ΚΑΜΙΑ ΔΙΚΗ ΜΑΣ ΛΕΞΗ ΣΤΙΣ ΚΑΡΤΕΣ.** Είχα βάλει ετικέτες τύπου «ρήτορας ·
  φιλόσοφος · πολιτικός» στα πρόσωπα — δεν υπάρχουν πουθενά. Το τεστ απαιτεί
  κάθε ετικέτα να είναι **τίτλος έργου που υπάρχει στην πηγή** (`Carmina`,
  `Metamorphoses`, `De rerum natura`, `Αινειάδα`, `De architectura`) και
  απαγορεύει διαχωριστικό ` · ` μέσα σε ετικέτα ή λεζάντα — «Γάιος Βαλέριος
  Κάτουλλος» και «Λουκρήτιος» είναι **δύο** λεζάντες, όχι μία ενωμένη.
- ⭐ **ΤΟ ΚΕΙΜΕΝΟ ΓΕΝΝΙΕΤΑΙ ΟΡΑΤΟ (σταθ. 10).** Η είσοδος `opacity:0` ζει ΜΟΝΟ
  κάτω από `html.anim`, που το προσθέτει η JS **αφού** δεσμευτεί να τα ανάψει,
  συν `setTimeout(revealAll, 1500)` για δίχτυ. Στην πρώτη έκδοση ένα
  IntersectionObserver που δεν χτύπησε έβγαλε **ολόκληρη μαύρη σελίδα** στο
  render — σιωπηλό μηδέν, όχι σφάλμα.
- ⭐ **Μια κλάση, μια δουλειά (σταθ. 14, σε CSS).** Το `.wk` ήταν ΚΑΙ ετικέτα
  κάρτας (`display:block`) ΚΑΙ τίτλος έργου μέσα σε πρόταση, οπότε το «Για τη
  φύση των πραγμάτων» έσπασε τη γραμμή στα τρία. Ο inline είναι τώρα `.ttl`
  και το τεστ απαγορεύει `<span class="wk">`.
- Οι εικόνες των σημειώσεων δεν υπάρχουν εδώ· **οι λεζάντες τους ναι**, ως
  δικό τους στοιχείο. Δεν προσποιούμαστε ότι δεν υπήρχε τίποτα (σταθ. 10).

- ⛔ **Τα `lectio16:v1` / `lectio17_known_v1` / `lectio18_known_v1` /
  `lectio19_known_v1` / `lectio22_known_v1` ΔΕΝ συγχρονίζονται, επίτηδες:**
