...
2. **Bump `CACHE` in `sw.js:15` on every deploy.** Currently `als-v493`. Never
   move it backwards.
...
## ⭐⭐ Η ΕΚΘΕΣΗ — `ekthesi.html`, als-v495

Το τελευταίο 30% που είχε **μηδέν δεδομένα**. Ισοβαθμεί με τα Αρχαία και ως την
v495 το `homework.html` το έλεγε ρητά («δεν υπάρχει σελίδα ακόμη») αντί να το
κρύψει. Αφορμή: **κάθε Τετάρτη για Πέμπτη** μαθαίνει μία σελίδα λεξικού
συνωνύμων/αντωνύμων και τη γράφει στο χαρτί. Πρώτη σελίδα μέσα: **547**
(Αγωνιστικότητα → Αισθάνομαι, 16 λήμματα, 89 στοιχεία).

**Αρχεία** — `ekthesi.html` (5 περάσματα) · `ekthesi-data.js` (το υλικό) ·
`ekthesi-engine.js` (ο βαθμολογητής) · `tests/ekthesi-engine.test.js` (183
βεβαιώσεις). Αποθήκη `ekt:v1` (`words` σκάλα + `els` ακρίβεια ανά ΣΤΟΙΧΕΙΟ +
`sessions`), appKey `ekthesi`, γραμμένη στο `ladders.js`, στο `backup.html`, στο
`launcher.js` και στο πλακίδιο της αρχικής.

### ⭐⭐ ΓΙΑΤΙ ΕΧΕΙ ΔΙΚΟ ΤΗΣ ΒΑΘΜΟΛΟΓΗΤΗ ΚΑΙ ΟΧΙ ΤΟ `lesson-grade.js`
Ιστορία και Αρχαία-γνωστό βαθμολογούνται **απαγγελία**, οπότε το `pho()` μπορεί
να συγχωρεί την ορθογραφία — δεν την πληκτρολογεί αυτός, τη γράφει η αναγνώριση
φωνής. **Εδώ γράφει ο ίδιος, και η εξέταση είναι στο χαρτί.** Άρα το
«αδωροδόκιτος» είναι ταυτόχρονα **ανάκληση πετυχημένη** και **ορθογραφία
χαμένη**, και καμία δυαδική απάντηση δεν λέει και τα δύο. Τρεις βαθμίδες που
μετράνε (`ok` · `spell` · `form`) και δύο που δεν μετράνε: `cross` (σωστή λέξη,
**λάθος στήλη** — η πιο χρήσιμη γραμμή της σελίδας) και `extra` (εκτός λεξικού,
**ποτέ τιμωρία**: το βιβλίο δίνει 3 συνώνυμα, η γλώσσα 15 — σταθ. 33).

⭐ **ΜΟΝΟ ΤΟ ΓΥΜΝΟ ΠΕΡΑΣΜΑ (4/5) ΚΟΥΝΑΕΙ ΤΗ ΣΚΑΛΑ**, και μόνο τα περάσματα 4-5
γράφουν ακρίβεια. Τα δύο με νήματα μετράνε **επίτηδες τίποτα**: ακρίβεια παρμένη
με βοήθεια είναι η ψευδαίσθηση ευχέρειας, και θα ξαναπρογραμμάτιζε σε 10 μέρες
μια λέξη που ξέρει ΜΟΝΟ με το νήμα.

⚠️ Σκάλα `[0,1,3,10,30,90]` — **η σκάλα του σπιτιού με ΕΝΑ σκαλί μπροστά**, γιατί
το «1» είναι Η ΠΕΜΠΤΗ. `PASS 0.8` (όχι 0.9): εδώ η μονάδα είναι λήμμα με ως 9
στοιχεία, όχι σημείο με 3-4.

### ⚠️⚠️ ΤΡΕΙΣ ΠΑΓΙΔΕΣ ΠΟΥ ΚΟΣΤΙΣΑΝ ΩΡΑ ΚΑΙ ΜΕΤΑΦΕΡΟΝΤΑΙ ΠΑΝΤΟΥ
1. **⭐ ΣΤΑΘΕΡΗ ΑΡΧΗ 42 — ΤΟ `\b` ΕΙΝΑΙ ΤΥΦΛΟ ΣΤΑ ΕΛΛΗΝΙΚΑ.** Στη JavaScript
   `\w` = `[A-Za-z0-9_]`, άρα **κάθε ελληνικό γράμμα μετράει ως ΜΗ-λέξη** και το
   `/\bκαι\b/` δεν ταιριάζει ΠΟΤΕ σε ελληνικό κείμενο. Δεν σκάει· απλώς δεν
   κόβει — και το «άπειρος και άσχετος» γινόταν ΕΝΑ αδέσποτο, δηλαδή δύο σωστές
   απαντήσεις χαμένες σιωπηλά. Ο διαχωριστής γράφεται με **κενά** (`\s+και\s+`),
   και από πάνω μπαίνει δεύτερος φρουρός που πετάει το σκέτο «και» όπου κι αν
   στέκεται. **Ψάξε αυτό το `\b` σε κάθε ελληνικό regex του project.**
2. **⭐ ΟΙ ΔΥΟ ΑΠΟΔΕΚΤΕΣ ΟΡΘΟΓΡΑΦΙΕΣ ΑΝΗΚΟΥΝ ΣΤΟ ΥΛΙΚΟ, ΟΧΙ ΣΤΗ ΜΗΧΑΝΗ.** Ο
   χειρόγραφος πίνακας έσκασε στο «φιλεύσπλα**γ**χνος» — και είχε δίκιο, αλλά όχι
   όπως νόμιζα: «σπλάχνο» και «σπλάγχνο» είναι **δύο σωστές γραφές**. Ένας
   κανόνας «γχ ≡ χ» μέσα στο `greek-ear.js` θα άγγιζε ΚΑΙ την Ιστορία ΚΑΙ τα
   Αρχαία (σταθ. 25) για να λύσει πρόβλημα λεξικού. Το στοιχείο έγινε
   `{w, alt}` — η γνώση ζει εκεί που την έγραψε άνθρωπος που την ξέρει. Ίδιο
   θα ξανασυμβεί με αβγό/αυγό, τρένο/τραίνο, αφτί/αυτί.
3. **⛔ ΤΟ `als-sync` EVENT ΔΕΝ ΥΠΑΡΧΕΙ.** Μια πρώτη γραφή της σελίδας άκουγε
   `window.addEventListener('als-sync', …)` — **επινοημένο API**, που δεν θα
   είχε σκάσει ποτέ: απλώς η πρόοδος από το άλλο κινητό δεν θα φαινόταν ως το
   επόμενο refresh. Το συμβόλαιο είναι `initCloudSync({appKey, syncedKeys,
   onApplied})` και **αντιγράφεται αυτολεξεί από το `tonos.html`**.

### ⛔ ΤΟ ΥΛΙΚΟ ΓΡΑΦΕΤΑΙ ΑΠΟ ΑΝΘΡΩΠΟ ΠΟΥ ΕΙΔΕ ΤΗ ΣΕΛΙΔΑ
Ίδιος νόμος με το `istoria-data.js`: **ποτέ μοντέλο σε χρόνο εκτέλεσης.** Ένα
λάθος συνώνυμο που το μάθει είναι χειρότερο από συνώνυμο που δεν έμαθε — το
πρώτο το γράφει στο διαγώνισμα με σιγουριά. Η φωτογραφία γίνεται δεδομένα ΕΞΩ
από την εφαρμογή, ελέγχεται, μπαίνει στο `ekthesi-data.js`.
⚠️ **Τα `id` είναι λατινικά slug, ΠΟΤΕ αριθμοί σειράς** — μια σελίδα μπορεί να
ξαναδιαβαστεί και ένα `p547-07` θα μετακινούσε σιωπηλά την πρόοδο έξι λέξεων.
🔴 **ΜΙΑ ΓΡΑΜΜΗ ΘΕΛΕΙ ΕΠΙΒΕΒΑΙΩΣΗ ΑΠΟ ΤΟ ΒΙΒΛΙΟ:** το «ανυποχώρητο**ς**» στο
`p547-adiallaktos` — το τυπωμένο έσπαγε σε δύο σειρές και η άλλη ανάγνωση δίνει
«αυτοχώρητος», που δεν είναι λέξη.
