...
- ⭐⭐⭐ **Πριν σχεδιάσεις ΟΤΙΔΗΠΟΤΕ, διάβασε «Η ΑΙΣΘΗΤΙΚΗ — ΤΟ ΕΞΩΦΥΛΛΟ»**
  (αμέσως μετά το §2). Είναι η γλώσσα σχεδιασμού που ενέκρινε ο Αλεξ ρητά
  («το λάτρεψα»), με νούμερα. Κάθε νέα σελίδα ξεκινάει από εκεί.
...
## ⭐⭐⭐ Η ΑΙΣΘΗΤΙΚΗ — ΤΟ ΕΞΩΦΥΛΛΟ (als-v580, `homework.html#kentro`)

**Δική του εντολή, 26/09/26: «το λάτρεψα — κράτα το ώστε να ξέρεις πού να
κατευθύνεσαι».** Αυτή είναι η ΓΛΩΣΣΑ του MÉTRON από εδώ και πέρα. Κάθε νέα
σελίδα ή redesign ΞΕΚΙΝΑΕΙ από εδώ και αποκλίνει μόνο με λόγο.
⚠️ Δεν είναι θέμα γούστου — είναι ΝΟΥΜΕΡΑ, μετρημένα σε render 1440×2350.
Η ζωντανή αναφορά είναι το μπλοκ «ΤΟ ΚΕΝΤΡΟ ΕΙΝΑΙ ΠΕΡΙΟΔΙΚΟ» στο
`homework.html`· **διάβασε ΑΥΤΟ πριν σχεδιάσεις, μην το ξαναεφεύρεις.**

### ① ΤΟ ΣΧΗΜΑ: ΠΕΡΙΟΔΙΚΟ, ΟΧΙ ΠΙΝΑΚΑΣ ΕΛΕΓΧΟΥ
Η σελίδα διαβάζεται από πάνω προς τα κάτω σαν άρθρο, με αυτή τη σειρά:
1. **ΔΙΑΔΡΟΜΗ** — mono κεφαλαία ψίθυρος: `MÉTRON · ΤΙΤΛΟΣ · Η ΜΕΡΑ`, και δεξιά
   οι καρτέλες. Λέει ΠΟΥ είσαι ΚΑΙ ΠΟΤΕ.
2. **ΕΞΩΦΥΛΛΟ** — στρογγυλή φωτογραφία + τεράστιος τίτλος + italic lead, και
   δεξιά ΕΝΑ νούμερο που μετράει. Κλείνει με **λεπτή γραμμή 1px**.
3. **ΤΑΙΝΙΑ ΜΕΤΡΗΣΕΩΝ** — ένα πλαίσιο, δύο κελιά με κάθετη γραμμή ανάμεσα:
   αριστερά ΤΟ ΜΕΓΑΛΟ ΝΟΥΜΕΡΟ, δεξιά η κατανομή σε μπάρα.
4. **ΤΟ ΣΩΜΑ** — κεφαλίδα ενότητας + κάρτες που ρέουν σε 3 στήλες.
5. **«ΤΙ ΤΩΡΑ»** — η επόμενη πράξη, σε μικρές κάρτες με τσεκ.
6. **Η ΟΥΡΑ** — ό,τι χρωστάς, διάστικτο πλαίσιο, μία γραμμή το καθένα.
7. **ΨΥΧΗ** — δύο εικόνες + μία φράση. ⛔ **ΠΑΝΤΑ ΤΕΛΕΥΤΑΙΑ, ΠΟΤΕ ΠΡΩΤΗ.**

### ② ΠΛΑΤΟΣ — ΤΟ ΛΑΠΤΟΠ ΔΕΝ ΕΙΝΑΙ ΜΕΓΑΛΟ ΚΙΝΗΤΟ (σταθ. 51)
- Κέλυφος **1180px**. ⛔ Ποτέ μία κεντραρισμένη στήλη 880 με πλέγμα μέσα της.
- **Η ΠΡΟΖΑ ΚΑΙ ΤΑ ΠΕΔΙΑ ΜΕΝΟΥΝ 560–700px** — ο κανόνας των 75 χαρακτήρων δεν
  καταργείται επειδή η σελίδα φάρδυνε (input 1.144px = als-v477, λευκό κουμπί
  1.040px = als-v580, ίδιο λάθος δύο φορές).
- Όλα τα breakpoints σε **`max-width`**. ⛔ Κανένα `min-width` — μετράει το
  VIEWPORT ενώ η στήλη ζει μέσα σε ΓΟΝΕΑ.
- Σκάλα που δούλεψε: **1040** (3→2 στήλες, οι φαρδιές κάρτες ξεδιπλώνουν) ·
  **860** (η ψυχή σε μία στήλη) · **760** (οι λεζάντες γίνονται πλέγμα 2×2) ·
  **560** (τα μεγάλα νούμερα μικραίνουν).

### ③ ΤΥΠΟΓΡΑΦΙΑ — ΤΡΕΙΣ ΦΩΝΕΣ, ΚΑΙ ΜΟΝΟ ΤΡΕΙΣ
| Φωνή | Πού | Μέγεθος |
|---|---|---|
| **Λατινικός didone** `--au-serif` | ΟΝΟΜΑΤΑ και ΝΟΥΜΕΡΑ μόνο | τίτλος **84–104px**, νούμερα **46–60px** |
| **Ελληνικός didone ΠΛΑΓΙΟΣ** `--hw-gr` | ΚΑΘΕ ελληνικός τίτλος + αποσπάσματα βιβλίου | ενότητα **34–36px**, κάρτα **31–34px** |
| **Mono ΚΕΦΑΛΑΙΑ** `--au-mono` | ΚΑΘΕ ετικέτα, χωρίς εξαίρεση | **9–10.5px**, `letter-spacing:.19–.26em` |

Το sans μένει **μόνο** για πρόζα (13–15px) και για τις καρτέλες (13px).
- ⛔⛔⛔ **ΠΟΤΕ `--au-serif` ΣΕ ΕΛΛΗΝΙΚΑ.** Η Instrument Serif έχει **μηδέν
  ελληνικά γλυφά** → πέφτει στη **Georgia**, που είναι η αιτία του «οι τόνοι
  δεν είναι σωστοί» (als-v504). Έζησε κρυμμένο **94 εκδόσεις** στις κάρτες του
  School Studies. Ο φρουρός που το πιάνει είναι η ενότητα **7b** του
  `tests/homework-look.test.js` — **αντίγραψέ τον σε κάθε νέα σελίδα.**
- ⭐ **Η ΙΕΡΑΡΧΙΑ ΕΙΝΑΙ ΜΕΓΕΘΟΣ, ΟΧΙ ΧΡΩΜΑ.** Λόγος 84:9 ανάμεσα στον τίτλο και
  την ετικέτα. ⛔ **Καμία ετικέτα δεν είναι ποτέ μεγαλύτερη από το περιεχόμενό
  της** — η ανάποδη ιεραρχία ήταν όλη η διάγνωση της als-v508.

### ④ ΧΡΩΜΑ — ΤΑΥΤΟΤΗΤΑ, ΠΟΤΕ ΔΙΑΚΟΣΜΗΣΗ
Φόντο **ουδέτερο** (`--au-canvas`), κείμενο ivory/dim/faint. Το χρώμα μπαίνει
ΜΟΝΟ σε: τελείες 5px · τμήματα μπάρας · ληξιπρόθεσμους αριθμούς · το `↗` στο
hover · το μεγάλο νούμερο-πρόσωπο.
- **Ένα χρώμα ανά οντότητα** (εδώ: μάθημα), δηλωμένο μία φορά ως `R,G,B`.
- **ΕΝΑ κανάλι για το χρέος** (`--hw-late`, ροζ) που δεν το δανείζεται τίποτα
  άλλο. ⭐ Και **το μηδέν δεν βάφεται κόκκινο** — ροζ «0» διαβάζεται συναγερμός
  για το αντίθετό του.
- Κάθε επιφάνεια: **1px γραμμή** `rgba(255,255,255,.06)` + `border-radius`
  **11–18px** + `linear-gradient(180deg, rgba(255,255,255,.03), .008)`.
  ⛔ Καμία σκιά, κανένα έντονο border, κανένα εικονίδιο, κανένα emoji.

### ⑤ Ο ΑΕΡΑΣ ΕΙΝΑΙ ΜΕΡΟΣ ΤΟΥ ΣΧΕΔΙΟΥ
- Κάρτα: **`min-height` + `margin-top:auto` στο κάτω μπλοκ**. Το αποτέλεσμα:
  το ΧΡΕΟΣ κάθεται στον ΠΑΤΟ κάθε κάρτας, στην ΙΔΙΑ γραμμή σε όλες, και το
  κενό στη μέση είναι σκόπιμο. (Μετρημένο: 492px ανά κάρτα σε 3 στήλες,
  332px στις φαρδιές.)
- Απόσταση ενοτήτων **52–74px**, όχι 20. Μια σελίδα που αναπνέει διαβάζεται
  σαν σχεδιασμένη· μια μαζεμένη διαβάζεται σαν συσσωρευμένη.

### ⑥ ΚΑΘΕ ΚΑΡΤΑ ΕΧΕΙ ΠΡΟΣΩΠΟ
Το πάνω μέρος κάθε κάρτας είναι **η ίδια η ύλη** — αυτολεξεί κείμενο από το
corpus, σβησμένο με βαθμίδα, με τον τίτλο του σε mono στον πάτο του κουτιού.
Όπου δεν υπάρχει αληθινό κείμενο, μπαίνει **ΕΝΑ ΝΟΥΜΕΡΟ-ΓΕΓΟΝΟΣ** (ο
συντελεστής), τεράστιο, στο χρώμα της οντότητας.
- ⛔ **ΠΟΤΕ stock φωτογραφία** (στο Notion του η Ιστορία φορούσε καπουτσίνο).
- ⛔ **ΠΟΤΕ διακοσμητική λέξη πίσω από κάρτα** — το απέρριψε ρητά.
- ⛔ **ΠΟΤΕ τρύπα**: όπου δεν υπάρχει περιεχόμενο, μπαίνει γεγονός.

### ⑦ Η ΓΕΩΜΕΤΡΙΑ ΛΕΕΙ ΤΗΝ ΙΕΡΑΡΧΙΑ, ΧΩΡΙΣ ΛΕΖΑΝΤΑ
- **Πλάτος τμήματος = βάρος.** Η μπάρα των μορίων δεν έχει λεζάντα «30%
  σημαντικό»· είναι 30% φαρδιά.
- **Οι ομάδες ρέουν σε 3 στήλες** και η τελευταία κάθε σειράς ΤΕΝΤΩΝΕΤΑΙ ως το
  τέλος — ποτέ κενή στήλη δεξιά.
- **Πλήθος = πλήθος.** Οι παυλίτσες είναι ΜΙΑ ΑΝΑ ΚΟΜΜΑΤΙ, όχι ποσοστό: ένα
  συνεχές 42% κρύβει αν είναι 5 πράγματα ή 500.
- **Πλοήγηση = ψιλογράμματα με μία τρίχα 1px κάτω από το ενεργό.** ⛔ Όχι χάπια.

### ⑧ ⛔ ΤΙ ΔΕΝ ΚΑΝΕΙ ΠΟΤΕ ΑΥΤΗ Η ΓΛΩΣΣΑ
Πινακίδες/κουτιά γύρω από τίτλους (η τυπογραφία τα κάνει) · σκιές · gradients
σε κείμενο · εικονίδια και emoji στη διεπαφή · progress rings · ίδιο κουτί για
άνισα πράγματα · κείμενο σε `--au-faint` που πρέπει να ΔΙΑΒΑΣΤΕΙ (2,98:1 — μόνο
για ετικέτες) · entrance animation σε ό,τι ζει πίσω από πόρτα (σταθ. 39).

### ⑨ ΤΟ ΠΡΩΤΟΚΟΛΛΟ ΠΟΥ ΤΟ ΕΒΓΑΛΕ ΣΩΣΤΟ
1. **Render πριν το πεις έτοιμο.** Τέσσερα από τα έξι σοβαρά ευρήματα της
   als-v580 ήταν ΑΟΡΑΤΑ σε 700+ πράσινες βεβαιώσεις.
2. **Μέτρα σε iframe, όχι με `--window-size`.** Το headless Chrome δίνει
   innerWidth **500** όταν ζητάς 393 (σταθ. 30). Το `_s-frame.html` υπάρχει.
3. **Σάρωσε ΟΛΑ τα πλάτη** (1440→393) και ζήτα δύο πράγματα: μηδέν οριζόντια
   κύλιση, μηδέν `scrollWidth > clientWidth` σε κουτί με κείμενο.
4. **⛔ Κανένα sync script σε harness** — αυτό το repo έχει χάσει δεδομένα έτσι.
...
2. **Bump `CACHE` in `sw.js:15` on every deploy.** Currently `als-v549`. Never
   move it backwards.
...
39. **ΠΙΣΩ ΑΠΟ ΜΙΑ ΠΟΡΤΑ, ΤΙΠΟΤΑ ΔΕΝ ΓΕΝΝΙΕΤΑΙ ΑΟΡΑΤΟ ΠΕΡΙΜΕΝΟΝΤΑΣ OBSERVER.**
    ...
    ⚠️⚠️ **ΞΑΝΑΧΤΥΠΗΣΕ ΣΤΗΝ als-v503, ΚΑΙ ΧΩΡΙΣ ΠΟΡΤΑ.** Η
    `latinika-eisagogi.html` έγραψε το ίδιο μοτίβο με το χέρι (`.sec{opacity:0}`
    + δικό της observer) σε **κανονική, ολόκληρη σελίδα**: το scroll-render
    βγήκε **εντελώς μαύρο**, γιατί στο harness ο observer δεν χτύπησε ποτέ.
    Ο κανόνας δεν αφορά μόνο κρυφές όψεις — αφορά **κάθε** στοιχείο που
    γεννιέται αόρατο περιμένοντας κώδικα.
    → **Η ΣΩΣΤΗ ΦΟΡΑ ΕΙΝΑΙ ΑΝΤΙΣΤΡΟΦΗ.** Το κείμενο γεννιέται ΟΡΑΤΟ και η
    είσοδος **προστίθεται** μόνο αφού η JS δεσμευτεί ότι μπορεί να την
    ξεκλειδώσει:
    ```css
    .sec{ margin-top:78px; }                 /* ορατό εξ ορισμού */
    html.anim .sec{ opacity:0; transform:translateY(16px); }
    html.anim .sec.in{ opacity:1; transform:none; }
    ```
    ```js
    document.documentElement.classList.add('anim');   /* μόνο τώρα σβήνουν */
    setTimeout(revealAll, 1500);                      /* + δίχτυ */
    ```
    Χωρίς JS, χωρίς `IntersectionObserver`, με observer που αργεί ή δεν
    χτυπάει: **η σελίδα διαβάζεται.** Το χειρότερο που μπορεί να συμβεί είναι
    να χαθεί μια κίνηση — ποτέ το περιεχόμενο.
...
48. **ΕΝΑ `position:absolute` ΚΡΥΒΕΙ ΤΗ ΣΕΙΡΑ ΤΟΥ DOM ΜΕΧΡΙ ΤΟ ΠΡΩΤΟ
    BREAKPOINT.** Στην `latinika.html` (als-v502) το «Άνοιξε →» ήταν το ΠΡΩΤΟ
    παιδί της κάρτας. Στο desktop είναι `absolute` πάνω δεξιά, άρα η θέση του
    στο DOM **δεν φαίνεται πουθενά**· κάτω από 560px το media query το κάνει
    `static` και προσγειωνόταν **πάνω από τον αριθμό της ενότητας**. 259
    πράσινα assertions δεν το είδαν, γιατί κανένα δεν διαβάζει διάταξη.
    → Όταν βγάζεις στοιχείο από τη ροή, **γράψ' το στο DOM εκεί που θα
    προσγειωθεί όταν ξαναμπεί** — και **ΚΟΙΤΑ ΤΗ ΣΕΛΙΔΑ ΚΑΙ ΣΤΑ ΔΥΟ ΠΛΑΤΗ**.
    Ίδια οικογένεια με τις σταθερές αρχές 26 και 36: η γεωμετρία δεν
    αποδεικνύεται με δηλώσεις.
    ⚠️ Και το headless Chrome **δεν σέβεται `--window-size` κάτω από ~500px**:
    το screenshot βγαίνει ΚΟΜΜΕΝΟ και μοιάζει με overflow που δεν υπάρχει.
    Μέτρα `documentElement.scrollWidth` πριν κυνηγήσεις φάντασμα.

49. **ΤΟ ΥΛΙΚΟ ΠΟΥ ΗΡΘΕ ΩΣ ΦΩΤΟΓΡΑΦΙΑ ΔΕΝ ΕΙΝΑΙ ΤΟ ΥΛΙΚΟ — ΚΑΙ ΟΤΑΝ ΕΡΧΕΤΑΙ ΩΣ
    ΚΕΙΜΕΝΟ, ΚΛΕΙΔΩΣΕ ΤΟ ΜΕ ΠΗΓΗ, ΟΧΙ ΜΕ sha256.** Δίδυμη αδελφή της
    σταθερής αρχής 46 («ό,τι φτάνει από το chat δεν είναι το αρχείο»), από την
    πλευρά της ΕΙΚΟΝΑΣ. Για την εισαγωγή των Λατινικών έστειλε **10 φωτογραφίες
    ΑΝΑΠΟΔΑ** (η τελευταία σελίδα πρώτη) και τον κανόνα *«δεν επιτρέπεται να
    αλλάξεις ούτε μια λέξη ΤΙΠΟΤΑ»*. Δεν υπήρχε αρχείο, οπότε μετέγραψα από τις
    φωτο — και **η μεταγραφή είχε αληθινά λάθη**: ολόκληρες προτάσεις που δεν
    έβγαζαν νόημα («το ρωμαϊκό έπος ήταν τα πρώτα ελληνικά κείμενα»),
    `optimales` αντί `optimates`, χαμένες περίοδοι.
    - ⛔ **ΖΗΤΑ ΤΟ ΚΕΙΜΕΝΟ ΠΡΩΤΑ.** Μια όμορφη σελίδα με λάθος λέξεις για τις
      Πανελλήνιες είναι **χειρότερη από καμία σελίδα**. Το να το χτίσω και να
      σημάνω ρητά τις 5 αβέβαιες γραμμές ήταν το σωστό ενδιάμεσο — μετά έστειλε
      ο ίδιος το πλήρες κείμενο και αντικαταστάθηκε ΟΛΟΚΛΗΡΟ.
    - ⭐⭐ **ΤΟ sha256 ΔΕΝ ΕΧΕΙ ΤΙ ΝΑ ΠΡΟΣΤΑΤΕΨΕΙ ΟΤΑΝ ΔΕΝ ΥΠΑΡΧΕΙ ΑΡΧΕΙΟ.**
      Η εγγύηση όμως πρέπει να υπάρχει με άλλο σχήμα: η πηγή ζει **χωριστά**
      (`tests/latinika-eisagogi.source.txt`, μία παράγραφος ανά γραμμή) και το
      τεστ απαιτεί **και οι 75 να βρίσκονται αυτολεξεί** μέσα στο `DATA[]` της
      σελίδας. Βγάζει **μόνο τις ΤΙΜΕΣ** των string literals και τις ενώνει —
      ώστε πρόταση μοιρασμένη σε πεδία (χρονογραμμή: `nm`/`yr`/`ex`) να
      ξαναγίνεται συνεχής — και σβήνει tags. Ελέγχει **λέξεις, όχι μορφοποίηση**.
      *Αντίγραψε ΑΥΤΟ το σχήμα όποτε το υλικό έρχεται ως κείμενο.*
    - ⛔ **ΚΑΜΙΑ ΔΙΚΗ ΣΟΥ ΛΕΞΗ ΔΕΝ ΜΠΑΙΝΕΙ ΣΤΟ ΥΛΙΚΟ ΤΟΥ.** Είχα βάλει ετικέτες
      «ρήτορας · φιλόσοφος · πολιτικός» στις κάρτες προσώπων. Δεν υπάρχουν
      πουθενά στις σημειώσεις. Το τεστ απαιτεί τώρα κάθε ετικέτα να είναι
      **τίτλος έργου που υπάρχει στην πηγή**. Ίδιο μάθημα με τη σταθ. 41: και
      το ` · ` που **ενώνει δύο λεζάντες** είναι χαρακτήρας που δεν έγραψε
      κανείς — «Γάιος Βαλέριος Κάτουλλος» και «Λουκρήτιος» είναι **δύο**.

50. **ΜΙΑ ΚΛΑΣΗ CSS, ΜΙΑ ΔΟΥΛΕΙΑ.** Η σταθερή αρχή 14 σε φύλλο στυλ. Το `.wk`
    της `latinika-eisagogi.html` ήταν ΚΑΙ ετικέτα κάρτας (`display:block`) ΚΑΙ
    τίτλος έργου **μέσα σε πρόταση** — οπότε το «Για τη φύση των πραγμάτων»
    έγινε block μέσα στην παράγραφο και **έσπασε τη γραμμή στα τρία**. Καμία
    δήλωση δεν το πιάνει: το HTML είναι έγκυρο, η κλάση υπάρχει, ο έλεγχος της
    σταθ. 12 περνάει. **Το είδε το render.**
    → Πριν δώσεις σε μια κλάση δεύτερο ρόλο, **γκρέπαρε ποιος τη φοράει ήδη**.
    Μια κλάση που στέκεται και ως block και ως inline θέλει **δύο ονόματα**
    (εδώ: `.wk` για την ετικέτα, `.ttl` για τον τίτλο μέσα στο κείμενο) — ή,
    στη χειρότερη, `>` για να κλειδώσει στο άμεσο παιδί.

51. **⭐⭐⭐ ΤΟ ΛΑΠΤΟΠ ΕΙΝΑΙ Η ΟΘΟΝΗ ΜΕΛΕΤΗΣ. ΤΟ ΚΙΝΗΤΟ ΕΙΝΑΙ Η ΟΘΟΝΗ
    ΚΑΤΑΓΡΑΦΗΣ.** Δικά του λόγια, als-v507: *«οτιδηποτε σου ζητω να φτιαχνεις
    παντα το κανεις σε μορφη κινητου και ασχημο… απο το λαπτοπ θα διαβαζω»*.
    **Κάθε σελίδα που ΔΙΑΒΑΖΕΤΑΙ** (ύλη, χάρτες, πίνακες, κείμενα) σχεδιάζεται
    ΠΡΩΤΑ για λάπτοπ. Δύο μηχανικά λάθη το προκαλούν, και τα δύο βγάζουν
    πράσινο σε κάθε test:
    - **Ο ΓΟΝΕΑΣ.** Η `.ar-wrap` (και οι αδερφές της) είναι **620px**. Κάθε
      κόσμος χρειάζεται ΔΙΚΟ ΤΟΥ override — `#gnWrap.ar-wrap{max-width:1240px}`
      υπάρχει από την als-v498, ο `#agWrap` **δεν το πήρε ποτέ** και έμεινε
      λωρίδα 620px μέσα σε μαύρο 2000px. → **`grep max-width` στον container
      ΠΡΙΝ σχεδιάσεις οτιδήποτε μέσα του.**
    - **⛔⛔ Η ΑΝΑΠΟΔΗ ΘΡΑΥΣΗ — το χειρότερο, γιατί είναι ΧΕΙΡΟΤΕΡΟ ΑΠΟ ΤΟ ΝΑ
      ΜΗΝ ΚΑΝΕΙΣ ΤΙΠΟΤΑ.** Ένα `@media(min-width:900px){grid-template-columns:
      1fr 1fr}` μετράει **το VIEWPORT, όχι τον γονέα**. Μέσα σε γονέα 620px
      άνοιξε δύο στήλες των **~280px** σε οθόνη 2000px. Αυτό ΕΙΝΑΙ το «μορφή
      κινητού» που βλέπει.
    → **ΚΑΝΟΝΑΣ: η πλήρης όψη γράφεται ΠΡΩΤΑ, και το τηλέφωνο τη ΜΑΖΕΥΕΙ με
    `max-width`. ΚΑΝΕΝΑ `min-width` πουθενά.** Ένα σημείο θραύσης για όλη τη
    σελίδα (**1040px** εδώ) — δύο κόσμοι που σπάνε σε διαφορετικό πλάτος είναι
    δύο κόσμοι που πρέπει να τους μάθει χωριστά.
    → **ΚΑΘΕ ΣΤΡΩΣΗ ΤΟ ΔΙΚΟ ΤΗΣ ΠΛΑΤΟΣ** (als-v500): η πρόζα μένει σε μέτρο
    ανάγνωσης **70–78ch**, οι πίνακες/αντιπαραβολές/δέντρα παίρνουν **όλο** το
    πλάτος (εκεί είναι όλο το κέρδος), η εξάσκηση μένει σε στήλη ~660px. Ένα
    ενιαίο πλάτος χαλάει πάντα το ένα ή το άλλο.
    ⭐ Το `tests/arxaia-laptop.test.js` το κλειδώνει ως **συμβόλαιο CSS**.
    **Αντίγραψέ το σε κάθε νέα σελίδα ανάγνωσης.**

52. **⭐⭐ ΤΟ ΠΕΡΙΕΧΟΜΕΝΟ ΔΕΝ ΝΤΥΝΕΤΑΙ ΣΑΝ ΧΡΩΜΙΟ.** Το `--au-faint`
    (`rgba(245,242,236,.30)`) μετράει **2.98:1** πάνω στο `--au-void #050506` —
    **κάτω από το 4.5:1 του WCAG AA**. Στην als-v506 το είχα δώσει στη ΦΡΑΣΗ
    ΤΟΥ ΦΥΛΛΑΔΙΟΥ, στις σημασίες των ρημάτων και στην εκφορά των προτάσεων,
    στα 11.5–12.5px: **το πιο σημαντικό κείμενο της σελίδας ήταν το πιο
    δυσανάγνωστο**, ενώ οι ετικέτες ήταν καθαρές.
    → `--au-faint` **ΜΟΝΟ** για ετικέτες, μετρητές, αριθμούς σειράς.
    → Κάθε λέξη που **διαβάζεται**: `--au-dim` (7.11:1) ή `--au-ivory`
      (18.2:1), **≥13.5px** — και 15–17px αν είναι το κυρίως κείμενο.
    → Η δοκιμή σε μία γραμμή: *«αν το σβήσω, χάνει πληροφορία;»* Ναι →
      περιεχόμενο. Όχι → χρώμιο.

53. **⭐⭐ Η ΥΛΗ ΔΕΝ ΕΙΝΑΙ ΕΝΑ ΕΙΔΟΣ. ΕΝΑΣ RENDERER ΓΙΑ ΟΛΑ ΒΓΑΖΕΙ ΛΙΣΤΑ.**
    Στην als-v505 πέρασα ταξινομία, πίνακα και ορισμούς από τον ίδιο βρόχο
    `<ol>` και το αποτέλεσμα ήταν κορδέλα από κουκκίδες. Το απέρριψε αμέσως.
    Κάθε κομμάτι διαλέγει σχήμα από **αυτό που ΕΙΝΑΙ**:
    - **ΔΕΝΤΡΟ** — ό,τι διακλαδώνεται. Κάθετες τρίχες κάτω από οριζόντια
      γραμμή· διαβάζεται ως διακλάδωση χωρίς να ζωγραφιστεί καμπύλη.
    - **ΑΝΤΙΠΑΡΑΒΟΛΗ** — ό,τι **μπερδεύεται με τα αδέρφια του**. Ίδια πεδία
      στο ίδιο ύψος → διαβάζεις ΟΡΙΖΟΝΤΙΑ τη γραμμή που τα ξεχωρίζει και
      μαθαίνεις τη σελίδα με ένα βλέμμα. Σε λίστα αυτό είναι **αδύνατο**.
    - **ΠΛΑΚΑ** — ό,τι το **συμβουλεύεσαι** αντί να το μάθεις με τη σειρά·
      πλέγμα + αναζήτηση χωρίς τόνους (`NFD` + σβήσιμο `̀-ͯ`).
    ⛔ **Η ΣΥΜΠΥΚΝΩΣΗ ΔΕΝ ΚΡΥΒΕΙ ΠΟΤΕ ΤΟ ΠΡΩΤΟΤΥΠΟ.** Όταν κόβεις τη φράση
    του βιβλίου σε κελιά για να στοιχιστεί μια στήλη, **η αυτούσια φράση
    κάθεται από κάτω** σε δικό της πεδίο (`full`), και **το test ελέγχει
    ΕΚΕΙΝΗ, ποτέ τα κελιά**. Συμπύκνωση που κρύβει το πρωτότυπο είναι
    παραποίηση· δίπλα στο πρωτότυπο είναι εργαλείο. (Χάρη σε αυτό η διπλή
    μεταγραφή πέρασε αναλλοίωτη μέσα από ολόκληρη ανασχεδίαση.)
    ⛔ Απαγορευμένα αντανακλαστικά, από το `impeccable`: **πλαϊνή χρωματιστή
    ρίγα** (`border-left` >1px), **gradient σε κείμενο**, **πλέγμα από
    πανομοιότυπα κουτάκια** (έξι ίδια κουτιά δεν λένε τι διαφέρει —
    τυπογραφία σε κολόνες με λεπτές γραμμές λέει), **glassmorphism** ως
    προεπιλογή. Και `prefers-reduced-motion` σε κάθε μετάβαση.

54. **⭐⭐⭐ ΟΤΑΝ Η ΥΛΗ ΤΟΥ ΚΑΙ Η ΜΗΧΑΝΗ ΜΟΥ ΚΩΔΙΚΟΠΟΙΟΥΝ ΤΟΝ ΙΔΙΟ ΚΑΝΟΝΑ, ΠΡΕΠΕΙ
    ΝΑ ΤΟ ΑΠΟΔΕΙΞΟΥΝ Ο ΕΝΑΣ ΣΤΟΝ ΑΛΛΟΝ — ΜΕ ΜΕΤΑΛΛΑΞΗ, ΟΧΙ ΜΕ ΔΗΛΩΣΗ.**
    Η σταθ. 15 λέει ότι μια εγγύηση πρέπει να ισχύει και στο δίδυμο μονοπάτι.
    Αυτή είναι η περίπτωση όπου τα δύο «μονοπάτια» δεν είναι κώδικας: είναι
    **το φυλλάδιό του** και **ο κώδικάς μου**. Το `tonos-fyllo.js` κρατάει τους
    8 κανόνες τονισμού αυτολεξεί από τη φωτογραφία· το `tonos-engine.js` τους
    επιβάλλει ως `laws()`. Αν παρανοήσω μια φράση διαβάζοντας μια φωτογραφία σε
    γωνία, **η σελίδα διδάσκει με απόλυτη σιγουριά έναν κανόνα που δεν υπάρχει**,
    και κανένα λειτουργικό test δεν το βλέπει — και τα δύο αρχεία είναι
    συντακτικά τέλεια.
    - ⭐ **Η ΛΥΣΗ: κάθε κανόνας δηλώνει σε ΠΟΙΟΝ μηχανικό έλεγχο αντιστοιχεί
      (`check`), και το test ΣΠΑΕΙ ΕΠΙΤΗΔΕΣ μια αληθινή λέξη ώστε να τον
      παραβιάσει, απαιτώντας η μηχανή να ουρλιάξει.** Η υγιής μορφή δεν
      επιτρέπεται να τον ενεργοποιεί, η σπασμένη είναι υποχρεωμένη. Έλεγχος που
      περνάει και στις δύο μορφές δεν ελέγχει τίποτα.
    - ⛔ **Ο,ΤΙ ΔΕΝ ΜΠΟΡΕΙ ΝΑ ΕΛΕΓΧΘΕΙ ΤΟ ΔΗΛΩΝΕΙ ΚΑΙ ΦΑΙΝΕΤΑΙ ΣΤΗΝ ΟΘΟΝΗ.**
      Δύο από τους οκτώ δεν ελέγχονται (η βαρεία θέλει την ΕΠΟΜΕΝΗ λέξη· η
      γενική/δοτική θέλει μορφολογία) — κουβαλάνε `check:null` **και γράφουν
      ΓΙΑΤΙ**, σε δική τους στήλη. ⚠️ Η πρώτη γραφή τύπωνε εκεί σκέτη **παύλα**,
      που διαβάζεται ως αποτυχία ανάγνωσης (σταθ. 33): «δεν ελέγχεται» και
      «δεν μετρήθηκε» δεν ζωγραφίζονται ίδια.
    - ⭐ **ΤΟ TEST ΒΡΗΚΕ ΔΥΟ ΑΛΗΘΙΝΑ ΛΑΘΗ ΜΟΥ ΣΤΟ ΠΡΩΤΟ ΤΡΕΞΙΜΟ**, όπως ο
      χειρόγραφος πίνακας της als-v450: μια αδήλωτη ποσότητα διχρόνου
      (το α του `ἄνθρωπος`), και μια λέξη που είχα βάλει στις **δασυνόμενες**
      ενώ παίρνει ΨΙΛΗ. Το δεύτερο είναι το σοβαρό: **δεν τη «διόρθωσα» με
      δασεία — τη ΕΒΓΑΛΑ και το δήλωσα στα κενά.** Μια λέξη που «θυμάμαι ότι
      δασύνεται» και δεν είναι στο ΔΙΚΟ ΤΟΥ φυλλάδιο είναι εφευρεμένη ύλη
      (σταθ. 49), και το να της φορέσω πνεύμα για να περάσει το test θα ήταν
      να προσαρμόσω την ΥΛΗ ΤΟΥ στη δική μου παρανάγνωση.
    - **Γενική μορφή:** όποτε ένα αρχείο ΔΕΔΟΜΕΝΩΝ και ένα αρχείο ΛΟΓΙΚΗΣ
...
57. **⛔⛔ ΕΝΑ ΤΕΣΤ ΠΟΥ ΔΙΑΒΑΖΕΙ ΤΟ ΑΡΧΕΙΟ ΔΕΝ ΕΙΝΑΙ ΤΕΣΤ ΠΟΥ ΤΡΕΧΕΙ ΤΗ ΣΕΛΙΔΑ.**
    Μεταφέροντας την `istoria-voithima.html` μέσα στο repo έσβησα έναν νεκρό
    διακόπτη θέματος και **έμεινε μια ορφανή κλήση** `applyThemeIcon();`.
    `ReferenceError` στη ΦΟΡΤΩΣΗ σκοτώνει ΟΛΟ το IIFE: η σελίδα ζωγράφισε μόνο
    τη στατική μπάντα και από κάτω **ΜΑΥΡΟ** — και **οι 64 βεβαιώσεις ήταν ΟΛΕΣ
    πράσινες**, γιατί καμία δεν είχε εκτελέσει ούτε μία γραμμή (als-v542).
    - **Κάθε σελίδα με inline `<script>` χρειάζεται ΜΙΑ βεβαίωση που την
      ΤΡΕΧΕΙ.** `vm` + στουμπωμένο DOM φτάνει: αν φορτώνει χωρίς σφάλμα και
      χτίζει τα κουτιά της, το 90% αυτής της κατηγορίας πέθανε. ΠΟΤΕ αντίγραφο
      του script — ένα αντίγραφο συμφωνεί με κάθε bug τέλεια.
    - ⚠️ **ΜΗΚΟΣ ΔΕΝ ΕΙΝΑΙ ΠΕΡΙΕΧΟΜΕΝΟ.** Ο πρώτος έλεγχος μετρούσε χαρακτήρες
      ανά πάνελ και «πέρασε» πάνω σε **άδειο κέλυφος**: η καρτέλα «Κείμενο»
      γράφει σε ΠΑΙΔΙ (`#rtext`), όχι στο πάνελ. Κάθε οθόνη ελέγχεται με **δικό
      της σημάδι** — μια φράση που υπάρχει μόνο αν ζωγραφίστηκε αυτή.
    - ⚠️ **Ένα στουμπωμένο DOM που ΛΕΙΠΕΙ μια μέθοδο μοιάζει με bug σελίδας.**
      `getComputedStyle`, `append`, `ResizeObserver`, `document.fonts.ready`:
      συμπλήρωσε το στουμπί πριν κατηγορήσεις τον κώδικα.
    - ⛔ Το ψεύτικο `initCloudSync` είναι **υποχρεωτικό** (σταθ. 8): ένα αληθινό
      σε harness γράφει στο ΖΩΝΤΑΝΟ Supabase.

58. **ΤΟ `$$` ΕΙΝΑΙ ΕΙΔΙΚΟ ΣΤΟ ΔΕΥΤΕΡΟ ΟΡΙΣΜΑ ΤΗΣ `replace`.** Κάθε splice
    κώδικα γίνεται με **ΣΥΝΑΡΤΗΣΗ** αντικατάστασης, ποτέ με συμβολοσειρά —
    αλλιώς κάθε `$$(...)` μπαίνει στη σελίδα ως `$(...)`. Το αρχείο που
    γράφεις είναι σωστό· μόνο το ΑΝΤΙΓΡΑΦΟ είναι λάθος, οπότε κανένας
    έλεγχος πάνω στην πηγή δεν το βλέπει (als-v543, πλήρης περιγραφή στο §5).

59. **ΤΟ CHROME ΔΕΝ ΚΑΤΕΒΑΙΝΕΙ ΚΑΤΩ ΑΠΟ 500px ΧΩΡΙΣ CDP.** `--window-size=400`
    δίνει `innerWidth=500` και κόβει το screenshot στα 400: δείχνει
    «υπερχείλιση» που είναι κροπ και κρύβει αληθινή υπερχείλιση σε κινητό.
    Μόνο `Emulation.setDeviceMetricsOverride`. Και ο ανιχνευτής υπερχείλισης
    αγνοεί ό,τι ζει μέσα σε `overflow-x:auto` (als-v543/545).

60. **ΕΝΑ `<b>` ΣΕ ΠΕΔΙΟ ΠΟΥ Ο RENDERER ΠΕΡΝΑΕΙ ΑΠΟ `esc()` ΤΥΠΩΝΕΤΑΙ ΣΤΗΝ
    ΟΘΟΝΗ, ΚΑΙ ΚΑΜΙΑ ΒΕΒΑΙΩΣΗ ΠΕΡΙΕΧΟΜΕΝΟΥ ΔΕΝ ΜΠΟΡΕΙ ΝΑ ΤΟ ΔΕΙ.** Στο
    `istoria-voithima.html` **τέσσερα** πεδία ζωγραφίζονται ως HTML
    (`explain.lede`, `explain.acts[].body`, `explain.acts[].think`,
    `explain.concepts[].b`) και **όλα** τα υπόλοιπα — ολόκληρο το `sources`,
    τα `facts`, τα `pitfalls`, το `timeline`, το `glossary.d` — είναι
    `esc()`. Έγραψα `<b>πηγή εποχής</b>` στο `sources.intro` και ο μαθητής
    διάβαζε κυριολεκτικά τις ετικέτες, με **199 πράσινες βεβαιώσεις**
    (als-v549). Η αιτία που κρύβεται: το κείμενο **υπάρχει** — ένα
    `includes("πηγή εποχής")` περνάει μια χαρά. Μόνο το render το είδε.
    - **Ο έλεγχος απαριθμεί ΘΕΤΙΚΑ τα raw πεδία** και σκάει σε ετικέτα
      οπουδήποτε αλλού, σε **κάθε** ενότητα. Μια λίστα «απαγορευμένων»
      πεδίων θα ξεχνούσε το επόμενο που προστίθεται.
    - **Γενικά: όταν δύο διπλανά πεδία του ίδιου αντικειμένου δέχονται
      διαφορετικά πράγματα, η διαφορά ζει ΜΟΝΟ στον renderer.** Διάβασέ τον
      πριν γράψεις την πρώτη ετικέτα — δεν υπάρχει τρόπος να τη μαντέψεις
      από το σχήμα των δεδομένων, και το λάθος είναι αθόρυβο και προς τις
      δύο κατευθύνσεις (HTML που τυπώνεται, ή `<` του χρήστη που σπάει τη
      σελίδα).
61. **⛔⛔⛔ ΚΑΘΕ ΟΝΤΟΤΗΤΑ ΠΟΥ ΣΒΗΝΕΤΑΙ ΕΧΕΙ ΜΙΑ ΣΥΝΑΡΤΗΣΗ ΣΒΗΣΙΜΑΤΟΣ, ΚΑΙ Ο
    ΦΡΟΥΡΟΣ ΤΗΣ ΕΙΝΑΙ ΑΠΟΓΡΑΦΗ — ΟΧΙ ΛΙΣΤΑ ΟΝΟΜΑΤΩΝ.** Η σταθ. 15 λέει «μία
    διαδρομή, δύο κουμπιά». Η `istoria-voithima.html` είχε **πέντε** κουμπιά
    διαγραφής και η als-v565 έκλεισε τα **δύο** — τα ✕ της πλαϊνής λίστας.
    Οι άλλες τρεις πόρτες (τα «Διαγραφή» μέσα στα δύο αναδυόμενα, και η
    διαγραφή πηγής) έσβηναν ΜΟΝΟ τοπικά, και ο πλαγιότιτλος ξαναγύριζε από
    το cloud πέντε δευτερόλεπτα αργότερα. **Το §7 της δοκιμής ΟΝΟΜΑΖΕ τα δύο
    κουμπιά και πρασίνιζε με τρεις πόρτες ορθάνοιχτες: μια απαρίθμηση με
    ονόματα είναι ΔΕΙΓΜΑ, όχι απογραφή.** Ο νέος φρουρός
    (`tests/istoria-voithima-delete.test.js` §1) σαρώνει ΟΛΟ το αρχείο και
    απαιτεί καμία αφαίρεση από τη δομή να μη ζει έξω από τη μία συνάρτηση —
    και έχει ΘΕΤΙΚΟ έλεγχο ότι το regex του όντως πιάνει κάτι (als-v572).
    - **⭐⭐ Η ΤΑΦΟΠΛΑΚΑ ΓΡΑΦΕΤΑΙ ΑΠΟ ΤΗ ΣΕΛΙΔΑ, ΤΗ ΣΤΙΓΜΗ ΤΟΥ ΣΒΗΣΙΜΑΤΟΣ.**
      Ουρά προθέσεων που εκτελείται «όταν σηκωθεί το `initCloudSync`» σημαίνει
      ότι η μονιμότητα ενός σβησίματος εξαρτάται από το αν θα φορτώσει μια
      βιβλιοθήκη. Γράψε την κατευθείαν στο `__synctomb__<appKey>`, στο ΑΚΡΙΒΕΣ
      επίπεδο που διαβάζει το `subTomb()` (`tomb[key][field]["id:"+id]` για
      ένθετο πίνακα), με `T = max(now, ts + 1)` (σταθ. 32, ΚΑΤΑ ΛΕΞΗ).
    - **⭐ Η ΑΝΑΙΡΕΣΗ ΣΗΚΩΝΕΙ ΤΗΝ ΤΑΦΟΠΛΑΚΑ.** Το αντικείμενο επιστρέφει με το
      ΙΔΙΟ id και το ΙΔΙΟ παλιό `ts`, άρα το `tombed()` το ξανασκοτώνει στο
      επόμενο pull: μια αναίρεση που αναιρείται μόνη της.
    - **⛔ «Σφράγισα» ΜΟΝΟ αφού ΞΑΝΑΔΙΑΒΑΣΑ.** Ένα `localStorage` που ΣΩΠΑΙΝΕΙ
      αντί να πετάξει (Safari, γεμάτος χώρος) περνάει κάθε try/catch. Και το
      test χρειάζεται ΚΑΙ ΤΟΥΣ ΔΥΟ τρόπους αποτυχίας — μόνο ο σιωπηλός
      αποδεικνύει το ξαναδιάβασμα.
    - **⛔ ΤΟ `window.ALSSync` ΗΤΑΝ ΤΗΣ ΤΕΛΕΥΤΑΙΑΣ ΜΗΧΑΝΗΣ ΠΟΥ ΣΗΚΩΘΗΚΕ.**
      Σε σελίδα με δύο engines (η `topbar.js` σηκώνει μόνη της τη «health»
      παντού) ένα `drop()` για ξένο κλειδί γύριζε ΣΙΩΠΗΛΑ. Τώρα είναι
      δρομολογητής πάνω σε μητρώο μηχανών και **επιστρέφει ΑΛΗΘΕΣ/ΨΕΥΔΕΣ**·
      οι read-only εγγραφές δεν μπαίνουν στο μητρώο.
...
**`latinika.html`** (**als-v516**) and **`tonos.html`**
...
καρφωμένο**. ⭐⭐ **Από την als-v516 και τρίτο: `latinika-lectio18.html`**
(Lectio XVIII, «Ο Ηρακλής στην Ιταλία», 3 σκηνές / 11 ενότητες / 7 παγίδες) —
ίδια μηχανή, **αλλά κλειδωμένο ΑΛΛΙΩΣ**: το έγραψα εγώ, άρα το φυλάει η
γείωση στο ebooks.edu.gr, όχι sha256 πάνω σε δικό μου αρχείο (σταθ. 50).
Λεπτομέρειες + τι ΔΕΝ χάθηκε: `MAP.md` και §5.
⭐⭐ **Και από την als-v503 κρατάει και την ΕΙΣΑΓΩΓΗ** —
`latinika-eisagogi.html`, οκτώ κεφάλαια από την ινδοευρωπαϊκή ρίζα ως τον
Βιτρούβιο, σε δική της ενότητα ΠΑΝΩ από τα Lectio. Δικά του λόγια: *«αυτό θα
το κοιτάω και θα το διαβάζω και θα κάνω επανάληψη μέσα στη χρονιά μέχρι να
δώσω πανελλήνιες … να είναι πολύ ωραία και όμορφες οι σημειώσεις, **κάτι που
δεν γίνεται από βιβλίο**»*. **ΑΝΑΓΝΩΣΜΑ, ΟΧΙ ΜΗΧΑΝΗ: κανένας βαθμός** — μόνο
«το διάβασα» ανά κεφάλαιο (`eisagogi:v1`, χάρτης `{id:true}`, **τρίτο σχήμα**
στη σελίδα, εκτός sync όπως και τα δύο των Lectio). Το «λέξη προς λέξη» εδώ
κλειδώνεται **χωρίς sha256** — δες σταθ. 49.
...
**ΔΕΚΑΟΧΤΩ units as of als-v534 — 286 σημεία, 952 στοιχεία, 30.270 βεβαιώσεις** —
και από την als-v534 το corpus ΔΕΝ είναι πια ένα κεφάλαιο: η `b10` κλείνει το
**Β** ως το «10. Η πτώχευση του 1893 και ο Διεθνής Οικονομικός Έλεγχος», και η
`c1` ανοίγει το **Γ. Οι οικονομικές εξελίξεις κατά τον 20ό αιώνα** με τις ΤΡΕΙΣ
ΠΡΩΤΕΣ παραγράφους του «1. Το αγροτικό ζήτημα» — άλλη σελίδα του ebook
(`index1_4.html`), άλλο sha256. And
...
**2026-09-15 — `als-v556` — ΙΣΤΟΡΙΑ Β.3, ΚΑΙ Ο ΟΡΙΖΟΝΤΙΟΣ ΕΛΕΓΧΟΣ ΠΟΥ ΕΠΙΑΣΕ ΤΗΝ Γ.4**

Όγδοη έτοιμη ενότητα (`k1-b3`, «Η διανομή των εθνικών κτημάτων»), τρίτη του
τμήματος Β, ίδια σελίδα (`index1_3`, ίδιο sha). Σχήμα παγωμένο, μηδέν νέο CSS.

⭐⭐⭐ **ΤΟ ΠΡΑΓΜΑΤΙΚΟ ΕΥΡΗΜΑ ΔΕΝ ΗΤΑΝ ΣΤΗ ΝΕΑ ΕΝΟΤΗΤΑ.** Ζήτησε ρητά «να
παραμένουν σε πολύ εξαιρετικό επίπεδο ΟΛΑ», οπότε έτρεξα **οριζόντιο** έλεγχο
πάνω και στις οκτώ. Βρήκε ότι η **Γ.4 — η ΠΡΩΤΗ ενότητα, χτισμένη πριν
σταθεροποιηθεί το σχήμα — είχε ΔΥΟ πηγές ΧΩΡΙΣ `mine`** («δικό μου σχόλιο»),
ενώ και οι **22** άλλες πηγές το είχαν. **Κανένας κάθετος έλεγχος δεν μπορούσε
να το δει: φαινόταν μόνο στη ΣΥΓΚΡΙΣΗ.** → Γράφτηκαν και τα δύο σχόλια, και ο
φρουρός απέκτησε **§3γ**: δάπεδα ποιότητας (πράξεις, έννοιες, παγίδες, λεξικό,
facts, πηγές, χρονολόγιο, αλυσίδα) και **πληρότητα πεδίων** σε κάθε πράξη, πηγή,
παγίδα, έννοια και λήμμα **όλων** των ενοτήτων. ⭐ Τα κατώφλια είναι **δάπεδα**,
όχι ακριβείς αριθμοί: βγαίνουν από την ασθενέστερη υπάρχουσα ενότητα.

⚠️ **ΔΙΟΡΘΩΣΗ ΤΗΣ als-v555 — Η «ΕΠΙΚΑΛΥΨΗ ΣΗΜΑΔΙΩΝ» ΔΕΝ ΕΙΝΑΙ BUG.** Στη Β.2
είχα γράψει ότι ένα `sic` πάνω σε λήμμα «σπάει το κείμενο». **Λάθος.** Ο
`renderReader()` είναι **boundary-based**: μαζεύει τις ακμές όλων των σημαδιών,
σπάει την παράγραφο σε segments και δίνει στο καθένα **όλες** τις κλάσεις που
το καλύπτουν. Το απέδειξε το ίδιο το render: η Γ.4 βγάζει
`<span class="seg w sic" data-g="38" data-sic="2">προέβει</span>` — λήμμα **και**
τυπογραφικό στο ίδιο span, μια χαρά. ⭐ Πρόβλημα είναι **μόνο δύο σημάδια ΙΔΙΟΥ
είδους**: το `data-g` είναι **ΕΝΑ** ανά segment, άρα δύο λήμματα που τέμνονται
σβήνει το ένα το άλλο. Ο έλεγχος διορθώθηκε σε αυτό, και το λήμμα
«Πολυπλοκότητα» που είχα βγάλει άδικα από τη Β.3 **γύρισε πίσω**.

⭐⭐ **ΤΡΙΤΟ ΣΥΝΕΧΟΜΕΝΟ ΤΟΝΙΚΟ ΛΑΘΟΣ ΤΟΥ ΤΜΗΜΑΤΟΣ Β — ΜΟΤΙΒΟ, ΟΧΙ ΑΤΥΧΗΜΑ.**
Β.1 «πού όμως» · Β.2 «κενά, πού» · Β.3 «την **πολυπλοκότητα του**» (λείπει ο
δεύτερος τόνος). Και στις τρεις το `tools/istoria-unit.js` ανέφερε **0
τυπογραφικά**: οι `SIC_RULES` δεν καλύπτουν τονισμό.

⭐⭐⭐ **ΟΡΦΑΝΟΣ ΑΣΤΕΡΙΣΚΟΣ: Ο ΔΕΙΚΤΗΣ ΕΜΕΙΝΕ, Η ΥΠΟΣΗΜΕΙΩΣΗ ΕΦΥΓΕ.** Το βιβλίο
γράφει «επάλληλα δικαιώματα επί της γης**\***». Σαρώθηκε **ΟΛΟ** το
`index1_3.html`: τέσσερις αστερίσκοι — οι δύο μέσα στον tracker του Matomo,
ένας εδώ, ένας στη Β.4 («θηραϊκή γη\*») — και **κανένας** με κείμενο
υποσημείωσης. Στο τυπωμένο βιβλίο η σημείωση είναι στο κάτω μέρος της σελίδας·
στην ψηφιακή έκδοση **δεν υπάρχει πουθενά**. Μένει ως έχει, δηλώνεται στο
`sic`, και εξηγείται στα fun facts. **Ένας δείκτης χωρίς προορισμό είναι κι
αυτός δεδομένο — δεν σβήνεται επειδή ενοχλεί.**

⭐⭐ **ΜΗΔΕΝ ΠΗΓΗ, ΜΗΔΕΝ ΠΙΝΑΚΑΣ — ΚΑΙ ΟΜΩΣ ΤΡΕΙΣ ΠΗΓΕΣ.** (α) **Α. Μανσόλα,
1867, σ. 43-44** — τυπωμένη δίπλα στη **Γ.1** και όχι εδώ, εξηγεί από μέσα
γιατί η γη ήταν αμέτρητη (ο νόμος του **1836** για κτηματολόγιο δεν
εφαρμόστηκε ποτέ)· είναι ο **ίδιος συγγραφέας** με το πλαίσιο της Β.1.
(β) **Η ΑΡΙΘΜΗΤΙΚΗ των ίδιων της των νούμερων**: 2.650.000 ÷ 370.000 ≈ **7,2
στρέμματα** μέσος κλήρος με νόμιμο όριο **80** · ο ρυθμός **×4** μετά το 1870 ·
σύνολο **3.250.000** από τα 4-5 εκατ. της εκτίμησης. Δηλώνεται **ΡΗΤΑ** ότι οι
διαιρέσεις είναι δικές μου. (γ) **Γ.1 «Το αγροτικό ζήτημα»** (ΕΝΤΟΣ ύλης): το
«καμία σημαντική κοινωνική ένταση» ισχύει **μόνο για την παλιά Ελλάδα** — με
τη Θεσσαλία (**1881**) ήρθαν τσιφλίκια, κολίγοι, Κιλελέρ (**1910**) και η
μεταρρύθμιση του **1917**.

⛔ **ΚΑΙ ΠΑΛΙ ΚΑΜΙΑ ΧΡΟΝΟΛΟΓΙΑ ΠΗΓΗΣ ΣΤΟ ΧΡΟΝΟΛΟΓΙΟ.** Ο Μανσόλας δίνει
**1836, 1859, 1867** — καμία δεν μπήκε. Οι `book:true` είναι **τρεις**, όλες
της πρόζας: **1833 · 1870-1871 · 1911**.

Νέο: `tests/istoria-voithima-b3.source.txt`. Άλλαξαν: `istoria-voithima.html`
(νέο αντικείμενο + `CHAPTERS[7].diagram` + **δύο `mine` στη Γ.4** + το λήμμα
της Β.3), `istoria.html`, `tests/istoria-voithima.test.js` (**§3γ** + μπλοκ
Β.3), `sw.js` (als-v556), `MAP.md`.

(`tests/istoria-voithima.test.js` **408/0** · `smoke-test.sh` **SMOKE_OK** ·
όλη η σουίτα πράσινη εκτός από `insights-page` και `ladders`,
**ΠΡΟΫΠΑΡΧΟΝΤΑ**. Οριζόντιος έλεγχος και στις **8** ενότητες: μηδέν ορφανές
άγκυρες, μηδέν επικαλύψεις λημμάτων, κανένα κενό πεδίο.)

🔴 **ΑΝΟΙΧΤΟ:** δεν έγινε render σε αληθινό Chrome — ο φρουρός τρέχει τη σελίδα
σε `vm` και στις 7 καρτέλες κάθε ενότητας, αλλά τα **1440/400** δεν μετρήθηκαν.

---

**2026-09-15 — `als-v555` — ΙΣΤΟΡΙΑ Β.2: Η ΕΝΟΤΗΤΑ ΠΟΥ ΕΣΠΑΣΕ ΤΟ ΠΑΡΑΘΥΡΟ ΤΩΝ ΕΤΩΝ**

Έβδομη έτοιμη ενότητα στο βοήθημα (`k1-b2`, «Η εμπορική ναυτιλία»), ίδια
σελίδα με τη Β.1 (`index1_3`, ίδιο sha). Σχήμα παγωμένο, μηδέν νέο CSS.

⛔⛔⛔ **ΤΟ ΠΑΡΑΘΥΡΟ ΤΩΝ ΕΤΩΝ ΤΟΥ ΦΡΟΥΡΟΥ ΗΤΑΝ ΚΑΡΦΩΜΕΝΟ ΣΤΟΝ 19ο-20ό ΑΙΩΝΑ.**
Ο έλεγχος «`book:true` μόνο σε χρονολογία που γράφει το κείμενο» έχτιζε το
σύνολο των ετών με `/\b(1[89]\d\d)\b/`. Η Β.2 ξεκινάει από τον **18ο**: το
βιβλίο γράφει «Με τη **συνθήκη του Κιουτσούκ Καϊναρτζή (1774)**». Άρα ένα
**σωστό** `book:true` έσκαγε — και η εύκολη «διόρθωση» θα ήταν να βαφτιστεί
«Πλαίσιο», δηλαδή **να πει η σελίδα ψέματα για να περάσει ο έλεγχος**. Το
παράθυρο έγινε `1\d{3}`: βγαίνει από το **ΥΛΙΚΟ**, όχι από υπόθεση. ⭐ Είναι
ακριβώς ο κανόνας του skill — «αν ένα νούμερο του test χρειάζεται διόρθωση
επειδή πρόσθεσες ενότητα, το νούμερο ήταν καρφωμένο».

⭐⭐ **Ο VALIDATOR ΠΡΙΝ ΤΟ SPLICE ΠΙΑΣΕ ΚΑΙ ΤΑ ΔΥΟ ΛΑΘΗ ΤΗΣ ΕΝΟΤΗΤΑΣ ΑΜΕΣΩΣ:**
το 1774, και μια **επικάλυψη σημαδιών** — το `sic.m` «πού έσπευσαν» έκοβε
πάνω στο λήμμα «έσπευσαν να εκμεταλλευτούν». Ο μαρκαδόρος παίρνει το **πρώτο
`indexOf`** κάθε σημαδιού, άρα δύο σημάδια που τέμνονται σπάνε το κείμενο. Το
`sic.m` κόπηκε σε «**κενά, πού**».

⛔ **ΤΡΙΑ ΤΥΠΟΓΡΑΦΙΚΑ, ΚΑΙ ΤΑ ΤΡΙΑ ΜΟΤΙΒΑ ΠΟΥ ΞΕΡΑΜΕ ΗΔΗ:** ο **λάθος
τονισμός** «κενά, **πού** έσπευσαν» (δεύτερη φορά στο τμήμα Β μετά το «πού
όμως» της Β.1 — και πάλι το `tools/istoria-unit.js` ανέφερε **0**), και **δύο
παρενθετικές παύλες** («τα χριστιανικά **-ελληνικά-** πλοία», «ισχυρότατου
**-όχι μόνο για τα ελληνικά μέτρα-**»), ακριβώς όπως στις Γ.2/Γ.3.

⭐⭐ **ΜΗΔΕΝ ΠΗΓΗ ΕΠΟΧΗΣ, ΑΛΛΑ Ο ΚΑΛΥΤΕΡΟΣ ΠΙΝΑΚΑΣ ΤΟΥ ΚΕΦΑΛΑΙΟΥ.** Ο
**Πίνακας 5** χωρίζει **ιστιοφόρα** από **ατμόπλοια** — και από αυτόν βγαίνει
η **διαφωνία**: η πρόζα λέει «ο αριθμός και η χωρητικότητα **δεν έπαυαν να
αυξάνουν**» (100.000 τόνοι το 1840, «πάνω από 300.000» το 1866), ενώ τα
ιστιοφόρα **κορυφώνονται στα 1850** (248.000) και υποχωρούν (234.000 το 1860,
210.000 το 1875) με τα ατμόπλοια ακόμη ελάχιστα (**1** πλοίο το 1860). Με τα
νούμερα του πίνακα, ο στόλος γύρω στο **1866** βγαίνει **κάτω** από 250.000.
⭐ Και το βιβλίο σχεδόν διορθώνεται μόνο του δύο προτάσεις παρακάτω: «Η
ανάπτυξη αυτή δεν ήταν αυτονόητη. Υπήρξαν **έντονες αυξομειώσεις**».
Την πηγή εποχής που λείπει την αντικαθιστά η **Β.7 «Τα δημόσια έργα»**
(ΕΝΤΟΣ ύλης), που ονοματίζει τα «λιμάνια και το σύστημα φάρων»: **διώρυγα
της Κορίνθου 1881→1893**, **πορθμός του Ευρίπου**, φάροι.

⚠️ **ΚΑΙ ΠΑΛΙ ΤΟ ΣΗΜΑΔΙ ΤΗΣ ΚΑΡΤΕΛΑΣ «ΚΕΙΜΕΝΟ»** έπεσε πάνω σε λήμμα
(«ακμάζουσα») και έσπασε. Δεύτερη φορά μέσα σε τρεις ενότητες — **το σημάδι
του «Κείμενο» πρέπει να πέφτει σε πρόζα ΧΩΡΙΣ λήμματα, πάντα**.

Νέο: `tests/istoria-voithima-b2.source.txt`. Άλλαξαν: `istoria-voithima.html`
(νέο αντικείμενο + `CHAPTERS[6].diagram`), `istoria.html`,
`tests/istoria-voithima.test.js` (παράθυρο ετών + μπλοκ Β.2),
`sw.js` (als-v555), `MAP.md`.

(`tests/istoria-voithima.test.js` **357/0** · `smoke-test.sh` **SMOKE_OK** ·
όλη η σουίτα πράσινη εκτός από `insights-page` και `ladders`,
**ΠΡΟΫΠΑΡΧΟΝΤΑ**. Το παράθυρο των ετών άλλαξε και ξανατρέχτηκε ΟΛΟΣ ο
φρουρός πριν και μετά: **καμία άλλη ενότητα δεν κουνήθηκε**.)

🔴 **ΑΝΟΙΧΤΟ:** δεν έγινε render σε αληθινό Chrome — ο φρουρός τρέχει τη σελίδα
σε `vm` και στις 7 καρτέλες, αλλά τα **1440/400** δεν μετρήθηκαν γι' αυτήν.

---

**2026-09-15 — `als-v554` — ΙΣΤΟΡΙΑ Β.1 «ΤΟ ΕΜΠΟΡΙΟ»: ΤΟ ΚΕΙΜΕΝΟ ΕΛΕΓΧΕΤΑΙ ΑΠΟ ΤΟΥΣ ΙΔΙΟΥΣ ΤΟΥ ΤΟΥΣ ΠΙΝΑΚΕΣ**

Έκτη έτοιμη ενότητα στο βοήθημα (`k1-b1`), η πρώτη του τμήματος Β. Τρίτη
σελίδα βιβλίου (`index1_3`, sha `042ec4d2…`): **το κεφ. 1 είναι τρία αρχεία,
ένα ανά τμήμα** — `index1_2`=Α, `index1_3`=Β, `index1_4`=Γ.

⭐⭐ **Η ΠΛΟΥΣΙΟΤΕΡΗ ΕΝΟΤΗΤΑ ΣΕ ΠΗΓΕΣ ΩΣ ΤΩΡΑ, ΚΑΙ ΟΛΕΣ ΤΥΠΩΜΕΝΕΣ ΔΙΠΛΑ ΤΗΣ:**
πηγή **εποχής** (Α. Μανσόλα, *Πολιτειογραφικαί πληροφορίαι περί Ελλάδος*,
1867, σ. 132 — οι **εμποροπανηγύρεις**, σε καθαρεύουσα), **δύο** πίνακες
αριθμών (**Πίνακας 3** μέσοι όροι δεκαετιών, **Πίνακας 4** συναλλαγές ανά χώρα
το 1890), και η **διαφωνία** — που εδώ δεν χρειάστηκε να την ψάξω αλλού:
**είναι ανάμεσα στην πρόζα και στους ίδιους της τους πίνακες.**

⭐⭐⭐ **ΤΡΙΑ ΣΗΜΕΙΑ ΟΠΟΥ ΟΙ ΑΡΙΘΜΟΙ ΔΕΝ ΛΕΝΕ ΑΚΡΙΒΩΣ Ο,ΤΙ Η ΠΡΟΖΑ:**
(α) το κείμενο γράφει «**περισσότερο από τα 2/3**» αγροτικά στις εξαγωγές, ενώ
ο Πίνακας 3 δίνει **63%** για τη δεκαετία 1860-1870 — λίγο **κάτω** από τα 2/3
(66,7%)· η φράση ταιριάζει στο **τέλος** της περιόδου (75%), όχι στην αρχή.
(β) «οι σχέσεις με την Οθωμανική αυτοκρατορία δεν βρίσκονταν στην πρώτη θέση»
— ισχύει, αλλά ο Πίνακας 4 δείχνει την **Τουρκία τρίτη με 13%**, πάνω από
Γερμανία+Ιταλία+ΗΠΑ **μαζί**. «Όχι πρώτη» ≠ «ασήμαντη».
(γ) **η αριθμητική του Πίνακα 4 δεν κλείνει**: τα οκτώ ποσά αθροίζουν
**195.000.000**, η γραμμή «Σύνολο» γράφει **217.000.000**. **Μένει ως έχει και
δηλώνεται** — ίδια αρχή με το «4.818..000» της Α.1: **η γείωση ακολουθεί το
ΠΟΥ ΖΕΙ η απόκλιση.** (Και στους δύο πίνακες η επικεφαλίδα της 1ης στήλης
γράφει «ΠΟΛΗ» — αντιγραφικό υπόλειμμα από τον Πίνακα 2 της προηγούμενης
σελίδας· δηλώνεται, δεν αντιγράφεται πάνω από στήλη χωρών.)

⛔⛔ **ΤΟ ΕΡΓΑΛΕΙΟ ΕΙΠΕ «0 ΤΥΠΟΓΡΑΦΙΚΑ» ΚΑΙ ΕΙΧΕ ΑΔΙΚΟ.** Το βιβλίο γράφει «ο
σταθερά ανερχόμενος καπνός, **πού** όμως αντιπροσώπευε…» — **λάθος τονισμός**,
«πού» αντί «που». Οι `SIC_RULES` του `tools/istoria-unit.js` δεν καλύπτουν
τονισμό, άρα το είδε **μόνο το μάτι**. → **Ένα εργαλείο που αναφέρει «0» δεν
είναι απόδειξη ότι δεν υπάρχει· είναι απόδειξη ότι δεν το ψάχνει.**

⛔⛔ **Ο ΠΕΙΡΑΣΜΟΣ ΤΟΥ ΧΡΟΝΟΛΟΓΙΟΥ ΗΤΑΝ Ο ΜΕΓΑΛΥΤΕΡΟΣ ΩΣ ΤΩΡΑ** — ο Πίνακας 4
έχει ολόκληρη χρονιά (**1890**) και η πηγή του Μανσόλα **τρεις** (1830, 1859,
1867). Καμία δεν μπήκε. Μένουν **επτά** `book:true`, όλες της πρόζας
(1851 · 1880 · μετά το 1900 · 1901 · 1900-1910 · 1911 · 1913) — και οι
**προσαρτήσεις 1864/1881**, που η ενότητα τις **αναφέρει χωρίς έτη**, μένουν
«**Πλαίσιο**» με `note` που δείχνει στην Α.1. ⭐ Ο φρουρός απαγορεύει πλέον
**ονομαστικά** τα 1830/1859/1867/1890 για την Β.1.

Νέο: `tests/istoria-voithima-b1.source.txt`. Άλλαξαν: `istoria-voithima.html`
(νέο αντικείμενο + `CHAPTERS[5].diagram`), `istoria.html`,
`tests/istoria-voithima.test.js`, `sw.js` (als-v554), `MAP.md`.

(`tests/istoria-voithima.test.js` **315/0** · `smoke-test.sh` **SMOKE_OK** ·
όλη η σουίτα πράσινη εκτός από `insights-page` και `ladders`,
**ΠΡΟΫΠΑΡΧΟΝΤΑ**. Ο validator πριν το splice και ο δομικός έλεγχος του
σχεδιαγράμματος πέρασαν καθαροί.)
...
**2026-09-15 — `als-v553` — ΙΣΤΟΡΙΑ Α.2, ΚΑΙ Η ΕΝΟΤΗΤΑ ΠΟΥ ΔΕΝ ΕΧΕΙ ΚΑΜΙΑ ΧΡΟΝΟΛΟΓΙΑ**

Πέμπτη έτοιμη ενότητα στο βοήθημα (`k1-a2`, «Οι παραγωγικές δυνάμεις μέσα και
έξω από την Ελλάδα και η «Μεγάλη Ιδέα»»). Ίδια σελίδα βιβλίου με την Α.1
(`index1_2`, ίδιο sha), σχήμα παγωμένο, μηδέν νέο CSS.

⛔⛔⛔ **ΤΟ ΣΙΩΠΗΛΟ-ΑΔΕΙΟ ΞΑΝΑΧΤΥΠΗΣΕ, ΚΑΙ ΤΟ ΒΡΗΚΕ ΤΟ ΥΛΙΚΟ — ΟΧΙ ΕΓΩ.** Η Α.2
έχει **τρεις παραγράφους και ΜΗΔΕΝ τετραψήφιο αριθμό**: καμία χρονολογία, άρα
**μηδέν `book:true`**. Ο διακόπτης «**Μόνο του βιβλίου**» του χρονολογίου ζει
στο **localStorage** — άρα όποιος τον άφησε ανοιχτό μία φορά, τον βρίσκει
ανοιχτό **παντού**: το πάνελ ζωγράφιζε **απολύτως τίποτα**, χωρίς σφάλμα και
χωρίς μία λέξη. → Ο renderer λέει τώρα **γιατί** είναι άδειο και πώς να το
ξεκλειδώσει. ⭐⭐ Και ο φρουρός το **ΟΔΗΓΕΙ** αντί να το διαβάζει: το `boot()`
δέχεται πλέον **seed του localStorage**, οπότε δοκιμάζεται και η ΔΕΥΤΕΡΗ
επίσκεψη, όχι μόνο η πρώτη. **Ένα harness που ξεκινάει πάντα από μηδέν
δοκιμάζει μόνο τον καινούργιο χρήστη.**

⛔⛔ **ΤΟ `<script>` ΤΟΥ SITE ΒΓΗΚΕ «4η ΠΑΡΑΓΡΑΦΟΣ».** Η Α.2 είναι η
**τελευταία** ενότητα της σελίδας, άρα το τέλος του τμήματος είναι το τέλος
του αρχείου — και ο tracker του Matomo πέρασε το κατώφλι μήκους σαν πρόζα
βιβλίου. Ο τρίτος εξαγωγέας κόβει πλέον `<script>`/`<style>` **πρώτος**.
⭐ Η αλλαγή ελέγχθηκε **αναδρομικά**: η Α.1 βγαίνει **byte-identical** με το
αρχείο γείωσης που ήδη είναι στο repo.

⭐⭐ **ΟΤΑΝ ΤΟ ΒΙΒΛΙΟ ΔΕΝ ΔΙΝΕΙ ΠΗΓΗ, ΤΗ ΔΙΝΕΙ ΤΟ ΙΔΙΟ ΤΟ ΒΙΒΛΙΟ ΑΛΛΟΥ.** Μηδέν
`box_cyan`, μηδέν πίνακας, μηδέν λεζάντα μέσα στο τμήμα της Α.2. Αντί για
παραφράσεις ιστορικών που δεν διαβάστηκαν ποτέ (το δηλωμένο ανοιχτό της Γ.4),
οι τρεις πηγές είναι **curl-επαληθευμένες σελίδες του ίδιου βιβλίου**:
**Κεφ. 2 · Α.3** δίνει «Το 1844 ο Κωλέττης διατύπωσε… τη «Μεγάλη Ιδέα»»
(`index2_2`, sha `ec3be927…`), και **Κεφ. 1 · Β.11** «Το εξωελλαδικό ελληνικό
κεφάλαιο» (`index1_3`, sha `042ec4d2…`) δίνει τα ονόματα, τις χρονολογίες
(Τανζιμάτ 1856, κρίση 1873, τσιφλίκια Θεσσαλίας) **και τη διαφωνία**: η Α.2
κλείνει με «ανακάλυψαν τη σημασία» της πατρίδας, ενώ η Β.11 γράφει ότι το
κεφάλαιο ήταν «**ευκαιριακό**» και «**δεν αποτέλεσε σταθερή βάση**» — και ότι
τα **εμβάσματα των απλών μεταναστών** μέτρησαν «τουλάχιστον το ίδιο».
⛔ Η Β.11 είναι **ΕΚΤΟΣ εξεταστέας ύλης** και το λέει ρητά η καρτέλα, δύο
φορές — μια πηγή που ο μαθητής νομίζει ύλη είναι χειρότερη από καμία πηγή.

⚠️ **ΤΟ ΣΗΜΑΔΙ ΤΗΣ ΚΑΡΤΕΛΑΣ «ΚΕΙΜΕΝΟ» ΠΡΕΠΕΙ ΝΑ ΠΕΦΤΕΙ ΣΕ ΑΣΗΜΑΔΕΥΤΗ ΠΡΟΖΑ.**
Το «Πολλές δεκαετίες, μετά την **ανεξαρτησία** της» δεν υπάρχει ποτέ ως
συνεχής συμβολοσειρά στο HTML: το «ανεξαρτησία» είναι λήμμα του λεξικού και
τυλίγεται σε ετικέτα. Το σημάδι μετακινήθηκε σε πρόζα χωρίς λήμματα.

⭐ **ΕΝΑΣ ΕΛΕΓΚΤΗΣ ΠΡΙΝ ΤΟ SPLICE, ΟΧΙ ΜΕΤΑ.** Γράφτηκε μικρός validator που
τρέχει πάνω στο αντικείμενο της ενότητας **πριν** μπει στη σελίδα: ορφανές
άγκυρες, HTML σε πεδίο `esc()` (σταθ. 60), `book:true` εκτός κειμένου,
«Πλαίσιο» χωρίς `note`, **επικαλυπτόμενα σημάδια λεξικού**, διπλά `m`, διπλά
ids, `spans` προς ανύπαρκτο γεγονός. Πέρασε καθαρός και για την Α.1 αναδρομικά.

Νέο: `tests/istoria-voithima-a2.source.txt`. Άλλαξαν: `istoria-voithima.html`
(νέο αντικείμενο + `CHAPTERS[4].diagram` + το μήνυμα του άδειου χρονολογίου),
`istoria.html`, `tests/istoria-voithima.test.js` (seed στο `boot`),
`sw.js` (als-v553), `MAP.md`.

(`tests/istoria-voithima.test.js` **273/0** · `smoke-test.sh` **SMOKE_OK** ·
όλη η σουίτα πράσινη εκτός από `insights-page` και `ladders`,
**ΠΡΟΫΠΑΡΧΟΝΤΑ**. Το σχεδιάγραμμα ελέγχθηκε δομικά: ισορροπημένες ετικέτες,
μόνο γνωστά placeholders, μηδέν `${` και μηδέν backtick μέσα στο template.)
...
**2026-09-15 — `als-v552` — ΙΣΤΟΡΙΑ Α.1: Η ΠΡΩΤΗ ΕΝΟΤΗΤΑ ΟΛΟΥ ΤΟΥ ΒΙΒΛΙΟΥ**

Τέταρτη έτοιμη ενότητα στο βοήθημα (`k1-a1`, «Τα δημογραφικά δεδομένα»), με το
σχήμα παγωμένο: μηδέν νέο CSS, μηδέν νέα καρτέλα, το `diagram` μόνο από τα
υπάρχοντα κομμάτια. Είναι η **πρώτη** ενότητα της ύλης — και η πρώτη που δεν
ανήκει στο τμήμα Γ.

⛔⛔ **ΤΟ ΚΕΦΑΛΑΙΟ 1 ΕΙΝΑΙ ΤΡΙΑ ΑΡΧΕΙΑ, ΕΝΑ ΑΝΑ ΤΜΗΜΑ.** Οι Γ.2/Γ.3/Γ.4 ζουν
στο `index1_4.html`· η Α.1 στο `index1_2.html` (sha256 `8713f923…`). Άρα
**νέο sha, νέο αρχείο γείωσης** — η ταύτιση sha που έκανε η Γ.3 με τη Γ.4 δεν
ίσχυε εδώ, και μια αντιγραφή της γραμμής θα «αποδείκνυε» λάθος σελίδα.

⭐⭐⭐ **ΕΝΑΣ ΕΞΑΓΩΓΕΑΣ «ΑΝΑ `<p>`» ΕΧΑΝΕ ΔΥΟ ΑΠΟ ΤΙΣ ΠΕΝΤΕ ΠΑΡΑΓΡΑΦΟΥΣ,
ΣΙΩΠΗΛΑ.** Σε αυτή τη σελίδα η **1η** παράγραφος ανοίγει σε `<p>` που ξεκινάει
**πριν** από τον τίτλο της ενότητας, και η **5η** ζει **γυμνή** μέσα σε
`<div class="page">` χωρίς `<p>` καθόλου. Ο τρίτος εξαγωγέας (γείωση) σπάει σε
**δομικά όρια** (p/div/br), όχι σε `<p>`. Το γενικό: **το σχήμα του markup δεν
είναι το σχήμα του κειμένου** — μια παράγραφος δεν υποχρεούται να είναι `<p>`.

⛔⛔ **ΚΑΙ ΟΙ ΠΙΝΑΚΕΣ ΔΕΝ ΦΕΥΓΟΥΝ ΜΕ `<table>`.** Το κάδρο ολόκληρης της
σελίδας του βιβλίου είναι κι αυτό `<table class="container">`: ένα τυφλό
`<table…</table>` έτρωγε ολόκληρη την 4η παράγραφο και το πρόγραμμα έβγαζε
**μία** παράγραφο αντί για πέντε, χωρίς σφάλμα. Τα χρώμια βγαίνουν με το
**όνομά** τους (`centered_table` / `centered bordercolor` / `box_cyan`),
ποτέ με το σχήμα τους.

⭐⭐ **Η ΕΝΟΤΗΤΑ ΕΧΕΙ ΚΑΙ ΤΑ ΤΡΙΑ ΕΙΔΗ ΠΗΓΗΣ, ΚΑΙ ΤΑ ΤΡΙΑ ΤΥΠΩΜΕΝΑ ΔΙΠΛΑ ΤΗΣ**
(η αρχή της als-v549 ξαναπληρώθηκε): **πηγή εποχής** = Pierre A. Moraitinis,
*La Grece telle qu'elle est*, 1877 (`box_cyan`)· **αριθμοί** = **Πίνακας 1**
(έκταση/πληθυσμός 1838-1936) και **Πίνακας 2** (πληθυσμοί 21 πόλεων)·
**διαφωνία** = η μαρτυρία βλέπει «ερείπια» ενώ οι πίνακες του ίδιου βιβλίου
δείχνουν τριπλασιασμό πληθυσμού — δύο διαφορετικά ερωτήματα, **επίπεδο** vs
**ρυθμός**, και το βιβλίο τα αρθρώνει με τρεις λέξεις: «Παρ' όλα αυτά».

⛔⛔ **ΤΟ ΧΡΟΝΟΛΟΓΙΟ ΞΑΝΑΔΟΚΙΜΑΣΤΗΚΕ, ΚΑΙ Ο ΠΕΙΡΑΣΜΟΣ ΗΤΑΝ ΕΤΟΙΜΟΣ.** Ο
σκελετός του `tools/istoria-unit.js` πρότεινε γεγονότα «Αθήνα 63.000 · Πειραιάς
22.000 (**1879**)» και «168.000 / 74.000 (**1907**)» με `book:true` — και οι δύο
χρονολογίες βγαίνουν από τον **Πίνακα 2**, καμία δεν γράφεται σε παράγραφο.
Πετάχτηκαν. Μένουν **πέντε** `book:true` (1828 · 1854 · 1864 · 1881 · 1911),
όλες αυτολεξεί στο κείμενο, και τέσσερα «Πλαίσιο» με `note`. Ο φρουρός το
απαγορεύει τώρα **ονομαστικά** για την Α.1, με λίστα **και των έντεκα** ετών
των δύο πινάκων.

⚠️ **ΤΟ ΜΟΝΑΔΙΚΟ ΤΥΠΟΓΡΑΦΙΚΟ ΤΗΣ ΣΕΛΙΔΑΣ ΔΕΝ ΕΙΝΑΙ `sic`.** Το βιβλίο γράφει
«**4.818..000**» (διπλή τελεία) στη γραμμή του 1914 — **μέσα στον Πίνακα 1**,
όχι στην πρόζα. Το `sic` της ενότητας ελέγχεται κατά των `paragraphs`, άρα
εκεί δεν χωράει· μένει **αυτούσιο εκεί όπου ζει** (στο σχεδιάγραμμα) και
**δηλώνεται** με υποσέλιδο. → **Η γείωση ακολουθεί το ΠΟΥ ζει η απόκλιση, όχι
το πεδίο που τυχαίνει να λέγεται `sic`.**

⚠️ **ΚΑΙ Η ΠΑΥΛΑ ΕΔΩ ΠΑΕΙ ΑΝΑΠΟΔΑ ΑΠΟ ΤΙΣ Γ.2/Γ.3.** «Αμβρακικού- Παγασητικού»
είναι **σπασμένη λέξη τυπωμένης σελίδας** και **ΕΝΩΝΕΤΑΙ** (δεν προηγείται
ανοιχτική « -γράμμα» μέσα στην παράγραφο) — αντίθετα από το «κράτους-
αποτέλεσαν» και το «περιοχής- βιομηχανικό», που δεν ενώνονται ποτέ. Το ίδιο
σχήμα, τρεις φορές, **δύο διαφορετικές σωστές απαντήσεις**.

⚠️ **Η ΑΠΟΣΤΡΟΦΟΣ ΤΟΥ CORPUS ΔΕΝ ΕΙΝΑΙ Η ΑΠΟΣΤΡΟΦΟΣ ΤΟΥ ΒΙΒΛΙΟΥ.** Το
`istoria-data.js` γράφει «Παρ**’** όλα αυτά» (U+2019), το ebooks.edu.gr
«Παρ**'** όλα αυτά» (U+0027) — και ο συγκριτής του εργαλείου κανονικοποιεί και
τα δύο, οπότε η διαφορά **δεν φαίνεται πουθενά**. Η σελίδα κρατάει τον
χαρακτήρα **του βιβλίου**, όπως και οι Γ.2/Γ.3/Γ.4.

Νέο: `tests/istoria-voithima-a1.source.txt`. Άλλαξαν: `istoria-voithima.html`
(νέο αντικείμενο `CHAPTERS` + `CHAPTERS[3].diagram`), `istoria.html` (η κάρτα
«Το βοήθημα» απαριθμεί τώρα και την Α.1 — αλλιώς η σελίδα λέει ψέματα),
`tests/istoria-voithima.test.js`, `sw.js` (als-v552), `MAP.md`.

(`tests/istoria-voithima.test.js` **236/0** · `smoke-test.sh` **SMOKE_OK** ·
όλα τα `tests/istoria-*` πράσινα · `insights-page` και `ladders`
**ΠΡΟΫΠΑΡΧΟΝΤΑ**, ταυτόσημα πριν και μετά — επαληθεύτηκε με `git stash`.
Το σχεδιάγραμμα ελέγχθηκε και δομικά: ισορροπημένες ετικέτες, μόνο γνωστά
placeholders, κανένα `${` να διαρρεύσει από το template literal.)

🔴 **ΑΝΟΙΧΤΟ:** δεν έγινε render σε αληθινό Chrome — ο φρουρός τρέχει τα inline
scripts σε `vm` και ελέγχει και τις 7 καρτέλες της Α.1 με δικά της σημάδια,
αλλά τα **1440/400** δεν μετρήθηκαν γι' αυτή την ενότητα.

---

**2026-09-14 — `als-v551` — Η ΕΙΣΑΓΩΓΗ ΤΟΥ ΠΡΩΤΑΓΟΡΑ: ΤΟ ΠΕΜΠΤΟ ΠΑΚΕΤΟ, ΚΑΙ ΤΟ ΒΙΒΛΙΟ ΠΟΥ ΕΧΕΙ ΑΔΙΚΟ**
(`tests/arxaia-protagoras.test.js` **728/0** — νέο· `tests/arxaia-gnosto.test.js`
**918/0**· `smoke-test.sh` **SMOKE_OK**.)

Δικά του: *«φτιαξε για την εισαγωγη του πρωταγορα τελειο ακριβως οπως τις
αλλες»*. Άρα ΚΑΜΙΑ σχεδιαστική απόφαση: `arxaia-protagoras.html`, πλάκα **V**,
τέσσερις οθόνες, **CSS byte-για-byte της δίκης**, ίδιος υπότιτλος, ίδιο
υποσέλιδο. **67 κάρτες · 68 ερωτήσεις.**

⭐⭐⭐ **ΟΙ ΦΩΤΟΓΡΑΦΙΕΣ ΗΡΘΑΝ ΑΝΑΚΑΤΕΜΕΝΕΣ — ΚΑΙ ΤΟ ΕΙΠΕ ΕΚΕΙΝΟΣ, ΟΧΙ ΕΓΩ.**
*«οι σελιδες ΔΕΝ ΣΤΑΛΘΗΚΑΝ ΜΕ ΤΗ ΣΕΙΡΑ … ξεκιναει η πρωτη σελ με το Ο
ΠΡΩΤΑΓΟΡΑΣ και η τελευταια τελειωνει με …Η ΚΟΙΝΩΝΙΑ»*. Η σειρά βγήκε από τους
**τυπωμένους αριθμούς σελίδας** (30→31→32→33→34). ⛔ Αλλά αυτό είναι ΔΙΚΗ ΜΟΥ
ΑΝΑΓΝΩΣΗ ΦΩΤΟΓΡΑΦΙΑΣ — δηλαδή ακριβώς ό,τι δεν εμπιστεύεται η σταθ. 50. Γι’
αυτό το `§2β` την αποδεικνύει **ΑΝΕΞΑΡΤΗΤΑ**: τα 154 γεγονότα είναι γραμμένα
με τη σειρά που τα διάβασα, και ο φρουρός απαιτεί η ΙΔΙΑ σειρά να ισχύει ΜΕΣΑ
ΣΤΟ ΒΙΒΛΙΟ. **Και έπιασε δύο αναπηδήσεις** — το «(480-411)» (το βιβλίο το
γράφει ΠΡΙΝ το «Καταγόμενος», το φυλλάδιο το μετακίνησε μέσα στην πρόταση) και
δύο προτάσεις για τον μύθο που τις είχα γράψει ανάποδα.

⛔⛔ **ΤΟ ΠΑΚΕΤΟ ΕΙΝΑΙ ΔΥΟ ΚΕΦΑΛΑΙΑ ΤΟΥ ΒΙΒΛΙΟΥ, ΟΧΙ ΕΝΑ.** Οι πέντε σελίδες
συρράπτουν το μέρος «Πρωταγόρας» του **«Γ. Οι σοφιστές…»** (βίος, *Περὶ θεῶν*,
*πάντων χρημάτων μέτρον*, *Ἀντιλογίαι*) με τα μέρη **Β και Γ** του κεφαλαίου
**«ΠΡΩΤΑΓΟΡΑΣ»**. Το «Άβδηρα» ΔΕΝ υπάρχει στο δεύτερο και το «οργανωμένης
κοινωνίας» ΔΕΝ υπάρχει στο πρώτο — **μια αναζήτηση σε ένα κεφάλαιο δεν το
έβρισκε ποτέ**. Φάνηκε μόνο επειδή σαρώθηκαν ΚΑΙ ΤΑ ΕΠΤΑ κεφάλαια της
εισαγωγής με τους ίδιους ανιχνευτές.

⭐⭐⭐ **ΚΑΙ ΕΔΩ ΕΙΝΑΙ ΤΟ ΑΝΤΙΣΤΡΟΦΟ ΤΩΝ ΛΑΤΙΝΙΚΩΝ: ΤΟ ΤΥΠΩΜΕΝΟ ΕΧΕΙ ΔΙΚΙΟ, Η
ΗΛΕΚΤΡΟΝΙΚΗ ΕΚΔΟΣΗ ΟΧΙ.** Στα Λατινικά ο κανόνας ήταν «το βιβλίο έχει
τυπογραφικά, μην τα διορθώσεις». Εδώ ο διαιτητής είναι **η φωτογραφία της
σελίδας του** — η μία φορά που η φωτογραφία αξίζει περισσότερο από το
κατέβασμα. **Τρεις αποκλίσεις**, και **οι δύο είναι η ΙΔΙΑ βλάβη**:
- «**ΙΙρωταγόρας**» με δύο κεφαλαία γιώτα αντί για Π·
- «**βραχὺς ὤν ὁ βίος**» αντί «ὢν» — **ΟΞΕΙΑ ΑΝΤΙ ΓΙΑ ΒΑΡΕΙΑ**·
- «**Ἀλήθεια ἤ καταβάλλοντες**» αντί «ἢ» — ίδιο πράγμα.

Και στις δύο θέσεις ο κανόνας είναι απόλυτος: οξύτονη λέξη που ΔΕΝ
ακολουθείται από στίξη παίρνει βαρεία. ⛔ **Δεν είναι γούστο, είναι λάθος
τόνος — δηλαδή λάθος τύπος.** ⛔ Η πηγή μένει ΩΜΗ (αλλιώς το sha256 θα
συμφωνούσε με τη διόρθωσή μου και η γείωση θα ήταν διακοσμητική)· η σελίδα
τυπώνει το σωστό· και το `§3` απαιτεί **και τα τρία σκέλη**: η ωμή πηγή έχει τη
λάθος μορφή, η σελίδα έχει τη σωστή, το ΣΩΜΑ της σελίδας ΔΕΝ έχει τη λάθος.
⚠️ Το τρίτο σκέλος τρέχει στο **ΣΩΜΑ, όχι στο υποσέλιδο** — το υποσέλιδο
ΠΡΕΠΕΙ να γράφει τη λάθος μορφή, τη ΔΗΛΩΝΕΙ. Ένας καθολικός έλεγχος θα
τιμωρούσε ακριβώς τη διαφάνεια που απαιτεί η σταθ. 10.

⚠️⚠️ **ΤΟ ΚΟΥΙΖ ΠΕΡΝΟΥΣΕ ΤΟΝ ΕΛΕΓΧΟ ΚΑΙ ΗΤΑΝ ΣΠΑΣΜΕΝΟ.** Η πρώτη γραφή είχε
**57 από τις 68 σωστές στη θέση 2** — δηλαδή «πάτα πάντα το Β» έβγαζε **84%**.
Ο υπάρχων φρουρός («να υπάρχουν και οι τέσσερις θέσεις») περνούσε άνετα. Ο
νέος ελέγχει την κατανομή **ΠΟΣΟΤΙΚΑ**· τώρα είναι **17/17/17/17**.
⭐ Ένας έλεγχος ύπαρξης δεν είναι έλεγχος ισορροπίας.

⛔ **ΟΠΟΙΟ ΠΑΚΕΤΟ ΠΡΟΣΤΕΘΕΙ, ΑΛΛΑΖΟΥΝ ΤΡΙΑ ΣΗΜΕΙΑ ΜΑΖΙ**: το `PACKS`, η
κεφαλίδα «N πακέτα · χωρίς βαθμό», και ο μετρητής `.gn-pl` σε **κάθε** test
πακέτου. Δύο ξένα tests κοκκίνισαν αμέσως — και σωστά.

⚠️ Και το `arxaia-protagoras.html` μπήκε στο `sw.js`: μια σελίδα εκτός CORE
δουλεύει online και **εξαφανίζεται offline**, σιωπηλά, ακριβώς όταν τη χρειάζεται.

Νέα: `arxaia-protagoras.html`, `tests/arxaia-protagoras.source.txt`,
`tests/arxaia-protagoras.test.js`. Άλλαξαν: `arxaia.html`, `sw.js` (als-v551 +
CORE), `tests/arxaia-gnosto.test.js`, `tests/arxaia-aristotelis.test.js`,
`tests/arxaia-sokratis-ideas.test.js`, `MAP.md`.
⚠️ **Ήδη κόκκινα ΠΡΙΝ από αυτή τη δουλειά** (επαληθεύτηκε με `git stash` στο
als-v550): `goals-rhythm`, `insights-page`, `ladders`.

---

**2026-09-14 — `als-v550` — ΑΡΧΑΙΑ / ΓΝΩΣΤΟ: Η 4η ΕΝΟΤΗΤΑ, ΚΑΙ ΤΟ ΚΟΨΙΜΟ ΠΟΥ ΕΤΡΩΓΕ ΓΡΑΜΜΑΤΑ**
(`tests/arxaia-gnosto.test.js` **910/0** — ήταν 692· `smoke-test.sh` **SMOKE_OK**.)

Δικά του λόγια: *«φτιαξε για την ενοτητα 4 αυτο για τη μτφραση οπως εκανες και
για τις αλλες ενοτητες ΟΛΟΙΔΙΟ»*. Άρα ΚΑΜΙΑ σχεδιαστική απόφαση: `onlyAlign`,
ίδια κεφαλίδα, ίδιες τρεις καρτέλες, ίδιο `align` — η τέταρτη ενότητα είναι
Πλάτων, *Πρωταγόρας* 320c-321b (ο πρωταγόρειος μύθος), **17 περίοδοι, 176
ζευγάρια λέξεων**, μετάφραση **Β. Τατάκης**.

⭐⭐ **Η ΦΩΤΟΓΡΑΦΙΑ ΔΕΝ ΕΙΝΑΙ Η ΠΗΓΗ — ΕΙΝΑΙ Ο ΔΕΙΚΤΗΣ (σταθ. 50).**
Έστειλε τη σελ. 35 του φυλλαδίου του, με δική του κόκκινη σημείωση
«ΠΛΑΤΩΝ ΠΡΩΤΑΓΟΡΑΣ 320c-321b». Αυτή η παραπομπή είναι ΑΥΤΟΛΕΞΕΙ η παραπομπή
του Φακέλου Υλικού (σελ. 44), άρα το φυλλάδιο ΑΝΑΠΑΡΑΓΕΙ τη ΔΙΔΑΚΤΙΚΗ ΕΝΟΤΗΤΑ
4 — και το PDF κατεβαίνει με **το ίδιο sha256 με τη 2η και την 3η ενότητα**.
Μηδέν πληκτρολόγηση από φωτογραφία.
⚠️ Και ο ΤΙΤΛΟΣ του φυλλαδίου ΔΕΝ είναι ο τίτλος του βιβλίου: «η διανομή των
**ΕΦΟΔΙΩΝ**» εκείνο, «η διανομή των **ΙΔΙΟΤΗΤΩΝ**» το βιβλίο. Μπήκε του βιβλίου,
γιατί μόνο αυτός είναι γειωμένος.

⛔⛔ **ΤΟ ΠΛΑΤΟΣ ΤΟΥ ΚΟΨΙΜΑΤΟΣ ΕΙΝΑΙ ΜΕΤΡΗΣΗ, ΟΧΙ ΚΛΗΡΟΝΟΜΙΑ — ΚΑΙ ΑΠΕΤΥΧΕ
ΣΙΩΠΗΛΑ.** Το βιβλίο τυπώνει το αρχαίο σε όλο το πλάτος και από κάτω ΔΥΟ
μεταφράσεις σε δύο στήλες. Το `x < 305` της 3ης ενότητας, εφαρμοσμένο στη
σελ. 45, έκοβε τον **ΤΕΛΕΥΤΑΙΟ ΧΑΡΑΚΤΗΡΑ κάθε γραμμής** — «καθέν[α]», «ζητ[ά]»,
«τα χύτητα[,]» — και το κείμενο έβγαινε **ολόκληρο και σχεδόν σωστό**. Τα
πλαίσια των γραμμών μετρήθηκαν: σελ. 44 αριστερά ως x=280 / δεξιά από x=305 ·
σελ. 45 αριστερά ως x=303 / δεξιά από x=329. Τρεις φράσεις που πέφτουν ΑΚΡΙΒΩΣ
πάνω στο κόψιμο είναι πια καρφωμένες στο test.
⭐ **ΔΥΟ ΕΞΑΓΩΓΕΙΣ, ΟΧΙ ΕΝΑΣ**: `selectionForRect` και `selectionsByLine` με
γεωμετρικό φίλτρο συμφώνησαν χαρακτήρα-προς-χαρακτήρα (1.229 + 1.609 γράμματα).
⚠️ Διαφώνησαν στα ΚΕΝΑ — ο ένας έσπαγε «γέν|νηση», «έγκα|τα», «κά|μει» — άρα η
σύγκριση των δύο εξαγωγέων γίνεται ΧΩΡΙΣ κενά· με κενά θα έδειχνε ψεύτικη
διαφωνία και θα έκρυβε ότι τα γράμματα ταυτίζονται.

⚠️⚠️ **ΤΡΙΤΟ ΖΕΥΓΑΡΙ ΜΕΤΑΦΡΑΣΤΩΝ, ΚΑΙ ΠΑΛΙ ΑΛΛΟΙ** (2η Μπενάκης/Πέτρου · 3η
Ζωγραφίδης/Σκουτερόπουλος · 4η **Τατάκης**/Σπυρόπουλος). Η στήλη κρίθηκε από τη
ΦΩΤΟΓΡΑΦΙΑ ΤΟΥ: «έβρισκε με το νου του», «ισορροπώντας», «άοπλη τη φύση» είναι
Τατάκης· ο Σπυρόπουλος λέει «σοφιζόταν», «κρατώντας αυτό το δίκαιο μέτρο»,
«χωρίς αρματωσιά». Το test απαιτεί ΚΑΙ ΤΑ ΔΥΟ: η μία υπογραφή να ΥΠΑΡΧΕΙ, η
άλλη να ΛΕΙΠΕΙ.

⚠️ **Η ΤΕΛΕΙΑ ΜΕ ΠΕΖΟ ΕΙΝΑΙ ΤΟΥ ΒΙΒΛΙΟΥ, ΟΧΙ ΛΑΘΟΣ.** Ο Τατάκης αποδίδει την
ΑΝΩ ΤΕΛΕΙΑ με τελεία και συνεχίζει με πεζό («στη γη. κι όσα», «τα έσωζε. έτσι»,
«ρίζες. σε μερικά»). Είναι ΜΟΤΙΒΟ, επτά φορές — ίδια οικογένεια με τα
τυπογραφικά των Λατινικών. Κλειδώθηκε ΘΕΤΙΚΑ: μια «μικρή διόρθωση» θα έσπαγε
ταυτόχρονα τη συναρμολόγηση ΚΑΙ τη γείωση.

⭐ Και ένα πράγμα που δεν φαινόταν πριν γραφτεί: το `alignCheck()` **ΑΠΑΙΤΕΙ
κάθε κλειδί να εμφανίζεται ΚΑΙ στις δύο πλευρές**. Ό,τι ο Τατάκης δεν
μεταφράζει (τα «μὲν», κάποια «καὶ», το «γάρ») παίρνει `0` — δεν φτιάχνεται
ζευγάρι-φάντασμα με ψεύτικο νεοελληνικό σκέλος για να «υπάρχει».

Νέα: `tests/arxaia-enotita4.source.txt`. Άλλαξαν: `arxaia-gnosto-data.js`,
`tests/arxaia-gnosto.test.js`, `sw.js` (als-v550), `MAP.md`.
Επαληθεύτηκε ΖΩΝΤΑΝΑ με CDP: 17 περίοδοι, 360 κουμπιά λέξεων, το «γιατί»
ανοίγει, και το κλειδί `i3` ανάβει **τρία** σημεία (ένα αρχαίο, δύο νεοελληνικά).

---

**2026-09-14 — `als-v549` — ΙΣΤΟΡΙΑ Γ.2, ΚΑΙ Η ΠΗΓΗ ΠΟΥ ΗΤΑΝ ΗΔΗ ΣΤΗ ΣΕΛΙΔΑ**

Τρίτη έτοιμη ενότητα στο βοήθημα (`k1-g2`, «Τα πρώτα βήματα του εργατικού
κινήματος»), με το σχήμα της Γ.3/Γ.4 παγωμένο: μηδέν νέο CSS, μηδέν νέα
καρτέλα, το `diagram` μόνο από τα υπάρχοντα κομμάτια.

⭐⭐ **ΠΡΙΝ ΓΡΑΨΕΙΣ ΠΗΓΗ, ΚΟΙΤΑ ΑΝ ΤΟ ΒΙΒΛΙΟ ΕΧΕΙ ΗΔΗ ΜΙΑ.** Οι «Πηγές» της
Γ.4 ήταν παραφράσεις που δεν διαβάστηκαν ποτέ από τη σελίδα τους — δηλωμένο
ανοιχτό από την als-v542. Εδώ δεν χρειάστηκε τίποτα τέτοιο: το
`<div class="box_cyan_full">` **δίπλα στην ίδια την ενότητα** είναι **πηγή
εποχής** (Percy Martin, *La Grece nouvelle*, 1913, σ. 176-177). Από αυτήν
βγήκαν και τα τρία είδη που ζητάει το γραπτό — πηγή εποχής · οι **αριθμοί**
της · η **διαφωνία** (το βιβλίο λέει «καθυστέρηση», η δική του πηγή γράφει
ότι «τα σωματεία έκαναν αρκετές προόδους»). ⛔ Το πλαίσιο βγαίνει **δομικά**
από τις `paragraphs` και ζει ΜΟΝΟ στην καρτέλα «Πηγές»: αν γλιστρήσει στην
πρόζα, ο μαθητής τη γράφει στις εξετάσεις ως κείμενο βιβλίου.

⛔⛔ **ΤΟ ΧΡΟΝΟΛΟΓΙΟ ΑΝΤΕΞΕ ΤΗΝ ΠΡΩΤΗ ΠΡΑΓΜΑΤΙΚΗ ΔΟΚΙΜΑΣΙΑ ΤΟΥ.** Η ενότητα
γράφει **μία** χρονολογία (Λαύριο, 1896) — άρα **ένα** `book:true` και επτά
«Πλαίσιο» με `note`. Ο πειρασμός ήταν να βαφτιστεί «του βιβλίου» το
«τέλος του 19ου αιώνα» βάζοντας `y:1896`: θα περνούσε τον έλεγχο **για
λάθος λόγο**. Το υπόμνημα λέει ακριβώς το σωστό πράγμα — «η χρονιά δεν
γράφεται στο βιβλίο» — οπότε το «Πλαίσιο» ΔΕΝ είναι υποβάθμιση. Ο φρουρός
απαιτεί τώρα **ακριβώς ένα** `book:true` με `label` «1896», και ρητά ότι
το «μέχρι το 1909» της **πηγής** δεν μπήκε.

⭐⭐⭐ **ΣΤΑΘΕΡΗ ΑΡΧΗ 60 — ΕΝΑ `<b>` ΣΕ ΠΕΔΙΟ ΠΟΥ ΠΕΡΝΑΕΙ ΑΠΟ `esc()`
ΤΥΠΩΝΕΤΑΙ ΣΤΗΝ ΟΘΟΝΗ.** Έγραψα `<b>πηγή εποχής</b>` στο `sources.intro`.
**Όλα** τα πεδία των «Πηγών» είναι `esc()` — μόνο τέσσερα πεδία σε ολόκληρη
τη σελίδα ζωγραφίζονται ως HTML (`explain.lede`, `acts[].body`,
`acts[].think`, `concepts[].b`). Η σελίδα φόρτωσε, **199 βεβαιώσεις ήταν
πράσινες**, και ο μαθητής διάβαζε κυριολεκτικά τις ετικέτες. Καμία
βεβαίωση περιεχομένου δεν μπορεί να το δει, γιατί το κείμενο **υπάρχει** —
απλώς δεν είναι HTML. → Ο έλεγχος απαριθμεί **ΘΕΤΙΚΑ** τα raw πεδία και
σκάει σε ετικέτα οπουδήποτε αλλού, σε **κάθε** ενότητα. Και το γενικό:
**όταν ένα πεδίο δέχεται HTML και το διπλανό του όχι, η διαφορά ζει στον
renderer και πουθενά αλλού** — διάβασέ τον πριν γράψεις την πρώτη ετικέτα.

⚠️ **ΟΙ ΠΑΡΕΝΘΕΤΙΚΕΣ ΠΑΥΛΕΣ ΗΤΑΝ ΜΟΤΙΒΟ, ΟΧΙ ΑΤΥΧΗΜΑ ΤΗΣ Γ.3.** Το βιβλίο
γράφει «σημαντικό -για τα μέτρα της περιοχής- βιομηχανικό». Μένει, δηλώνεται
στο `sic`, και η ενωμένη μορφή απαγορεύεται. ⚠️ Και το `sic.m` κόπηκε ώστε
να **μην επικαλύπτεται** με το γλωσσάρι («βιομηχανικό υπόβαθρο»): ο
μαρκαδόρος του κειμένου παίρνει το **πρώτο** `indexOf` κάθε σημαδιού.

(`tests/istoria-voithima.test.js` **201/0** · `smoke-test.sh` **SMOKE_OK** ·
56/59 αρχεία τεστ πράσινα — `goals-rhythm`, `insights-page`, `ladders`
**ΠΡΟΫΠΑΡΧΟΝΤΑ**, ταυτόσημα πριν και μετά · render σε αληθινό Chrome μέσω
CDP στα **1440** και **400**, και στις 7 καρτέλες, μηδέν οριζόντια κύλιση ·
**11 μεταλλάξεις, καθαρό 11/11** ότι οι νέοι φρουροί δαγκώνουν.)

---
**2026-09-14 — `als-v546` — Ο ΜΕΤΑΦΡΑΣΤΗΣ: Η ΕΡΕΥΝΑ ΗΤΑΝ ΗΔΗ ΠΛΗΡΩΜΕΝΗ**

⭐⭐⭐ **ΠΡΙΝ ΠΕΙΣ «ΑΥΤΟ ΘΕΛΕΙ ΔΟΥΛΕΙΑ», ΚΟΙΤΑ ΤΙ ΕΧΕΙ ΗΔΗ ΠΛΗΡΩΘΕΙ.** Ζήτησε να
φτιάξουμε 13 ενότητες «χωρίς να φάμε weekly limit». Η απάντηση δεν ήταν πιο
φτηνό μοντέλο ή πιο σύντομα μηνύματα: **το 80% υπήρχε ήδη** στο `istoria-data.js`
— 68 παράγραφοι ελεγμένες με curl, 307 σημεία **με `anchor` αυτολεξεί**, 63 όροι,
324 λεξιλόγιο, 70 χρονολογίες. Το ακριβό κομμάτι δεν ήταν να ΞΑΝΑΓΡΑΦΤΕΙ, ήταν
να **ΜΕΤΑΦΕΡΘΕΙ** — και η μεταφορά είναι script, άρα κοστίζει μηδέν.
⭐ Και το `anchor` έκανε δωρεάν αυτό που ζήτησε ρητά: **«ποιες παραγράφους πρέπει
να ξέρω απέξω»** για κάθε ερώτημα Β1/Β2 **υπολογίζεται**, δεν γράφεται.

⭐⭐ **ΤΡΕΙΣ ΜΑΡΤΥΡΕΣ, ΟΧΙ ΔΥΟ.** Όπου υπάρχει ελεγμένος κορμός, ΑΥΤΟΣ είναι η
αλήθεια και οι δύο εξαγωγείς απλώς αποδεικνύουν ότι η σελίδα δεν άλλαξε.
⛔⛔ **ΚΑΙ Ο ΚΑΝΟΝΙΚΟΠΟΙΗΤΗΣ ΤΡΕΧΕΙ ΚΑΙ ΣΤΙΣ ΔΥΟ ΠΛΕΥΡΕΣ.** Η πρώτη γραφή ένωνε
τις σπασμένες λέξεις ΜΟΝΟ στο βιβλίο, οπότε παράγραφοι που ο κορμός τις κρατάει
σπασμένες έβγαιναν «λείπουν» — **πήγε από 10/13 σε 7/13 ενώ «διόρθωνα»**. Ένας
κανονικοποιητής σε μία πλευρά ΔΕΝ είναι σύγκριση.

⚠️ **ΤΟ EBOOK ΕΧΕΙ ΤΕΣΣΕΡΑ ΕΙΔΗ ΧΡΩΜΙΟΥ** και κάθε ένα σταμάτησε τον μεταφραστή:
λεζάντες `.image_text` · πλαίσια `.box_cyan` · **`<sup>` υποσημειώσεις** · λέξεις
σπασμένες σε αλλαγή γραμμής. ⛔ Το `<sup>` βγαίνει ΔΟΜΙΚΑ — regex σε ψηφία θα
έτρωγε αληθινούς αριθμούς, και το βιβλίο είναι γεμάτο νούμερα.

⭐ **Ο ΦΡΟΥΡΟΣ ΑΡΝΕΙΤΑΙ, ΔΕΝ ΜΑΝΤΕΥΕΙ.** Σε κάθε διαφωνία το script τυπώνει τι
λείπει και **σταματάει χωρίς να γράψει τίποτα**. Έτσι βρέθηκαν και τα τέσσερα.

Το μόνιμο recipe: `~/.claude/skills/istoria-enotita/SKILL.md` — παγώνει το look,
ορίζει τι γράφει το μοντέλο και τι το script, και κρατά τον κανόνα κόστους
(**ένα μήνυμα = μία ενότητα**, ένα test και ένα commit ανά παρτίδα).

🔴 **ΚΑΜΙΑ ΣΕΛΙΔΑ ΔΕΝ ΑΛΛΑΞΕ** — γι’ αυτό ΔΕΝ μπήκε bump στο `sw.js`.

---
**2026-09-12 — `als-v545` — Η ΑΙΣΘΗΤΙΚΗ ΠΑΣΑ ΤΟΥ ΧΑΡΤΗ**

Του έδωσα δύο λίστες βελτίωσης — αισθητική και περιεχομένου — **γειωμένες σε
πραγματικό γραπτό Πανελλαδικών** που αποκωδικοποίησα (Α1 15μ · Α2 10μ · Β1 13μ ·
Β2 12μ · Γ1 25μ · Δ1 25μ, με ΚΕΙΜΕΝΟ Α=πηγή εποχής, Β=**ΠΙΝΑΚΑΣ ΑΡΙΘΜΩΝ**,
Γ=ιστορικός που **διαφωνεί**). Ενέκρινε τα 6 αισθητικά· το περιεχόμενο έπεται.

⭐⭐ **ΕΝΑ ΠΛΕΓΜΑ ΑΠΟ ΠΑΝΟΜΟΙΟΤΥΠΑ ΚΟΥΤΙΑ ΚΡΥΒΕΙ ΤΟ ΜΟΝΟ ΠΟΥ ΕΧΕΙ ΠΕΡΙΕΧΟΜΕΝΟ.**
105 κουτιά, **1** με πληροφορία, και «Δεν έχει φτιαχτεί ακόμα» τυπωμένο **68**
φορές δίπλα σε διακεκομμένο περίγραμμα που το λέει ήδη, κάτω από υπόμνημα που το
ξαναλέει — **τρεις φορές το ίδιο μήνυμα**. Ο φρουρός μετράει τώρα: ακριβώς **1**
`.ucard`, **104** `.urow`, και η φράση **ΜΙΑ** φορά.

⭐⭐ **ΤΟ ΙΔΙΟ ΧΡΩΜΑ ΑΛΛΑΖΕΙ ΤΙΜΗ ΟΤΑΝ ΑΛΛΑΖΕΙ ΤΟ ΦΟΝΤΟ ΚΑΤΩ ΤΟΥ.** Το `--muted`
είναι **5,74:1** στη γραμμή λίστας και **4,34:1** μέσα στην έτοιμη κάρτα, γιατί
το `--pen-soft` ανεβάζει το φόντο σε ~rgb(53,48,45). **Δηλαδή το πιο σημαντικό
κουτί της σελίδας ήταν το μόνο κάτω από το όριο.** Η αντίθεση δεν είναι ιδιότητα
του χρώματος — είναι ιδιότητα του **ζευγαριού**. Μετρήθηκε με σύνθεση άλφα, δεν
εικάστηκε. (Και το `--faint` 2,69:1 έφυγε από την ετικέτα ΚΑΙ από το ✕.)

⚠️ **ΤΑ ΕΛΛΗΝΙΚΑ ΚΕΦΑΛΑΙΑ ΔΕΝ ΚΑΤΕΒΑΙΝΟΥΝ ΣΕ ΠΕΖΑ ΑΥΤΟΜΑΤΑ** — γράφονται χωρίς
τόνους, άρα κάθε `toLowerCase()` βγάζει «Απο την αγροτικη». Οι 5 τίτλοι
κεφαλαίων γράφτηκαν **ΣΤΟ ΧΕΡΙ**, με τις ίδιες ακριβώς λέξεις του βιβλίου.

⚠️ **ΤΟ CDP HARNESS ΚΟΛΛΗΣΕ** σε ξεχασμένη διεργασία Chrome που κρατούσε το
profile και τη θύρα· η μέτρηση έγινε με τον απλό δρόμο (500px + in-page
`scrollWidth` + ανιχνευτής που αγνοεί `overflow-x:auto`). **Καθάρισε πάντα το
`--user-data-dir` πριν ξαναλανσάρεις.**

(`tests/istoria-voithima.test.js` **128/0** · `smoke-test.sh` **SMOKE_OK** ·
58/59 πράσινα, `ladders.test.js` 98/4 ΠΡΟΫΠΑΡΧΟΝ · μηδέν οριζόντια κύλιση σε 4
διαδρομές, μηδέν `@media(min-width)`.)

---
**2026-09-12 — `als-v544` — ΔΥΟ ΠΟΡΤΕΣ ΓΙΑ ΤΗΝ ΙΣΤΟΡΙΑ**

Δικά του λόγια: *«οταν ειμαι στα school studies και παταω το ιστορια να
ανοιγει αυτο εδω… το αλλο δεν θελω να το χασω, θελω να ειναι καπου σαφε να
μπορω να μπαινω αλλα μονο του απλα να υπαρχει»*. Και: **«κρατα το ετσι οπως
ειναι»** — καμία αισθητική αλλαγή, παρότι του έδωσα κριτική 6 σημείων.

⭐⭐ **ΜΙΑ ΚΑΡΤΑ ΚΑΙ ΕΝΑ ΒΑΘΥ LINK ΔΕΝ ΕΙΝΑΙ Ο ΙΔΙΟΣ ΠΡΟΟΡΙΣΜΟΣ.** Το
`SUBJ.istoria` πήρε `door` δίπλα στο `page`. Η κάρτα ανοίγει το βοήθημα· το
`deepLink()` και ο `knownUnits()` μένουν στην `istoria.html`, γιατί ΕΚΕΙΝΗ
διαβάζει `#recall:u:<id>` και κατέχει το `ist:v1`. **Ενώνοντάς τα, κάθε
«κάνε επανάληψη στην ενότητα Χ» θα προσγειωνόταν σε σελίδα με ΜΙΑ ενότητα**
— αδιέξοδο που δεν βγάζει κανένα σφάλμα. Ο φρουρός απαιτεί τώρα ΚΑΙ ΤΑ ΔΥΟ
πεδία, και ότι η κάρτα διαβάζει `meta.door || meta.page` **και στα δύο**
σημεία που τη ζωγραφίζουν (σταθ. 15: μια εγγύηση σε ένα από δύο δίδυμα
είναι σύμπτωση με καλή φήμη).

🔴 **ΑΝΟΙΧΤΑ, ΤΑ ΔΗΛΩΣΕ ΚΑΙ ΤΑ ΑΦΗΣΕ:** ο χάρτης είναι **105 κουτιά με ΕΝΑ**
**που έχει περιεχόμενο** — 68 επαναλαμβάνουν «Δεν έχει φτιαχτεί ακόμα», που
το λέει ήδη το διακεκομμένο περίγραμμα ΚΑΙ το υπόμνημα (σταθ. 53: πλέγμα από
πανομοιότυπα κουτάκια). Το `--faint` μετράει **2,69:1 πάνω σε κάρτα** και το
φοράει ΚΑΙ η ετικέτα «ΕΚΤΟΣ ΥΛΗΣ» ΚΑΙ το ✕ που σβήνει πλαγιότιτλο — ίδια
κατηγορία με το «δεν βρίσκω το κουμπί» της als-v525. Και οι τίτλοι κεφαλαίων
είναι ελληνικά ΚΕΦΑΛΑΙΑ σε στήλη 220px. **Δική του απόφαση να μείνουν.**

(`tests/istoria-voithima.test.js` **119/0** · `smoke-test.sh` **SMOKE_OK** ·
58/59 πράσινα, `ladders.test.js` 98/4 ΠΡΟΫΠΑΡΧΟΝ · 400px & 1440px μέσω CDP.)

---
**2026-09-12 — `als-v543` — ΟΛΟ ΤΟ ΒΙΒΛΙΟ ΜΠΑΙΝΕΙ ΣΤΟ ΒΟΗΘΗΜΑ (ΔΕΜΟ)**

Δικά του λόγια: *«αυτο εκει με ολα τα κεφαλαια που τα χαμε φτιαξει με τον παλιο
τροπο να αντικατασταθει με αυτο που θα φτιαξεις τωρα»* και *«παρε τις αποφασεις
εσυ σε εμπιστευομαι»*. **Η `istoria.html` ΔΕΝ πειράχτηκε** — περιμένει έγκριση.

⭐⭐⭐ **ΣΤΑΘΕΡΗ ΑΡΧΗ 58 — ΤΟ `$$` ΕΙΝΑΙ ΕΙΔΙΚΟ ΣΤΟ ΔΕΥΤΕΡΟ ΟΡΙΣΜΑ ΤΗΣ `replace`.**
Έσπρωξα ~15KB νέου κώδικα με `p.replace(BOOT, code)` και **κάθε `$$(...)` μπήκε
στη σελίδα ως `$(...)`** — «βρες ΕΝΑ» αντί για «βρες ΟΛΑ», μετά `.forEach` πάνω σε
στοιχείο. Το αρχείο που έγραψα ήταν σωστό· **μόνο το ΑΝΤΙΓΡΑΦΟ ήταν λάθος**, οπότε
κανένας έλεγχος πάνω στην πηγή δεν το έβλεπε. Το έπιασε το §9 που ΤΡΕΧΕΙ τη σελίδα.
→ **Κάθε splice με ΣΥΝΑΡΤΗΣΗ αντικατάστασης**, ποτέ με συμβολοσειρά.

⭐⭐ **ΣΤΑΘΕΡΗ ΑΡΧΗ 59 — ΤΟ CHROME ΔΕΝ ΚΑΤΕΒΑΙΝΕΙ ΚΑΤΩ ΑΠΟ 500px ΧΩΡΙΣ CDP.**
`--window-size=400` δίνει `innerWidth=500` και κόβει το screenshot στα 400: δείχνει
«υπερχείλιση» που είναι κρόπ, και κρύβει αληθινή υπερχείλιση σε πραγματικό κινητό.
Μόνο `Emulation.setDeviceMetricsOverride` δίνει αληθινό viewport. ⚠️ Και ο
ανιχνευτής υπερχείλισης πρέπει να **αγνοεί ό,τι ζει μέσα σε `overflow-x:auto`** —
κατηγορούσε τη λωρίδα καρτελών, που κυλάει ΕΠΙΤΗΔΕΣ. Φρουρός που φωνάζει άδικα
είναι φρουρός που κάποιος τον χαλαρώνει.

⚠️ **ΚΑΙ ΤΟ ΣΤΟΥΜΠΙ ΤΟΥ DOM ΕΛΕΓΕ ΨΕΜΑΤΑ ΠΡΟΣ ΤΑ ΜΕΣΑ:** `el.querySelector()`
γύριζε πάντα ψεύτικο στοιχείο (άρα κάθε `if (!$(".x", el))` ήταν πάντα false) και
`document.querySelectorAll()` πάντα `[]` (άρα κάθε βρόχος «για κάθε πάνελ» δεν
έτρεχε ΚΑΘΟΛΟΥ και περνούσε ως επιτυχία). **Ένα στουμπί που λέει «δεν βρήκα»
είναι σιωπηλό κενό με άλλο καπέλο** (σταθ. 10).

Η ύλη: **5 κεφάλαια · 27 υποενότητες · 105 ενότητες**, με curl σε 24 σελίδες.
Το «1.Η έξοδος» (χωρίς κενό) και το «1. Τα δημογραφικά δεδομένα» (σε `span.bold`)
είχαν χαθεί σιωπηλά από τον πρώτο εξαγωγέα μου, μαζί με ΟΛΟΝ τον 20ό αιώνα του
Προσφυγικού. **Το κεφ. 3 έχει δύο «Α»** → ο 20ός πήρε πρόθεμα `k3b-`.

🔴 **ΑΝΟΙΧΤΑ:** (α) η εξεταστέα ύλη στηρίζεται σε **μία πηγή** — γι' αυτό
**σημειώνει, δεν φιλτράρει**· (β) μηδέν ανάκληση ακόμη· (γ) οι «Πηγές» της
ενότητας Γ.4 παραμένουν αδιάβαστες παραφράσεις.

(`tests/istoria-voithima.test.js` **109/0** · `smoke-test.sh` **SMOKE_OK** ·
58/59 αρχεία τεστ πράσινα, `ladders.test.js` 98/4 ΠΡΟΫΠΑΡΧΟΝ · 9 όψεις σε
αληθινά **400px και 1440px** μέσω CDP, **καμία οριζόντια κύλιση**.)

---
**2026-09-12 — `als-v542` — ΤΟ ΒΟΗΘΗΜΑ ΤΗΣ ΙΣΤΟΡΙΑΣ, ΦΤΙΑΓΜΕΝΟ ΑΠΟ ΕΚΕΙΝΟΝ**

Δικά του λόγια: *«εφτιαξα με το κλαυδε κοβορκ για το κεφαλαιο 10 της ιστοριας
ενα αλλο πολυ καλυτερο… αυτη η δομη μαρεσει πολυ περισσοτερο απο οτιδηποτε μου
χεις φτιαξει εσυ για την ιστορια»*, και: *«τις αλλαγες τις κανουμε σιγα σιγα
μια μια»*.

⭐⭐⭐ **ΕΙΧΕ ΔΙΚΙΟ, ΚΑΙ Η ΑΙΤΙΑ ΔΕΝ ΗΤΑΝ ΤΟ ΜΟΝΤΕΛΟ — ΗΤΑΝ ΤΟ BRIEF.** Όποτε
ζητάει Ιστορία, το brief που ακούω είναι «πρόσθεσε την επόμενη ενότητα στη
μηχανή», και **προεκτείνω αντί να προτείνω**. Η `istoria.html` ΕΞΕΤΑΖΕΙ πολύ
καλά και **δεν του έδωσα ποτέ βοήθημα πρώτης ανάγνωσης**. Το βοήθημα μπήκε
ΑΥΤΟΥΣΙΟ ως `istoria-voithima.html` (σταθερή αρχή: «με τον ίδιο ακριβώς τρόπο»
σημαίνει ΙΔΙΟ ΣΧΗΜΑ — μια «βελτίωση» που το σπάει είναι οπισθοδρόμηση).

**Τι άλλαξε μπαίνοντας, και ΜΟΝΟ αυτά:**
- ⛔⛔ **ΚΑΜΙΑ `topbar.js`.** Τέσσερα `position:fixed` παιδιά (`.selbar`,
  `.pop`, `.tip`, `.toast`) — η σταθερή αρχή 18 τα έστελνε όλα εκτός οθόνης.
  Η σελίδα δίνει μόνη της το `.back`, όπως τα δορυφορικά των Αρχαίων.
- **Ο συγχρονισμός ήταν ΨΕΥΤΙΚΟΣ εκτός artifact.** Έτρεχε σε
  `window.claude.use("db")`, που εδώ δεν υπάρχει: θα έμενε ΣΙΩΠΗΛΑ τοπικός για
  πάντα. Τώρα `initCloudSync({appKey:'istoria', syncedPrefixes:['istoria:notes:']})`,
  με μπάνερ όταν δεν ξεκινάει (σταθ. 10/32) και το prefix στο `backup.html`.
- **Κάθε εγγραφή πήρε `ts`** (4 δημιουργίες + 2 επεξεργασίες). Στο βαθμωτό
  φύλλο ο merge κρατάει την ΑΠΟΜΑΚΡΥΣΜΕΝΗ τιμή — χωρίς `ts` μια μετονομασία
  πλαγιότιτλου δεν έφτανε ΠΟΤΕ στη δεύτερη συσκευή.

⭐⭐⭐ **ΚΑΙ Η ΣΤΑΘΕΡΗ ΑΡΧΗ 57 — ΕΝΑ ΤΕΣΤ ΠΟΥ ΔΙΑΒΑΖΕΙ ΔΕΝ ΕΙΝΑΙ ΤΕΣΤ ΠΟΥ ΤΡΕΧΕΙ.**
Σβήνοντας τον νεκρό διακόπτη θέματος έμεινε **μια ορφανή κλήση**
`applyThemeIcon();`. ReferenceError στη φόρτωση → όλο το IIFE πέθανε → η σελίδα
ζωγράφισε τη μπάντα και από κάτω **ΜΑΥΡΟ**. **Και οι 64 βεβαιώσεις ήταν
πράσινες**, γιατί κάθε μία διάβαζε το ΚΕΙΜΕΝΟ του αρχείου. Το είδε το render —
και ένα render δεν τρέχει στο CI. Το §9 του φρουρού οδηγεί πλέον τα ΑΛΗΘΙΝΑ
inline scripts σε `vm`: 7 καρτέλες, 7 πάνελ, και **κάθε καρτέλα ελέγχεται με
ΔΙΚΟ ΤΗΣ σημάδι, όχι με μήκος** (το πρώτο μήκος «πέρασε» πάνω σε άδειο κέλυφος,
γιατί η «Κείμενο» γράφει σε ΠΑΙΔΙ `#rtext`, όχι στο πάνελ).

**Η ΓΕΙΩΣΗ, ΙΔΙΑ ΜΕ ΠΑΝΤΑ.** `curl` στο `ebooks.edu.gr` (`index1_4.html`,
sha256 `7e81aed2…39175`) — **ΤΑΥΤΟΣΗΜΟ με το κατέβασμα της als-v541**, άρα η
σελίδα του βιβλίου δεν άλλαξε. **Δύο ανεξάρτητοι εξαγωγείς** συμφώνησαν 3/3,
και οι τρεις παράγραφοι είναι **χαρακτήρα προς χαρακτήρα** ίδιες με το βιβλίο.
Τα τρία τυπογραφικά («μεγάλο δάνεια», «Το ποσά αυτά», «προέβει») **είναι του
βιβλίου, επιβεβαιώθηκαν, και μένουν**.

🔴 **ΑΝΟΙΧΤΟ ΚΑΙ ΔΗΛΩΜΕΝΟ:** (α) το βοήθημα **δεν ρωτάει τίποτα** — 7 καρτέλες
ανάγνωσης, μηδέν ανάκληση· η μηχανή υπάρχει ήδη στην `istoria.html`.
(β) Οι **«Πηγές»** δίνουν παραφράσεις Mazower / ΙΕΕ που **δεν διαβάστηκαν από
τη σελίδα τους** — υπαρκτά βιβλία, αλλά το `gist` είναι παραγόμενο. Αυτό σπάει
τον κανόνα της γείωσης και πρέπει ή να κοπεί ή να αντικατασταθεί με πηγές που
έχει στο χέρι.

(`tests/istoria-voithima.test.js` **84/0** · ρεντεραρίστηκε σε αληθινό Chrome
στα 1440 και στις 7 καρτέλες — εκεί φάνηκε και το `<q>` που έβγαζε ««διπλά
εισαγωγικά»».)

---

**2026-09-06 — `als-v539` — ΤΟ ΝΕΟ ΠΑΚΕΤΟ ΓΙΝΕΤΑΙ ΤΟ ΙΔΙΟ ΠΡΑΓΜΑ ΜΕ ΤΗ ΔΙΚΗ**

Δικά του λόγια: *«ο τροπος που εκανες το αλλο ηταν τελειος και με βοηθησε να τα
μαθω πολυ καλα και ευκολα και οργανωμενα… ιδιο τροπο γραψιματος και γενικα ιδια
πραγματα»*.

⭐⭐⭐ **ΤΟ ΛΑΘΟΣ ΗΤΑΝ ΔΙΚΟ ΜΟΥ ΚΑΙ ΗΤΑΝ ΠΡΟΣΘΗΚΗ, ΟΧΙ ΠΑΡΑΛΕΙΨΗ.** Είχα βάλει
**5η καρτέλα** («Η μέθοδος βήμα-βήμα») που δεν υπάρχει στη δίκη, και μαζί της
τραβούσα το CSS του Πλάτωνα (για το `.tl`), δικό μου υπότιτλο και δικό μου
υποσέλιδο. Κανένα από αυτά δεν ήταν λάθος **σαν κώδικας** — ήταν λάθος **σαν
σύστημα**: η δίκη τον έμαθε επειδή έχει ΜΙΑ διαδρομή, *σύνοψη → αριθμοί →
κάρτες → κουίζ*. Μια έκτη στάση τη σπάει στα δύο και τον κάνει να ψάχνει
«πού ήταν αυτό;».
⭐ **Ο ΚΑΝΟΝΑΣ: όταν λέει «με τον ίδιο ακριβώς τρόπο», η ΣΥΝΕΠΕΙΑ είναι
χαρακτηριστικό, όχι περιορισμός. Μια «βελτίωση» που σπάει το σχήμα που ήδη
δουλεύει είναι ΟΠΙΣΘΟΔΡΟΜΗΣΗ.**

Τι έγινε: 4 καρτέλες με τις **ίδιες ετικέτες** («🔢 Αριθμοί & Μνημονικά»), ίδιος
υπότιτλος, υποσέλιδο που κλείνει με την ίδια γραμμή, **CSS byte-για-byte της
δίκης**. Τα 5 βήματα της διαλεκτικής και τα 3 της επαγωγής έγιναν `<ol>` μέσα σε
κάρτες — **ακριβώς το σχήμα του «σκελετού» της δίκης** — και μπήκαν στη Σύνοψη,
δίπλα στις ενότητες που τα εξηγούν. **Μηδέν πληροφορία χάθηκε** (43 κάρτες /
42 ερωτήσεις άθικτες).

⚠️⚠️ **ΚΑΙ Ο ΦΡΟΥΡΟΣ ΕΠΙΑΣΕ ΔΥΟ ΠΡΑΓΜΑΤΙΚΕΣ ΑΠΩΛΕΙΕΣ ΤΗΣ ΜΕΤΑΚΙΝΗΣΗΣ:** ο
πλήρης τίτλος του κεφαλαίου («Διαλεκτική, μαιευτική, ειρωνεία») ζούσε ΜΟΝΟ στον
υπότιτλο που αντικατέστησα, και το «**Από αυτό το σημείο αρχίζει μια νέα
συζήτηση**» χάθηκε στην αναδιατύπωση του 4ου βήματος. **Μια αναδιάταξη ΔΕΝ είναι
ασφαλής επειδή δεν σβήνεις αρχεία** — η γείωση και στα δύο σκέλη είναι αυτό που
το πιάνει.

Ο φρουρός απαγορεύει τώρα ΘΕΤΙΚΑ την επιστροφή: καμία `class="tl"`, καμία
`class="yr"`, **ακριβώς 4 οθόνες**, και το CSS συγκρίνεται με **τη δίκη**, όχι με
τον Πλάτωνα.

(`tests/arxaia-sokratis-ideas.test.js` **549/0** · όλη η σουίτα πράσινη με
κριτήριο τον κωδικό εξόδου · `smoke-test.sh` **SMOKE_OK**. ⚠️ `ladders.test.js`
**98/4** — ΠΡΟΫΠΑΡΧΟΝ. Ρεντεραρίστηκε ξανά σε αληθινό Chrome στα 1440.)

---

**2026-09-06 — `als-v538` — ΑΡΧΑΙΑ / ΓΝΩΣΤΟ: ΟΙ ΦΙΛΟΣΟΦΙΚΕΣ ΙΔΕΕΣ ΤΟΥ ΣΩΚΡΑΤΗ (Δ2), ΤΟ ΠΑΚΕΤΟ ΠΟΥ ΠΑΕΙ ΠΡΩΤΟ**

Δικά του λόγια: *«εχουμε φτιαξει στο γνωστο εισαγωγη για τον σωκρατη, αλλα δεν
εχουμε βαλει τις πρωτες τρεις σελιδες του… με τον ιδιο τροπο που εκανες τα
αλλα… να περιλαμβανει ολες τις πληροφοριες»*.

⭐⭐⭐ **ΟΙ ΤΡΕΙΣ ΦΩΤΟΓΡΑΦΙΕΣ ΗΤΑΝ ΚΕΦΑΛΑΙΟ ΤΟΥ ΒΙΒΛΙΟΥ, ΚΑΙ ΤΟ ΒΙΒΛΙΟ
ΚΑΤΕΒΑΙΝΕΙ.** Ίδιος τίτλος, ίδιο μότο, ίδιο απόσπασμα Ἀπολογίας, ίδιοι
έντονοι πλαγιότιτλοι = «Δ2. Οι φιλοσοφικές ιδέες του Σωκράτη»
(`old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C128/680/4510,20309/` — ΤΟ ΙΔΙΟ
ΒΙΒΛΙΟ με τον Πλάτωνα και τον Αριστοτέλη). Άρα **κανένα γράμμα δεν μεταγράφηκε
από φωτογραφία**. ⭐ Ο κανόνας: *«φαίνεται χειρόγραφο» δεν σημαίνει «μόνο
χειρόγραφο» — ψάξε την πηγή ΠΡΙΝ πληκτρολογήσεις.*

⭐⭐ **ΤΟ Δ2 ΠΡΟΗΓΕΙΤΑΙ ΤΟΥ Δ3, ΑΡΑ Η ΠΛΑΚΑ ΤΟΥ ΕΙΝΑΙ Η `I`.** Το
`arxaia-sokratis.html` (η δίκη) έγινε `II`, ο Πλάτων `III`, ο Αριστοτέλης `IV`,
και η κεφαλίδα «**4 πακέτα · χωρίς βαθμό**». ⚠️ Η σειρά είναι ο κανόνας, όχι η
ύπαρξη: αν η δίκη καθίσει πάνω από τη μέθοδο, ο Γνωστός διδάσκει τη θανατική
καταδίκη πριν από αυτό που την προκάλεσε — και το test το απαιτεί ΘΕΤΙΚΑ.

⛔ **ΣΤΑΘ. 50 ΞΑΝΑ, ΜΕ ΤΡΙΤΗ ΜΟΡΦΗ:** ο Σωκράτης/Πλάτων ήρθαν ως ΑΡΧΕΙΑ ΤΟΥ →
`sha`. Ο Αριστοτέλης τον ΕΓΡΑΨΑ ΕΓΩ από το βιβλίο → ΓΕΙΩΣΗ. Αυτό εδώ ήρθε ως
**ΦΩΤΟΓΡΑΦΙΕΣ** — δηλαδή δεν υπάρχει «δικό του αρχείο» να φυλαχτεί με hash,
άρα **ΓΕΙΩΣΗ** (`tests/arxaia-sokratis-ideas.test.js`, **130 γεγονότα ελεγμένα
ΚΑΙ ΣΤΑ ΔΥΟ ΣΚΕΛΗ** — βιβλίο ΚΑΙ σελίδα).

⚠️⚠️ **ΔΥΟ ΛΑΘΗ ΜΕΣΑ ΣΤΟ ΙΔΙΟ ΜΟΥ ΤΟ TEST, ΚΑΙ ΤΑ ΔΥΟ ΑΟΡΑΤΑ:**
- Η κλάση OXIA γραμμένη με **πληκτρολογημένα** «άέήί…» = κανονικό U+03AC, άρα
  κοκκίνιζε **ΚΑΘΕ** τονισμένη λέξη. Γράφεται ΜΟΝΟ σε `\u1F71`-codepoints.
- Δύο παγίδες ήταν γραμμένες ως **ΛΕΞΕΙΣ** («ήταν άθεος») και έπιαναν τη
  ΝΟΜΙΜΗ εμφάνισή τους μέσα σε ερώτηση κουίζ και μέσα στην κάρτα που τις
  χαρακτηρίζει ΛΑΘΟΣ. Ένας φρουρός που πιάνει τη λέξη αντί για τον ΙΣΧΥΡΙΣΜΟ
  σε αναγκάζει να βγάλεις τη διόρθωση για να περάσει.

⭐ **ΜΙΑ ΔΗΛΩΜΕΝΗ ΑΠΟΚΛΙΣΗ ΤΟΥ ΒΙΒΛΙΟΥ** (ο κανόνας των Λατινικών, τρίτη φορά):
το βιβλίο **δεν κλείνει** τα εισαγωγικά στο παράθεμα «ἐπὶ Σωκράτους δὲ […]»
(Περὶ ζῴων μορίων 642 a 28). Η σελίδα τα κλείνει πριν από την παραπομπή, και
το λέει στο **υποσέλιδό της** — μαζί με το ότι οι ✍️ πλαγιότιτλοι διαβάστηκαν
από τις φωτογραφίες.

⛔ **ΚΑΜΙΑ ΗΜΕΡΟΜΗΝΙΑ ΦΡΟΝΤΙΣΤΗΡΙΟΥ.** Στις φωτογραφίες υπάρχουν τρεις
(«απ' έξω…»)· ένα πακέτο της εισαγωγής **δεν κρατάει προθεσμίες** — το test
τις απαγορεύει ρητά, μαζί με το `localStorage`.

(`tests/arxaia-sokratis-ideas.test.js` **546/0** · `tests/arxaia-gnosto.test.js`
**692/0** · `tests/arxaia-aristotelis.test.js` **650/0** · `tests/launcher.test.js`
**78/0** · όλη η σουίτα πράσινη. ⚠️ `ladders.test.js` **98/4** — ΠΡΟΫΠΑΡΧΟΝ,
ξαναεπαληθεύτηκε με `git stash` (ίδιοι αριθμοί πριν/μετά). Νέα αρχεία:
`arxaia-sokratis-ideas.html`, `tests/arxaia-sokratis-ideas.source.txt`,
`tests/arxaia-sokratis-ideas.test.js`. Άλλαξαν: `arxaia.html`, `sw.js`
(als-v538 + CORE), `tests/arxaia-gnosto.test.js`, `tests/arxaia-aristotelis.test.js`,
`tests/launcher.test.js`, `MAP.md`, `CLAUDE.md`.
⭐ Ελέγχθηκε και σε ΑΛΗΘΙΝΟ browser: και οι πέντε οθόνες, με ΚΛΙΚ στα κουμπιά
της πλοήγησης — όχι με προ-ενεργοποιημένη οθόνη, γιατί harness που δεν
πλοηγεί δεν ελέγχει πλοήγηση.)

---

**2026-09-03 — `als-v534` — ΙΣΤΟΡΙΑ: Η ΠΤΩΧΕΥΣΗ ΚΑΙ Ο ΔΟΕ (β10), ΚΑΙ ΑΝΟΙΓΕΙ ΤΟ ΚΕΦΑΛΑΙΟ Γ (γ1)**
(`tests/istoria-data.test.js` 23.454 → **30.270/0**· `tests/istoria-plag.test.js`
1.460 → **1.671/0**· `tests/istoria-ear.test.js` 1.068 → **1.221/0**· `smoke-test.sh`
**SMOKE_OK**, 55 html · 533 links· **55 σουίτες πράσινες**. ⚠️ `ladders.test.js`
**98/4** — ΠΡΟΫΠΑΡΧΟΝ, ξαναεπαληθεύτηκε με `git stash` (ίδιοι αριθμοί πριν/μετά).
Άλλαξαν **πέντε αρχεία** και η `istoria.html` ΔΕΝ αγγίχτηκε: `istoria-data.js`,
`tests/istoria-data.test.js`, `docs/ISTORIA_SPEC.md`, `MAP.md`, `sw.js`.)

Δύο κάρτες μαζί, και είναι διαφορετικού είδους. Η **`b10` «10. Η πτώχευση του
1893 και ο Διεθνής Οικονομικός Έλεγχος»** μπαίνει ΟΛΟΚΛΗΡΗ (4 παράγραφοι,
**27 σημεία / 79 στοιχεία**, 18/21/17/23) και **κλείνει το κεφάλαιο Β** ως το
«10.». Η **`c1` «1. Το αγροτικό ζήτημα»** μπαίνει με τις **ΤΡΕΙΣ ΠΡΩΤΕΣ
παραγράφους** μόνο (18 σημεία / **50 στοιχεία**, 21/18/11), όπως τις ζήτησε.
Corpus: **18 ενότητες · 286 σημεία · 952 στοιχεία · 30.270 βεβαιώσεις.**

- **Η ΓΕΙΩΣΗ, ΚΑΙ ΓΙΑ ΠΡΩΤΗ ΦΟΡΑ ΑΠΟ ΔΥΟ ΣΕΛΙΔΕΣ.** `curl` στο `ebooks.edu.gr`:
  το κεφάλαιο Β ζει στο `index1_3.html` (sha256 `042ec4d2…a0f24b`) και το
  κεφάλαιο Γ στο **`index1_4.html`** (sha256 `7e81aed2…39175`), και τα δύο
  καρφωμένα στο test. ⭐⭐ **Το sha256 του `index1_3.html` ΒΓΗΚΕ ΤΑΥΤΟΣΗΜΟ με το
  κατέβασμα της als-v520** — δηλαδή αποδείχθηκε ότι η σελίδα του βιβλίου δεν
  άλλαξε ανάμεσα στις δύο προσθήκες· αν είχε αλλάξει, οι ήδη ελεγμένες
  παράγραφοι θα έλεγαν ψέματα. **Δύο ανεξάρτητοι εξαγωγείς** → ταυτόσημες
  **7/7** χαρακτήρα προς χαρακτήρα.
- ⚠️⚠️ **ΤΟ MARKUP ΤΟΥ ΚΕΦΑΛΑΙΟΥ Γ ΒΑΖΕΙ ΔΥΟ ΠΑΡΑΓΡΑΦΟΥΣ ΤΟΥ ΒΙΒΛΙΟΥ ΜΕΣΑ ΣΕ
  ΕΝΑ `<p>`.** Η 1η και η 2η παράγραφος του αγροτικού ζητήματος χωρίζονται μόνο
  με κενές γραμμές. Ένας εξαγωγέας «ανά `<p>`» — ακριβώς αυτός που δούλευε σε
  δεκατέσσερις προηγούμενες κάρτες — τις ΕΝΩΝΕΙ σιωπηλά σε μία τερατώδη
  παράγραφο. **Ο κανόνας: ένας εξαγωγέας που δούλεψε δεν είναι εξαγωγέας που
  δουλεύει· η επαλήθευση είναι ΔΥΟ εξαγωγείς, όχι ένας δοκιμασμένος.**
- ⭐⭐⭐ **ΤΟ ΟΡΙΟ ΤΗΣ β9→β10 ΕΓΙΝΕ ΔΙΠΛΗΣ ΚΑΤΕΥΘΥΝΣΗΣ ΜΕ ΤΗΝ ΙΔΙΑ ΜΕΤΑΒΛΗΤΗ.**
  Η λίστα που η «9. Τα εθνικά δάνεια» ΑΠΑΓΟΡΕΥΕΙ στον εαυτό της («πτώχευσ ·
  1893 · Διεθν · Ελέγχου · 1897 · Βενιζέλ · Οθωμανικ · αποζημίωσ · μονοπωλί»)
  ΑΠΑΙΤΕΙΤΑΙ τώρα από τη «10.» — και εξάγεται σε μεταβλητή ώστε να είναι
  ΚΥΡΙΟΛΕΚΤΙΚΑ η ίδια λίστα, όχι αντίγραφο που αποκλίνει σιωπηλά. ⚠️ Η
  μετατροπή αποκάλυψε **λάθος στην παλιά λίστα**: απαγόρευε «Έλεγχο» με
  κεφαλαίο Έ, λέξη που **δεν υπάρχει σε καμία από τις δύο κάρτες** (το βιβλίο
  γράφει «Ελέγχου» μέσα στο κείμενο και «Έλεγχος» μόνο στον τίτλο, που δεν
  απαγγέλλεται). Μια απαγόρευση που δεν μπορεί να παραβιαστεί δεν φυλάει τίποτα.
- ⭐⭐⭐ **Η ΧΕΙΡΟΤΕΡΗ ΠΑΓΙΔΑ ΤΗΣ β10 ΕΙΝΑΙ ΠΛΗΡΗΣ ΤΑΥΤΙΣΗ, ΔΥΟ ΦΟΡΕΣ:**
  «εξυπηρετ**ήσει**» ≡ «εξυπηρ**έ**τηση» (και τα δύο `εξιπιρετισι`) και
  «αποζημι**ώσεις**» ≡ «αποζημ**ί**ωσης» (και τα δύο `αποζιμιοσισ`). Στα
  ΓΡΑΜΜΑΤΑ διαφορετικές λέξεις· μετά το η→ι και το ω→ο **η ίδια συμβολοσειρά**,
  δύο παραγράφους μακριά, για ΑΛΛΟ γεγονός — η αδυναμία του 1893 απέναντι στους
  δανειστές vs ο στόχος του ΔΟΕ απέναντι στην Οθωμανική αυτοκρατορία. Σώζονται
  ΜΟΝΟ από γειτνίαση. Μαζί τους: «αποπληρωμή ⊂ αποπληρωμής» σε **σχεδόν ίδια
  φράση** σε διαδοχικές παραγράφους, «Δυνάμεων» ×3, «εγγύηση» ×2 (η μία
  «χορηγήθηκε», η άλλη «αύξησε»), και οκτώ υποσύνολα κατάληξης.
- ⭐⭐⭐ **ΚΑΙ Η ΧΕΙΡΟΤΕΡΗ ΤΗΣ γ1 ΕΙΝΑΙ ΠΛΗΡΗΣ ΤΑΥΤΙΣΗ ΜΕ ΑΝΤΙΘΕΤΟ ΝΟΗΜΑ:**
  «που **γνώρισε** ο σύγχρονος κόσμος» ≡ «**δεν γνώρισε** τις εντάσεις». Ίδια
  λέξη, ίδια μορφή, και η δεύτερη είναι ΑΡΝΗΣΗ — ένα κλειδί στο σκέτο «γνώρισε»
  θα έλεγε «το είπες» για το ακριβώς αντίθετο από αυτό που είπε. Και μία που
  είναι ΟΛΟΚΛΗΡΗ ΦΡΑΣΗ: «άσκησαν … πιέσεις» ×2, με άλλο υποκείμενο (οι ραγδαίες
  εξελίξεις / οι τσιφλικάδες), συν «χώρο» ×2 **και οι δύο μετά από «στον»**.
- ⚠️⚠️ **ΣΤΑΘ. 42, ΠΛΗΡΩΜΕΝΗ ΞΑΝΑ ΜΕΣΑ ΣΤΟ ΙΔΙΟ ΜΟΥ ΤΟ TEST: το `\b` ΕΙΝΑΙ
  ΤΥΦΛΟ ΣΤΑ ΕΛΛΗΝΙΚΑ.** Το `/χώρο\b/g` επέστρεψε **0** πάνω σε κείμενο που έχει
  τη λέξη δύο φορές — δηλαδή η βεβαίωση κόντεψε να «αποδείξει» ότι η λέξη δεν
  υπάρχει καθόλου. Μετριέται τώρα με άρνηση κατάληξης, `/χώρο(?![ςυ])/g`.
- ⭐ **Ο ΤΙΤΛΟΣ ΤΗΣ β10 ΚΟΠΗΚΕ, ΚΑΙ Η ΚΟΠΗ ΕΙΝΑΙ ΜΕΤΡΗΜΕΝΗ ΜΕΣΑ ΣΤΟ TEST.** Ο
  πλήρης τίτλος (56 χαρ.) θα έδινε γραμμή ύλης 81 χαρακτήρων, δηλαδή θα έτρωγε
  τα νούμερα. Το `section` έγινε **«10. Η πτώχευση και ο ΔΟΕ»** (49 με τα
  νούμερα) και **κρατάει ΚΑΙ ΤΑ ΔΥΟ ΜΙΣΑ** — το test το απαιτεί ρητά. Η
  συντομογραφία δεν είναι δική μου: τη δίνει το βιβλίο σε παρένθεση. ⚠️ Και
  αποδείχθηκε ότι το `pho('ΔΟΕ')` = `δοε` **δεν ζει μέσα σε καμία άλλη λέξη**
  της ενότητας, αλλιώς ένα τριγράμματο κλειδί θα άναβε από άσχετη πρόταση.
- ⭐ **Η γ1 ξαναρχίζει την αρίθμηση από το «1.»** — τρίτο «1.» του corpus, μετά
  τα δημογραφικά δεδομένα (Α) και το εμπόριο (Β). Ό,τι τα ξεχωρίζει στην οθόνη
  είναι ΤΟ ΚΕΦΑΛΑΙΟ, και το test απαιτεί κάθε «1.» να ανήκει σε άλλο.
- ⭐⭐ **Η γ1 ΕΙΝΑΙ ΜΕΡΙΚΗ ΕΞ ΑΡΧΗΣ, ΚΑΙ ΤΟ ΜΠΡΟΣΤΙΝΟ ΟΡΙΟ ΕΙΝΑΙ ΤΟ ΣΗΜΑΝΤΙΚΟΤΕΡΟ
  ΤΗΣ ΚΟΜΜΑΤΙ.** 18 λέξεις της συνέχειας (1907 · απαλλοτρι · ακτήμον · Κιλελέρ ·
  1910 · Βαλκανικ · 1913 · μουσουλμάν · διχασμ · 1917 · Βενιζέλ · Θεσσαλονίκ ·
  αναδιανομ · Μακεδονί · μικροϊδιοκτησ · Τράπεζ · συνεταιρισμ · προσφυγικ)
  ελέγχονται ότι ΛΕΙΠΟΥΝ, ζουν σε μεταβλητή `C1_FORWARD`, και **όταν μπει η
  συνέχεια η ίδια λίστα γίνεται ΑΠΑΙΤΗΣΗ εκεί**.
- ⭐⭐ **Δοκιμασμένο ότι δαγκώνει με 10 μεταλλάξεις**, καθαρό **0**: χαλαρό
  κλειδί στην πλήρη ταύτιση → 2 fail · λάθος ψηφίο (1898→1899) → 4 · διαρροή
  «Κιλελέρ» στο αγροτικό → 5 · γυμνό «γνώρισε» → 3 · κομμένος τίτλος χωρίς ΔΟΕ →
  2 · ψεύτικη άγκυρα → 1 · γυμνό «Θεσσαλία» → 2 · σβησμένη λέξη «υπέρογκες» → 3 ·
  χαλαρό «εξυπηρέτηση» → 2 · χαλαρό «εγγύηση των Δυνάμεων» → 2.
- ⛔ **ΕΠΟΜΕΝΟ = οι ΔΥΟ ΤΕΛΕΥΤΑΙΕΣ παράγραφοι του αγροτικού ζητήματος** (νόμοι
  του 1907, Κιλελέρ 1910, Βενιζέλος 1917, αναδιανομή 85%/68%/40%, Αγροτική
  Τράπεζα) ως `c1b`, με το όριο αντίστροφο. Και μετά, αν ζητηθεί, η «11. Το
  εξωελλαδικό ελληνικό κεφάλαιο» του Β. Καμία τους δεν μπαίνει πριν τη ζητήσει.

**2026-08-30 — `als-v520` — ΙΣΤΟΡΙΑ: Η 8η ΚΛΕΙΝΕΙ (β8b) ΚΑΙ ΤΑ ΕΘΝΙΚΑ ΔΑΝΕΙΑ (β9)**
(`tests/istoria-data.test.js` 19.638 → **23.454/0**· `tests/istoria-plag.test.js`
1.326 → **1.460/0**· `tests/istoria-ear.test.js` 969 → **1.068/0**· `smoke-test.sh`
**SMOKE_OK**, 53 html · 530 links· **54 σουίτες πράσινες**. ⚠️ `ladders.test.js`
**98/4** — ΠΡΟΫΠΑΡΧΟΝ, ξαναεπαληθεύτηκε με `git stash` (ίδιοι αριθμοί πριν/μετά).
Άλλαξαν **έξι αρχεία** και η `istoria.html` ΔΕΝ αγγίχτηκε: `istoria-data.js`,
`tests/istoria-data.test.js`, `tests/istoria-ear.test.js`, `docs/ISTORIA_SPEC.md`,
`sw.js` — και το `greek-ear.js` **δοκιμάστηκε και ΕΠΕΣΤΡΑΦΗ ΑΘΙΚΤΟ**, δες πιο κάτω.)

Δύο υποενότητες μαζί: η **`b8b`** κλείνει τις δύο τελευταίες παραγράφους των
σιδηροδρόμων (12 σημεία, **31 στοιχεία**, 14/17) και η **`b9` «Τα εθνικά δάνεια»**
μπαίνει ΟΛΟΚΛΗΡΗ (16 σημεία, **44 στοιχεία**, 16/13/15) μαζί με τον **Πίνακα 6**.
Corpus: **16 ενότητες · 241 σημεία · 823 στοιχεία · 23.454 βεβαιώσεις**.
⭐ **Η υποενότητα «8. Το δίκτυο των σιδηροδρόμων» είναι πλέον ΟΛΟΚΛΗΡΗ** — και το
test το αποδεικνύει όχι με ισχυρισμό αλλά **δείχνοντας ότι `b8` + `b8b` είναι
ΣΥΝΕΧΟΜΕΝΟ κείμενο μέσα στο BOOK**, άρα καμία παράγραφος δεν χάθηκε στην τομή.

- **Η ΓΕΙΩΣΗ.** `curl` στο `ebooks.edu.gr` (`index1_3.html`, sha256
  `042ec4d2…a0f24b` καρφωμένο στο test), **δύο ανεξάρτητοι εξαγωγείς** → ταυτόσημες
  **5/5** χαρακτήρα προς χαρακτήρα, για τις 2 + 3 παραγράφους μαζί.
- ⭐⭐⭐ **ΤΟ ΟΡΙΟ ΜΙΑΣ ΚΟΜΜΕΝΗΣ ΥΠΟΕΝΟΤΗΤΑΣ ΕΛΕΓΧΕΤΑΙ ΚΑΙ ΠΡΟΣ ΤΙΣ ΔΥΟ
  ΚΑΤΕΥΘΥΝΣΕΙΣ, ΜΕ ΤΗΝ ΙΔΙΑ ΛΙΣΤΑ ΛΕΞΕΩΝ.** Η `b8` ΑΠΑΓΟΡΕΥΕΙ στον εαυτό της
  «επωμίστηκε · 30% · υστέρηση · αναμφίβολο · εκβιομηχάνιση…» και η `b8b`
  **ΑΠΑΙΤΕΙ** ακριβώς τις ίδιες. Χωρίς το δεύτερο μισό, μια λέξη μπορεί να ζει σε
  **ΚΑΜΙΑ** από τις δύο κάρτες και να μη σκάσει τίποτα. Ίδιο σχήμα με b6→b7→b8.
- ⭐⭐⭐ **Η ΧΕΙΡΟΤΕΡΗ ΠΑΓΙΔΑ ΤΗΣ b8b ΕΙΝΑΙ ΠΛΗΡΗΣ ΤΑΥΤΙΣΗ, ΟΧΙ ΥΠΟΣΥΝΟΛΟ:
  «Πρόσφερε» ≡ «πρόσφερε»** — δύο προτάσεις απόσταση, ΙΔΙΑ παράγραφος, και το
  `pho` ισοπεδώνει το κεφαλαίο. Καμία λέξη-κλειδί δεν τις ξεχωρίζει· σώζονται
  ΜΟΝΟ από γειτνίαση. ⭐⭐ Και μία που **τη γεννάει το ίδιο το αυτί**:
  **«ανάπτυξη» ⊂ «αναπτυξιακές»** — στα ΓΡΑΜΜΑΤΑ δεν είναι υποσύνολο, μετά το
  η→ι γίνεται `αναπτιξι` ⊂ `αναπτιξιακεσ`, σε **διαδοχικές** προτάσεις που
  αρχίζουν και οι δύο με «Δεν κατόρθωσε». **Ο κανόνας: η σάρωση συγκρούσεων
  τρέχει ΠΑΝΩ ΣΤΟ `pho`, ποτέ πάνω στα γράμματα.**
- ⭐⭐ **ΚΑΙ ΣΤΗ b9 ΤΟ ΜΑΘΗΜΑ ΤΗΣ b8 ΞΑΝΑΧΤΥΠΑΕΙ ΜΕ ΑΛΛΟ ΠΡΟΣΩΠΟ: ΤΟ «1880»
  ΖΕΙ ΜΕΣΑ ΣΤΟ «1877-1880».** Εκεί ήταν γυμνό ψηφίο, εδώ ολόκληρη χρονολογία —
  το `pho` πετάει την παύλα και δίνει `18771880`. Ίδια θεραπεία: γειτνίαση
  («δεκαετία του 1880» ως ΜΙΑ συμβολοσειρά) ΣΥΝ ζεύγος `['1877','1880']`.
  Οι υπόλοιπες ονομαστικά: **δανεισμό ⊂ δανεισμός** (και «δανεισμός» ΔΥΟ φορές) ·
  **αποπληρωμή ΔΥΟ φορές με ΑΝΤΙΘΕΤΟ ρόλο** (την αρνούνται το 1832, δανείζονται
  για να την κάνουν το 1880) · **επενδύσεις** δημόσιες/παραγωγικές · **χώρα ⊂
  χώρας** · προϋπολογισμού/προϋπολογισμών · χρηματαγορά/χρηματαγορές.
  **21 μεταλλάξεις, ΟΛΕΣ δάγκωσαν, καθαρό 0.**
- ⭐⭐⭐ **ΚΑΙ ΠΑΛΙ ΤΟ ΜΑΘΗΜΑ ΕΙΝΑΙ ΓΙΑ ΤΟ ΕΡΓΑΛΕΙΟ (σταθ. 28, ΤΕΤΑΡΤΗ φορά):
  ΜΙΑ ΕΛΛΗΝΙΚΗ ΛΕΞΗ ΔΕΝ ΑΡΧΙΖΕΙ ΠΟΤΕ ΜΕ ΔΙΑΛΥΤΙΚΑ.** Το `splitWords` έκοβε το
  «ευρωπαϊκές» σε «ευρωπα **ϊ**κές» — δεύτερο κομμάτι που ξεκινάει με ϊ, που
  καμία αναγνώριση φωνής δεν γράφει· και επειδή τα διαλυτικά είναι ΑΚΡΙΒΩΣ το
  σημάδι ότι τα δύο φωνήεντα ΔΕΝ διαβάζονται μαζί, το κόψιμο άλλαζε τον ήχο.
  **22 λέξεις του corpus έχουν διαλυτικά.** Η b9 έβγαινε **95%** (το χειρότερο
  του corpus, ΠΑΝΩ στο κατώφλι) και θα με έσπρωχνε να χαλαρώσω ΣΩΣΤΕΣ
  λέξεις-κλειδιά· με τη διόρθωση **98% και «και τα τρία μαζί» 100%, χωρίς να
  πέσει καμία άλλη ενότητα**. ⛔ Το κόψιμο ΜΕΤΑΚΙΝΗΘΗΚΕ, δεν ακυρώθηκε — ένας
  φρουρός που σταματάει να δαγκώνει δεν είναι διόρθωση.
- ⭐⭐ **ΤΟ RENDER ΞΑΝΑΕΠΙΑΣΕ 24 ΔΙΚΑ ΜΟΥ ΛΑΘΗ ΠΟΥ ΚΑΜΙΑ ΑΠΟ ΤΙΣ 23.454
  ΒΕΒΑΙΩΣΕΙΣ ΔΕΝ ΔΙΑΒΑΖΕΙ**, και δύο από αυτά έγιναν ΜΟΝΙΜΟΣ ΦΡΟΥΡΟΣ (§8λ):
  **(1) ΟΝΟΜΑΤΑ ΚΩΔΙΚΑ ΜΕΣΑ ΣΕ ΚΕΙΜΕΝΟ ΠΟΥ ΔΙΑΒΑΖΕΙ** — τυπωνόταν «ό,τι έλειπε
  στη 2η παράγραφο της **b8**», και ο Αλεξ δεν έχει δει ποτέ αυτό το κλειδί
  (10 εμφανίσεις· βρέθηκαν και **2 προϋπάρχουσες στη b3b**, διορθώθηκαν στο ίδιο
  commit — φρουρός με grandfather list δεν φυλάει τίποτα). **(2) ΧΑΜΕΝΗ
  ΑΠΟΣΤΡΟΦΟΣ**, «γι αυτό» αντί «γι’ αυτό», 7 φορές — ΣΥΣΤΗΜΑΤΙΚΟ λάθος αυτού
  του αρχείου, γιατί τα strings ζουν σε μονά εισαγωγικά και η απόστροφος τα
  σπάει. Και τρία ακόμη που μόνο το μάτι πιάνει: **αριθμητικό** («τριάντα
  χρόνια» ενώ 1832→1861 είναι **29**), **λάθος μέτρηση δική μου** («η λέξη
  αποδοτικότητα δύο φορές» ενώ είναι η ΡΙΖΑ δύο φορές, «αποδοτικό» +
  «αποδοτικότητα»), και **ΑΙΤΙΟΤΗΤΑ ΠΟΥ ΕΦΕΥΡΑ** (ότι η διακοπή των επενδύσεων
  καθυστέρησε το δίκτυο ως το 1909 — το βιβλίο δίνει ΑΛΛΗ αιτία και δεν συνδέει
  τα δύο). ⭐ Μπήκε και ⛔ ρητό «ΜΗΝ ΓΡΑΨΕΙΣ 70%»: δικός μου υπολογισμός ήταν.
- ⚠️⚠️ **ΜΕΤΡΗΘΗΚΕ ΚΑΙ ΔΕΝ ΜΠΗΚΕ, ΔΗΛΩΜΕΝΟ: ΤΟ `pho` ΚΑΝΕΙ ΤΟ COLLAPSE ΤΩΝ
  ΔΙΠΛΩΝ ΣΥΜΦΩΝΩΝ *ΠΡΙΝ* ΦΥΓΟΥΝ ΤΑ ΚΕΝΑ.** Άρα «ελλειμ μάτων» (ΠΡΑΓΜΑΤΙΚΗ
  πιθανή έξοδος αναγνώρισης) δίνει `ελιμματον` ενώ η ακέραιη λέξη δίνει
  `ελιματον` — και αυτό είναι ο λόγος που a1a/b1b/b2b/b3b/b4/b5 κάθονται στο
  96–99% εδώ και μήνες. **Η αντιστροφή δοκιμάστηκε: ΚΑΙ ΟΙ 7 ΣΟΥΙΤΕΣ ΠΡΑΣΙΝΕΣ
  (Ιστορία, Αρχαία ×4, Τονισμός, Λατινικά) και ανέβηκαν b3 98→100 και b9
  98→100, χωρίς να πέσει καμία.** ⛔ **ΔΕΝ ΜΠΗΚΕ ΣΕ ΑΥΤΟ ΤΟ COMMIT ΕΠΙΤΗΔΕΣ**:
  το `greek-ear.js` το μοιράζονται ΤΕΣΣΕΡΑ μαθήματα, και η αλλαγή κάνει το
  collapse να χτυπάει **ΠΑΝΩ ΑΠΟ ΟΡΙΑ ΛΕΞΕΩΝ** — φωνητικά σωστό, αλλά χαλαρώνει
  το ταίριασμα παντού με τρόπο που **καμία υπάρχουσα βεβαίωση δεν εξετάζει**.
  Θέλει δικό του commit με νέες βεβαιώσεις γι’ αυτή ακριβώς τη συμπεριφορά.
- ⚠️ §1.2 ξαναμετρημένη: `units 16 · TOTAL points 241 · unique-paragraph 241 ·
  multi 0 · NONE 0`. Η **`b9` είναι η ΠΙΟ ΠΥΚΝΗ του corpus (35,2 χαρ./στοιχείο)**
  και δηλώνεται στο §1.3: η 3η παράγραφός της είναι ΚΑΤΑΛΟΓΟΣ προορισμών με
  χρονολογίες και ποσό, όχι αφήγηση. Η **b8b βγαίνει 40,3** — στη ζώνη της b8.
- ⭐ **Ο Πίνακας 6 είναι ΥΛΙΚΟ ΑΝΑΓΝΩΣΗΣ, και αποδεικνύεται**: βεβαίωση δείχνει
  ότι **ΚΑΝΕΝΑ ποσό του δεν υπάρχει μέσα στο κείμενο προς απαγγελία**, άρα καμία
  άγκυρα δεν μπορεί να ζήσει εκεί (§5.6). Η γραμμή «Συνολικό χρέος 1892» έχει
  `colspan=2` στο ebook· ισιώθηκε σε ορθογώνιο με **ΚΕΝΟ** κελί ΑΙΤΙΑΣ — και το
  test απαγορεύει να εφευρεθεί περιεχόμενο εκεί.
- 🔴 **ΕΠΟΜΕΝΟ = «10. Η πτώχευση του 1893 και ο Διεθνής Οικονομικός Έλεγχος»**,
  και η `b9` κλειδώνει ΟΝΟΜΑΣΤΙΚΑ ότι δεν διέρρευσε λέξη της (πτώχευση, 1893,
  1897, Διεθνής Έλεγχος, Βενιζέλος, μονοπώλια). Είναι η πιο εύκολη παγίδα της
  ενότητας, γιατί η b9 τελειώνει ακριβώς εκεί που αρχίζει εκείνη.
- 🔴 **Αδοκίμαστο στη συσκευή του** — θέλει πλήρες reopen του PWA.

**2026-08-27 — `als-v517` — ΙΣΤΟΡΙΑ: ΤΟ ΔΙΚΤΥΟ ΤΩΝ ΣΙΔΗΡΟΔΡΟΜΩΝ (β8), ΟΙ ΤΕΣΣΕΡΙΣ ΠΡΩΤΕΣ**
(`tests/istoria-data.test.js` 16.727 → **19.638/0**· `tests/istoria-plag.test.js`
**1.326/0**· `tests/istoria-ear.test.js` **969/0**· `smoke-test.sh` **SMOKE_OK**,
53 html · 530 links. ⚠️ `ladders.test.js` **98/4** — ΠΡΟΫΠΑΡΧΟΝ, ξαναεπαληθεύτηκε
με `git stash`. Άλλαξαν **πέντε αρχεία** και η `istoria.html` ΔΕΝ αγγίχτηκε:
`istoria-data.js`, τα δύο test της, το `docs/ISTORIA_SPEC.md` και το `sw.js`.)

Η δέκατη τέταρτη υποενότητα. ⚠️ **ΜΕΡΙΚΗ ΑΝΑΘΕΣΗ**: η «8. Το δίκτυο των
σιδηροδρόμων» έχει **ΕΞΙ** παραγράφους, ανατέθηκαν οι **ΤΕΣΣΕΡΙΣ ΠΡΩΤΕΣ** — ως
το «…χωρίς φιλοδοξίες να αποτελέσει τμήμα του διεθνούς δικτύου». **23 σημεία,
93 στοιχεία** (18 / 28 / 29 / 18), 4 όροι του βιβλίου, 33 λέξεις λεξιλογίου,
8 σημειώσεις πλαισίου, 6 χρονολογίες. Corpus: **14 ενότητες · 213 σημεία ·
19.638 βεβαιώσεις**. Ο τίτλος λέει «(οι πρώτες παράγραφοι)» και το `section`
κόπηκε σε «8. Οι σιδηρόδρομοι» (ο πλήρης έβγαζε γραμμή ύλης **54** σε μπάτζετ
50 — η b4 ξανά).

- **Η ΓΕΙΩΣΗ.** `curl` στο `ebooks.edu.gr` (`index1_3.html`), **δύο ανεξάρτητοι
  εξαγωγείς** → ταυτόσημες **6/6** χαρακτήρα προς χαρακτήρα σε ΟΛΗ την
  υποενότητα· και οι τέσσερις παράγραφοι επαληθεύτηκαν **απευθείας πάνω στο
  κατεβασμένο HTML**, παρακάμπτοντας το `BOOK` του test.
- ⭐⭐⭐ **Η ΠΑΓΙΔΑ ΤΗΣ ΕΝΟΤΗΤΑΣ ΕΙΝΑΙ ΑΡΙΘΜΗΤΙΚΗ ΚΑΙ Η ΧΕΙΡΟΤΕΡΗ ΩΣ ΤΩΡΑ: ΤΟ
  ΓΥΜΝΟ «9».** Το `pho` πετάει κενά και τελείες, οπότε το ψηφίο 9 των «9
  χιλιομέτρων» ζει **ΟΚΤΩ φορές** μέσα στην ίδια ανάθεση (19ου · 19ο · 1892 ·
  900 · 1890 · και **δύο** φορές στο 1909). Το πιο αξιομνημόνευτο νούμερο της
  ενότητας ήταν και το πιο εύθραυστο. Θεραπεία: **γειτνίαση** ΣΥΝ ολόγραφες
  εναλλακτικές, γιατί όταν το αυτί γράφει «εννέα χιλιόμετρα» το `expand`
  κολλάει το ψηφίο στο **ΤΕΛΟΣ**, όχι στη θέση της λέξης.
- ⭐⭐ Οι άλλες οκτώ, κλειδωμένες ΟΝΟΜΑΣΤΙΚΑ: **1880 ΔΥΟ φορές με ΑΝΤΙΘΕΤΟ
  ρόλο** (όριο του τίποτα στην 3η παρ., αφετηρία στην 4η) · **υλοποίησή ≡
  υλοποίηση** (ΠΛΗΡΗΣ ταύτιση, ΙΔΙΑ παράγραφος — η `b4` ξανά) · **μέτρων ⊂
  χιλιομέτρων** (το πλάτος μέσα στο μήκος) · **Ασία ⊂ εργοστάσια** (ήπειρος
  μέσα σε κτίριο, δύο παράγραφοι μακριά) · **αποδοτικό ⊂ αποδοτικότητα** ·
  **θέση ⊂ υπόθεση ⊂ προϋπόθεση ⊂ προϋποθέσεις** (ΤΕΤΡΑΠΛΗ φωλιά) ·
  **πρόβλημα ⊂ προβλήματα** · **γραμμή/κατασκευαστεί/σχέδια** της 3ης μέσα στη
  4η. **20 μεταλλάξεις, όλες δάγκωσαν, καθαρό 0.**
- ⚠️⚠️ **ΤΟ ΒΙΒΛΙΟ ΑΥΤΟΑΝΑΙΡΕΙΤΑΙ ΔΥΟ ΠΑΡΑΓΡΑΦΟΥΣ ΜΑΚΡΙΑ, ΚΑΙ ΜΕΝΟΥΝ ΚΑΙ ΤΑ
  ΔΥΟ ΟΠΩΣ ΕΙΝΑΙ:** «για την **είσοδο τους**» (2η παρ.) ΧΩΡΙΣ δεύτερο τόνο, ενώ
  «η **υλοποίησή** τους» (3η παρ.) ΜΕ αυτόν. Επαληθευμένο **σε επίπεδο byte**
  στο κατεβασμένο HTML, κλειδωμένο και προς τις δύο κατευθύνσεις. Ίδιος κανόνας
  με το «Η δραστηριότητα του» της b7. ⭐ Επίσης: **ΚΑΜΙΑ απόστροφος και κανένα
  « » σε όλη την ανάθεση** — μετρημένο, γι’ αυτό μονά εισαγωγικά όπως η b6.
- ⭐⭐⭐ **ΚΑΙ ΤΟ ΜΑΘΗΜΑ ΕΙΝΑΙ ΞΑΝΑ ΓΙΑ ΤΟ ΕΡΓΑΛΕΙΟ ΜΟΥ: Η ΠΡΟΣΟΜΟΙΩΣΗ ΤΟΥ
  ΑΥΤΙΟΥ ΠΑΡΗΓΕ ΑΔΥΝΑΤΗ ΕΙΣΟΔΟ ΕΠΙ ΜΗΝΕΣ** (σταθερή αρχή 28, τρίτη φορά). Ο
  φύλακας των διψήφων ήταν **ΠΕΖΟΣ ΚΑΙ ΑΤΟΝΟΣ**: το «Κεντρική **Ευ**ρώπη» γινόταν
  «Ειρώπη» (κεφαλαίο Ε) και το «βοηθ**ού**σε» γινόταν «βοιθόισε» (τονισμένο ού) —
  λέξεις που καμία αναγνώριση φωνής δεν γράφει. Και η λίστα τακτικών δεν είχε το
  **σκέτο «ο»**, οπότε το «κατά το 19**ο** αιώνα» έβγαινε «δέκατο ένατ**οο** αιώνα»
  — ακριβώς η αδύνατη φράση που το ίδιο το αρχείο προειδοποιούσε να μη γίνει.
  ⭐ **Η ΑΠΟΔΕΙΞΗ ΟΤΙ ΕΦΤΑΙΓΕ Ο ΜΕΤΡΗΤΗΣ: με ΤΑ ΙΔΙΑ δεδομένα ανέβηκαν ΤΕΣΣΕΡΙΣ
  ενότητες και δεν κατέβηκε καμία** — `a2` 97→100, `b1b` 98→100, `b2b` 97→100,
  `b8` 98→100 (ομόηχη)· `b8` 99→100 (αριθμοί). Δηλαδή η **a2, η b1b και η b2b
  μετριούνταν χαμηλότερα απ’ ό,τι ήταν από τότε που γράφτηκαν.** Μπήκε και ο
  ΟΡΙΣΜΟΣ ως δίχτυ (ομόηχο = ίδιο `pho`, αλλιώς η λέξη μένει άπειραχτη) και
  **τρία κατώφλια ανέβηκαν στο 100%** — ένας φρουρός που έβλεπε 98 και ζητούσε
  85 δεν φύλαγε τίποτα.
- ⭐⭐ **ΤΟ RENDER ΞΑΝΑΕΠΙΑΣΕ 12 ΔΙΚΑ ΜΟΥ ΛΑΘΗ** που καμία από τις 19.638
  βεβαιώσεις δεν διαβάζει: μια **ΛΕΞΗ ΠΟΥ ΕΦΕΥΡΑ** («μετ**εωχή**» αντί
  «μετατρόχιο»), έναν **ΑΝΑΧΡΟΝΙΣΜΟ** («πριν φέρει ένα **ευρώ**» στον 19ο
  αιώνα), μια **ΑΝΤΙΦΑΣΗ ΜΕ ΤΟΝ ΔΙΚΟ ΜΟΥ ΓΑΝΤΖΟ** (έγραφα ότι στην 1η
  παράγραφο ο σιδηρόδρομος είναι «ΑΠΟΤΕΛΕΣΜΑ» ενώ ο γάντζος 3 λέει το
  αντίθετο), ένα «**ΤΕΣΣΕΡΙΣ ΑΡΙΘΜΟΙ**» που απαριθμούσε **έξι**, ένα «σε τρεις
  πόλεις απόσταση» που δεν σημαίνει τίποτα, και ορολογία κώδικα (`` `text` ``)
  μέσα σε κείμενο που ΔΙΑΒΑΖΕΙ. ⚠️ Το `openLesson` ζει μέσα σε IIFE — το
  render ανοίγει την ενότητα με **κλικ στην πλαϊνή στήλη**, όχι με κλήση.
  Χρειάστηκε **CDP** (`--remote-debugging-port` + `Page.captureScreenshot`).
- ⚠️ §1.2 ξαναμετρημένη: `units 14 · TOTAL points 213 · unique-paragraph 213 ·
  multi 0 · NONE 0`. Η `b8` είναι η **πιο ακριβή του corpus (93 στοιχεία)** και
  δεν είναι από αφειδία: **3.864 χαρακτήρες**, περισσότεροι από τις ΠΕΝΤΕ
  παραγράφους της `b5` — **41,5 χαρ./στοιχείο**, ίδια πυκνότητα με b5/b6/b7.
  Η απάντηση στο μέγεθος είναι ο **πλαγιότιτλος**: μια βραδιά είναι 18, όχι 93.
- 🔴 **ΕΠΟΜΕΝΟ = οι ΔΥΟ ΤΕΛΕΥΤΑΙΕΣ παράγραφοι της 8ης** (ποιος πλήρωσε · ο
  απολογισμός), δηλωμένες ρητά στο `context` της b8 και κλειδωμένες ότι ΔΕΝ
  διέρρευσαν. 🔴 **Αδοκίμαστο στη συσκευή του** — θέλει πλήρες reopen του PWA.

**2026-08-27 — `als-v516` — ΛΑΤΙΝΙΚΑ: LECTIO XVIII, ΤΟ ΤΡΙΤΟ ΚΕΙΜΕΝΟ**
(`tests/latinika-lectio.test.js` 275 → **464/0**· `smoke-test.sh` **SMOKE_OK**,
53 html · 530 links· `launcher` 77/0· `study-sync-persist` 35/0· `library` 229/0.
⚠️ `ladders.test.js` **98/4** — ΠΡΟΫΠΑΡΧΟΝ, ξαναεπαληθεύτηκε με `git stash`.)

Δικά του λόγια: *«δες ακριβώς τι μου έχεις φτιάξει για τα άλλα δύο κείμενα για
να μάθω τη μετάφρασή τους, θέλω να κάνεις ΑΚΡΙΒΩΣ το ίδιο»* + φωτογραφία της
σελ. 27 του φυλλαδίου. «Ακριβώς το ίδιο» = **η ΜΗΧΑΝΗ**, όχι η παλέτα: τα δύο
υπάρχοντα πακέτα δεν μοιάζουν καν μεταξύ τους (το XVI είναι χαρτί, το XVII
σκοτεινό) γιατί ήρθαν από διαφορετικές πηγές. Πάρθηκε η μηχανή του XVII (ομάδες
`g` λατινικά↔ελληνικά, `d`+`x` ανά λέξη, «✓ το ξέρω» ανά ενότητα) **συν** το
μπλοκ «Οι παγίδες» του XVI, ώστε να έχει ό,τι έχουν και τα δύο.

⭐⭐⭐ **Η ΣΤΑΘΕΡΑ ΠΟΥ ΓΕΝΝΗΘΗΚΕ ΕΔΩ (50): ΤΟ ΕΙΔΟΣ ΤΗΣ ΕΓΓΥΗΣΗΣ ΑΚΟΛΟΥΘΕΙ
ΤΟ ΠΟΙΟΣ ΕΓΡΑΨΕ ΤΟ ΑΡΧΕΙΟ.** Τα XVI/XVII τα έστειλε έτοιμα → sha256, που
αποδεικνύει «δεν το πείραξα». Το XVIII το έγραψα ΕΓΩ → ένα sha256 δεν αποδεικνύει
τίποτα, **θα συμφωνούσε με κάθε λάθος μου τέλεια**. Άρα πήρε τη γείωση της
Ιστορίας: `curl` στο **ebooks.edu.gr** (ΜΑΘΗΜΑ XVIII, sha256 του κατεβάσματος
καρφωμένο στην πηγή), το κείμενο ζει χωριστά στο
`tests/latinika-lectio18.source.txt`, και το test **ενώνει τις 11 ενότητες και
απαιτεί να ξαναδίνουν το βιβλίο αυτολεξεί**. Ίδιο μάθημα με τη σταθ. 49.

- ⚠️⚠️ **ΤΟ ΒΙΒΛΙΟ ΕΧΕΙ ΤΥΠΟΓΡΑΦΙΚΟ ΚΑΙ Η ΣΕΛΙΔΑ ΕΧΕΙ ΔΙΚΙΟ.** Η ψηφιακή
  έκδοση γράφει `pergit ad proximan speluncam` — το `spelunca` είναι θηλυκό σε
  αιτιατική, ο μόνος δυνατός τύπος είναι **`proximam`**, και έτσι το τυπώνει και
  το φυλλάδιο. ⛔ Μη «διορθώσεις» τη σελίδα προς την πηγή. Η γείωση σε επίσημο
  κείμενο **δεν σημαίνει ότι το επίσημο κείμενο είναι αλάνθαστο** — σημαίνει ότι
  κάθε απόκλιση πρέπει να είναι ΔΗΛΩΜΕΝΗ και αιτιολογημένη.
- ⭐ **ΚΑΙ ΟΙ ΑΠΟΚΛΙΣΕΙΣ ΕΠΙΒΑΛΛΟΝΤΑΙ ΘΕΤΙΚΑ.** Οι δύο μετασχηματισμοί
  (αφαίρεση αστερίσκων, `proximan`→`proximam`) θα γίνονταν σιωπηλά **no-op** αν
  η πηγή άλλαζε, και θα έκρυβαν πραγματική απόκλιση. Το test απαιτεί ρητά να
  ΥΠΑΡΧΟΥΝ ακόμη στην πηγή. Ένας μετασχηματισμός που δεν χτυπάει τίποτα είναι
  ένας έλεγχος που δεν ελέγχει τίποτα.
- ⚠️ **Η μετάφραση ΔΕΝ είναι του βιβλίου** — μεταγράφηκε από τη φωτογραφία, ο
  μόνος τρόπος που υπήρχε. Το λέει η πηγή, το λέει το υποσέλιδο της σελίδας, το
  ελέγχει το test (σταθ. από `als_latinika_page`: όταν υπάρχει αρχείο, ΠΟΤΕ OCR
  — εδώ δεν υπήρχε, άρα η αβεβαιότητα **γράφεται**, δεν σιωπάται).
- ⛔ **ΔΙΚΟ ΤΟΥ ΚΛΕΙΔΙ: `lectio18_known_v1`.** XVII και XVIII μοιράζονται
  μηχανή· ένα copy-paste κλειδί θα έδειχνε την **ίδια πρόοδο σε δύο κάρτες** και
  θα φαινόταν απόλυτα αληθινό. Το §6 του test το αποδεικνύει με χωριστό σενάριο:
  γεμίζει ΜΟΝΟ το κλειδί του XVII και απαιτεί το XVIII να λέει «δεν ξεκίνησε».
- Το hero της βιβλιοθήκης πάει **27 → 38 προτάσεις** και «Δύο» → «Τρεις
  ενότητες», όλα **μετρημένα από τα πακέτα** (σταθ. 33).
- ⭐ **Το render ξανά:** η σελίδα οδηγήθηκε ζωντανή μέσω **CDP** (το headless
  `--screenshot` κρεμάει σε αυτό το μηχάνημα) σε 1440px και 500px —
  0 σφάλματα, 0 οριζόντιο overflow, και επαληθεύτηκαν με κλικ η ομάδα
  λατινικά↔ελληνικά, η συντακτική σημείωση, το «το ξέρω», τα δύο κρυψίματα και
  το «μηδένισε».

**2026-08-27 — `als-v515` — ΙΣΤΟΡΙΑ: ΤΑ ΔΗΜΟΣΙΑ ΕΡΓΑ (β7), ΟΛΟΚΛΗΡΗ**
(`tests/istoria-data.test.js` 14.734 → **16.727/0**· `tests/istoria-plag.test.js`
**1.190/0**· `tests/istoria-ear.test.js` **864/0**· `smoke-test.sh` **SMOKE_OK**,
52 html · 528 links. ⚠️ `ladders.test.js` **98/4** — ΠΡΟΫΠΑΡΧΟΝ, ξαναεπαληθεύτηκε
με `git stash`. Άλλαξαν **τέσσερα αρχεία** και η `istoria.html` ΔΕΝ αγγίχτηκε:
`istoria-data.js`, το test του, το `docs/ISTORIA_SPEC.md` και το `sw.js`.)

Η δέκατη τρίτη υποενότητα, και οι **πέντε** παράγραφοι της «7. Τα δημόσια έργα»:
**18 σημεία, 68 στοιχεία** (20 / 12 / 13 / 10 / 13 ανά παράγραφο), 4 όροι του
βιβλίου, 29 λέξεις λεξιλογίου, 8 σημειώσεις πλαισίου, 5 χρονολογίες. Corpus:
**13 ενότητες · 190 σημεία · 16.727 βεβαιώσεις**.

- **Η ΓΕΙΩΣΗ, ΙΔΙΑ ΜΕ ΠΑΝΤΑ.** `curl` στο `ebooks.edu.gr` (`index1_3.html`,
  2026-08-27), **δύο ανεξάρτητοι εξαγωγείς** → ταυτόσημες 5/5. ⚠️ Ο πρώτος
  (ανά `<p>`) δίνει **ΤΡΕΙΣ** μπλοκ, γιατί το ebook κλείνει τις τρεις πρώτες
  παραγράφους σε ΕΝΑ `<p>` — η ταύτιση ελέγχθηκε **ως ένωση**, όχι ένα-προς-ένα.
  Ο δεύτερος (ολικό strip με τα block tags σε νέα γραμμή) δίνει και τη λεζάντα
  «Η διάνοιξη της διώρυγας της Κορίνθου», που πήγε στο `context`.
- ⭐⭐⭐ **ΤΟ ΜΑΘΗΜΑ, ΚΑΙ ΕΙΝΑΙ ΓΙΑ ΤΟ ΕΡΓΑΛΕΙΟ ΜΟΥ, ΟΧΙ ΓΙΑ ΤΗΝ ΥΛΗ: Η ΣΑΡΩΣΗ
  ΣΥΓΚΡΟΥΣΕΩΝ «ΛΕΞΕΙΣ >4 ΧΑΡΑΚΤΗΡΩΝ» — ΑΥΤΗ ΠΟΥ ΔΟΥΛΕΨΕ ΣΤΗ b6 — ΕΙΝΑΙ Η ΙΔΙΑ
  ΤΥΦΛΗ ΣΕ ΜΙΑ ΠΑΓΙΔΑ.** Το **«ελών» είναι ΤΕΣΣΕΡΑ γράμματα**, άρα δεν μπήκε
  ποτέ στη σάρωση — και ζει **αυτούσιο μέσα στην «ελονοσίας»** (`ελον` ⊂
  `ελονοσιασ`), δύο προτάσεις παρακάτω, στην **ΙΔΙΑ παράγραφο**, με άσχετο
  νόημα (βάλτος vs αρρώστια). Το βρήκε **το μάτι, όχι η μέτρηση**. Ξανατρέχτηκε
  με όριο `>3` και βγήκαν **32 συγκρούσεις αντί για 13**. ⭐ Ο κανόνας:
  **το φίλτρο μήκους δεν προστατεύει από τίποτα — προστατεύει ΕΜΕΝΑ από τον
  θόρυβο, και ο θόρυβος κρύβει παγίδες. Σάρωσε από >3 και διάβασε όλες.**
- ⭐⭐ **Η ΔΕΥΤΕΡΗ ΚΑΛΥΤΕΡΗ: ΕΝΑ ΣΤΟΙΧΕΙΟ ΜΕ ΔΥΟ ΛΕΞΕΙΣ ΚΑΙ ΚΑΘΕ ΜΙΑ ΤΟΥΣ ΕΙΝΑΙ
  ΠΑΓΙΔΑ.** Το «πέρασε στην **πρώτη θέση**» (3η παρ.): `πρωτη` ⊂ `προτυπα`
  («ευρωπαϊκά πρότυπα», 1η παρ.) ΚΑΙ `θεσι` ⊂ `προθεσισ` («οι προθέσεις»,
  1η παρ.). Χωριστά κλειδιά θα άναβαν από την 1η παράγραφο· **σώζεται μόνο από
  τη ΓΕΙΤΝΙΑΣΗ** — `['πρώτη θέση']` ως ΜΙΑ συμβολοσειρά, γιατί το `pho` πετάει
  τα κενά. Ίδια θεραπεία με το «απόπειρα ανάπτυξης» της b6.
- ⚠️ **Οι υπόλοιπες πέντε, κλειδωμένες ΟΝΟΜΑΣΤΙΚΑ**: `κοντά` ⊂ `βρίσκονταν`
  (δύο παράγραφοι μακριά) · `δρόμοι` ⊂ `σιδηροδρομικό/σιδηροδρομικών` (γι' αυτό
  ΚΑΝΕΝΑ στοιχείο δεν κλειδώνει σε γυμνό «δρόμ-») · `ενδιαφέρον` ⊂
  `ενδιαφέροντος` (**ΑΝΤΙΘΕΤΟ νόημα**: η διοίκηση ΘΕΛΕΙ έργα, οι ιδιώτες ΔΕΝ) ·
  `δίκτυο` ⊂ `δικτύου` · `κράτους` ⊂ `επικρατούσαν`.
- ⚠️ **ΔΥΟ ΙΣΙΕΣ ΑΠΟΣΤΡΟΦΟΙ (U+0027) ΤΟΥ ΒΙΒΛΙΟΥ** («Κάτω απ' αυτές», «απ'
  ευθείας») → οι δύο πρώτες παράγραφοι σε **ΔΙΠΛΑ** εισαγωγικά, σε αντίθεση με
  τη b6 που δεν είχε καμία. Και **ένα τυπογραφικό που ΔΕΝ διορθώνεται**:
  «Η δραστηριότητα του» — το ebook δεν τονίζει το «του». Και τα δύο κλειδωμένα.
- ⭐ **ΔΟΚΙΜΑΣΜΕΝΟ ΟΤΙ ΔΑΓΚΩΝΕΙ: 12 μεταλλάξεις, ΟΛΕΣ δάγκωσαν, καθαρό 0** —
  χαλαρό «ελών» · χαλαρό «πρώτη»+«θέση» · χαλαρό «κοντά» · γυμνό «δρόμων» ·
  γυμνό «ενδιαφέρον» · ένα ψηφίο (1893→1898) · ψεύτικη άγκυρα · μία λέξη του
  κειμένου · η απόστροφος «διορθωμένη» · ο άτονος τύπος «διορθωμένος» ·
  διαρροή λέξης της επόμενης υποενότητας · σημείο με ΕΝΑ στοιχείο.
- ⭐ **ΤΟ RENDER ΞΑΝΑΕΠΙΑΣΕ ΑΥΤΟ ΠΟΥ ΚΑΜΙΑ ΒΕΒΑΙΩΣΗ ΔΕΝ ΠΙΑΝΕΙ — 13 ΔΙΟΡΘΩΣΕΙΣ
  ΔΙΚΕΣ ΜΟΥ**, όλες σε ελληνικά που κανένα test δεν διαβάζει: «σαράντα χρόνια»
  που **ανήκει στη b6 και όχι εδώ** · «δεδομένα» που δεν είναι λέξη του βιβλίου ·
  «τα απαραίτητα» ενώ το βιβλίο γράφει γενική («των απαραίτητων… έργων») ·
  «ΓΙ ΑΥΤΟ» χωρίς απόστροφο · η αντίφαση «το ΜΟΝΑΔΙΚΟ όνομα τόπου… μαζί με» ·
  ο Εύριπος που τοποθετήθηκε «ανάμεσα στη Χαλκίδα και τη Στερεά» (η Χαλκίδα
  ΕΙΝΑΙ στην Εύβοια) · και ένα «τρεις φορές» που ήταν **δύο, μετρημένο**.
- ⚠️ **Το `pho` ΔΕΝ διαβάζει «19ου» όταν το αυτί γράφει «δέκατου ένατου».**
  Η προσομοίωση `αριθμοί με λέξεις` έβγαζε **99%** για ένα ακριβώς στοιχείο·
  μπήκαν οι ολόγραφες εναλλακτικές και η b7 έγινε **100% και στις έξι γραμμές**
  του πίνακα του αυτιού — η μόνη ενότητα του corpus που το κάνει.
- ⚠️ **Η §1.2 και ο πίνακας §1.3 του `docs/ISTORIA_SPEC.md` ΞΑΝΑΜΕΤΡΗΘΗΚΑΝ:**
  `units 13 · TOTAL points 190 · unique-paragraph 190 · multi 0 · NONE 0`.
- 🔴 **ΕΠΟΜΕΝΟ = «8. Το δίκτυο των σιδηροδρόμων»**, δηλωμένο ρητά στο `context`
  της b7 — και η b7 ελέγχει ότι δεν διέρρευσε λέξη του (Τρικούπης, Πειραιάς,
  9 χιλιόμετρα, Βόλος).
- 🔴 **Αδοκίμαστο στη συσκευή του** — θέλει πλήρες reopen του PWA.

**2026-08-25 — `als-v510` — Ο ΤΟΝΙΣΜΟΣ ΞΑΝΑΧΤΙΣΤΗΚΕ: ΤΑ ΣΚΑΛΟΠΑΤΙΑ**
(`tests/tonos-page.test.js` **46/0** — νέο· `tests/tonos-fyllo.test.js` **644/0**
— νέο· `smoke-test.sh` **SMOKE_OK**, 51 html · 514 links. ⚠️ `ladders.test.js`
**98/4 ΠΡΟΫΠΑΡΧΟΝ**, αποδεδειγμένο με `git stash`: ίδιοι αριθμοί πριν/μετά.)

Δικά του λόγια, με τεστ την επομένη στις 15:00: *«αυτο που εχουμε για τον
τονισμο ειναι χαλια αρα δεν μετραει»* → *«ετσι οπως το θελω ΠΡΑΓΜΑΤΙΚΑ»*.

### ⭐⭐ «ΧΑΛΙΑ» ΗΤΑΝ ΤΡΙΑ ΝΟΥΜΕΡΑ, ΚΑΙ ΤΟ ΠΡΩΤΟ ΗΤΑΝ ΣΟΒΑΡΟ
- ⛔⛔ **Η ΓΡΑΜΜΑΤΟΣΕΙΡΑ, ΣΕ ΣΕΛΙΔΑ ΠΟΥ ΤΟ ΘΕΜΑ ΤΗΣ ΕΙΝΑΙ Ο ΤΟΝΟΣ.** Η λέξη
  ζητούσε `--au-serif` = Instrument Serif (**ΜΗΔΕΝ ελληνικά**) → Georgia
  (**ΜΗΔΕΝ πολυτονικά**). Άρα κάθε δασεία και κάθε περισπωμένη έπεφτε
  **γλυφικό-γλυφικό** σε άλλη γραμματοσειρά από τα γράμματα δίπλα της. Και το
  `@import` της Instrument Serif ζει στο `aurora-page.css`, **που η σελίδα δεν
  φόρτωνε καν** (σταθ. 43, τρίτη σελίδα που την πατάει). Λύση: δικό της `<link>`
  GFS Didot + `--ar-gr`, ⛔ Georgia απαγορευμένη ακόμη και ως fallback.
- ⛔ **Πλάτος 600px** (το κουίζ **520**) σε λάπτοπ ~2000px — σταθ. 51. → 1240,
  θραύση στα 1040, **κανένα `min-width`**.
- ⛔ **Αντίθεση 2.98:1** σε 9 σημεία, μαζί με την **εξήγηση της ερώτησης** στα
  12.5px — σταθ. 52. → `--au-dim`, ≥13.5px.

### ⭐⭐⭐ ΤΑ ΣΚΑΛΟΠΑΤΙΑ — Η ΜΕΘΟΔΟΣ ΕΙΝΑΙ ΔΙΚΗ ΤΟΥ, ΟΧΙ ΔΙΚΗ ΜΟΥ
Στο ασκησάκι της σελ. 5 έγραψε **ΜΟΝΟΣ ΤΟΥ «Μ-Β» πάνω από κάθε λέξη ΠΡΙΝ βάλει
τόνο**. Αυτό διάβασα από τη φωτογραφία, και αυτό ΕΙΝΑΙ η σελίδα: **① η λήγουσα
→ ② η παραλήγουσα → ο ΝΟΜΟΣ ΑΝΑΒΕΙ ΜΟΝΟΣ ΤΟΥ → ③ ο τόνος.**
- ⭐ **Ο νόμος εμφανίζεται ΜΟΝΟ ΜΕΤΑ τις δύο δηλώσεις**, γιατί αυτό είναι:
  το ΑΠΟΤΕΛΕΣΜΑ τους. Δείχνεται πριν, γίνεται άλλη μια πρόταση να αποστηθίσει.
- ⛔ **Καμία απάντηση δεν είναι αποθηκευμένη** — και οι τρεις βγαίνουν από
  `quantity()` / `forcedType()` / `analyze()` πάνω στη γραμμένη ΣΤΟ ΧΕΡΙ λέξη.
- Οι λάθος επιλογές είναι **πραγματικές γειτονικές γραφές της ΙΔΙΑΣ λέξης**
  (`restrike`): `πλῆρης / πληρής / πλήρης`. Οδηγήθηκε ζωντανά, 3/3 σκαλοπάτια.
- «Έφυγε» = καθαρό πέρασμα **ΜΕ ΤΗΝ ΠΡΩΤΗ** (σταθ. 43)· ξαναγέμισμα = ΡΗΤΗ πράξη.

### ⛔⛔ ΤΟ ΛΑΘΟΣ ΜΟΥ ΠΟΥ ΕΠΙΑΣΕ Ο ΦΡΟΥΡΟΣ — ΣΤΑΘ. 50, ΠΛΗΡΩΜΕΝΗ ΞΑΝΑ
Ονόμασα τα κουμπιά `.tn-opt/.tn-opts` — **ονόματα που ΑΝΗΚΑΝ ΗΔΗ στο ΚΟΥΙΖ**,
όπου το δοχείο είναι `flex-direction:column`. Δύο συνέπειες, καμία ορατή σε
συντακτικό έλεγχο: τα δύο κουμπιά ποσότητας βγήκαν **710px το καθένα μέσα σε
δοχείο 710** και στοιβάχτηκαν, **και** οι δικοί μου κανόνες ΞΑΝΑΕΝΤΥΝΑΝ σιωπηλά
το κουίζ που δεν είχα αγγίξει. Τώρα λέγονται `.st-opt(s)`, και το test φυλάει
**ΚΑΙ ΤΑ ΔΥΟ** — το δικό μου να είναι πλέγμα, ΚΑΙ του κουίζ να έχει μείνει
στήλη (οδηγήθηκε: `flex/column`, 2 επιλογές × 720px, αμετάβλητο).
⭐ **Το πλέγμα είναι άτρωτο σε αυτό:** οι στήλες ορίζονται από το δοχείο, όχι
από ένα `width` που κληρονομείται και γίνεται flex-basis.

### ⚠️ ΤΡΙΑ ΠΟΥ ΒΡΗΚΕ ΤΟ RENDER, ΑΟΡΑΤΑ ΣΤΙΣ ΒΕΒΑΙΩΣΕΙΣ
1. **Ο πίνακας των κανόνων έβγαινε 1888px σε κουτί 1160** (`min-width:max-content`)
   και η **ΤΕΤΑΡΤΗ στήλη — «Η ΜΗΧΑΝΗ», δηλαδή ΑΚΡΙΒΩΣ η στήλη της ειλικρίνειας —
   ζούσε εκτός οθόνης.** Πίνακας που κρύβει τη μία του στήλη δεν είναι
   αντιπαραβολή. Χωράει στο λάπτοπ τώρα· η κύλιση έμεινε για το τηλέφωνο.
2. **Η σκηνή έμενε 760px μέσα σε 1240** και άφηνε 480px μαύρο δεξιά — η ίδια
   «λωρίδα μέσα σε μαύρο» με άλλο πρόσωπο. → δεύτερη στήλη που κάνει δουλειά:
   **ο νόμος ανάβει ΔΙΠΛΑ στη λέξη**, όχι τρεις οθόνες πιο κάτω, και μένει
   ΜΙΑ αριστερή ακμή.
3. Οι δύο ανέλεγκτοι κανόνες τύπωναν **σκέτη παύλα** (σταθ. 33).

### Αρχεία
`tonos-fyllo.js` (η ύλη σελ. 5-6-7: 8 κανόνες αυτολεξεί, 75 δασυνόμενες, 10
άτονες, το ασκησάκι με 16 λέξεις) · `tonos.html` (σκαλοπάτια + φυλλάδιο) ·
`arxaia.html` (ράφι **Τονισμός**, ΠΡΩΤΟ στη σειρά του βιβλίου) ·
`tests/tonos-fyllo.test.js` · `tests/tonos-page.test.js`.
⛔ **Το `ton:v1` το κατέχει η `tonos.html`** (σταθ. 16): η `arxaia.html` το
ΔΙΑΒΑΖΕΙ μόνο, και το test απαγορεύει κάθε `setItem` από εκεί. Ίδιο σχήμα με
τα δύο πακέτα της εισαγωγής στον Γνωστό, που εγκρίθηκε στην als-v497.

### 🔴 ΤΑ ΚΕΝΑ, ΔΗΛΩΜΕΝΑ
- 🔴 **Αδοκίμαστο σε συσκευή του.** Οδηγήθηκε πλήρης ροή σε αληθινό Chrome
  (3 σκαλοπάτια, ο νόμος 5 άναψε, `πλήρης` σωστό, μετρητής 1→2, αποθηκεύτηκε)
  και ρεντεραρίστηκε στα 1440px. **Κανένα δάχτυλο.**
- 🔴 **ΔΕΝ χτίστηκαν ΟΙ ΚΛΙΣΕΙΣ (σελ. 8-21) και ΟΙ ΑΡΧΙΚΟΙ ΧΡΟΝΟΙ (141-148)** —
  οι άλλες δύο τρίτες της αυριανής ύλης. Είναι η επόμενη δουλειά, με τη σειρά:
  **το ΔΕΝΤΡΟ της γ΄ κλίσης** (απάντησε «το δεύτερο»: του δείχνουν έναν τύπο
  και ρωτάνε τι είναι → το δέντρο ΕΙΝΑΙ ολόκληρη η άσκηση), μετά τα ρήματα.
- 🔴 **Τρία κενά μεταγραφής δηλωμένα ΜΕΣΑ στη σελίδα** (`GAPS`): οι τελευταίες
  σειρές των δασυνόμενων, οι **δικές του χειρόγραφες απαντήσεις** στο ασκησάκι
  (η φωτογραφία είναι σε γωνία — ⛔ δεν μαντεύτηκαν), και **4 από τις ~20
  λέξεις** του ασκησακιού. Μία καθαρή φωτογραφία ΜΟΝΟ του ασκησακιού τα κλείνει.

---

**2026-08-24 — `als-v509` — ΙΣΤΟΡΙΑ: Η ΒΙΟΜΗΧΑΝΙΑ (β6), ΟΛΟΚΛΗΡΗ**
(`tests/istoria-data.test.js` 12.609 → **14.734/0**· `tests/istoria-plag.test.js`
**1.081/0**· `smoke-test.sh` **SMOKE_OK**. ⚠️ `ladders.test.js` **98/4** και
`goals-rhythm.test.js` **7/1** — και τα δύο ΠΡΟΫΠΑΡΧΟΥΝ στο HEAD και δεν
αγγίχτηκε η `istoria.html` ούτε σε μία γραμμή.)

Η δωδέκατη υποενότητα, και οι **πέντε** παράγραφοι της «6. Η βιομηχανία»:
18 σημεία, **74 στοιχεία** (13 / 19 / 19 / 10 / 13 ανά παράγραφο), 3 όροι του
βιβλίου, 25 λέξεις λεξιλογίου, 7 σημειώσεις πλαισίου, 5 χρονολογίες. **Μόνο δύο
αρχεία άλλαξαν** — `istoria-data.js` και το test του· η σελίδα είναι δεδομενο-
οδηγούμενη και δεν χρειάστηκε γραμμή.

- **Η ΓΕΙΩΣΗ, ΙΔΙΑ ΜΕ ΠΑΝΤΑ.** `curl` στο `ebooks.edu.gr` (`index1_3.html`,
  2026-08-24), **δύο ανεξάρτητοι εξαγωγείς** (ανά `<p>` vs ολικό strip με τα
  block tags σε νέα γραμμή) → **ταυτόσημες 5/5**, και οι πέντε παράγραφοι
  επικολλημένες αυτούσιες στο `BOOK` του test.
- ⭐ **ΤΟ ΝΕΟ ΕΙΔΟΣ ΠΑΓΙΔΑΣ, ΚΑΙ ΔΕΝ ΤΟ ΕΠΙΑΣΕ Ο ΦΡΟΥΡΟΣ 8γ.** Το στοιχείο
  «μια πρώτη απόπειρα ανάπτυξης» ήταν κλειδωμένο σε `['απόπειρα','ανάπτυξης']`.
  Κανένα ΑΛΛΟ σημείο δεν το άναβε **μόνο του** — το άναβε ο **ΣΥΝΔΥΑΣΜΟΣ** δύο
  άλλων σημείων της ίδιας παραγράφου («η απόπειρα αυτή έχασε τη δυναμική της» +
  «με τάσεις ανάπτυξης της βαριάς βιομηχανίας»). Ο 8γ συγκρίνει
  **σημείο-προς-σημείο**· το `tests/istoria-plag.test.js` απαγγέλλει
  **ΟΛΟΚΛΗΡΗ την παράγραφο** και γι' αυτό το βρήκε. **Η θεραπεία είναι
  ΓΕΙΤΝΙΑΣΗ**: `['απόπειρα ανάπτυξης']` ως ΜΙΑ συμβολοσειρά, γιατί το `pho`
  πετάει τα κενά. Γενικά: **ένα `say` με δύο κοινές λέξεις είναι ασφαλές μόνο
  απέναντι σε ΕΝΑ σημείο τη φορά** — η μονάδα εξέτασης όμως είναι ο
  πλαγιότιτλος, δηλαδή ΠΟΛΛΑ σημεία μαζί.
- ⚠️ **ΟΙ ΣΥΓΚΡΟΥΣΕΙΣ ΜΕΤΡΗΘΗΚΑΝ ΠΡΙΝ ΓΡΑΦΤΟΥΝ ΤΑ ΚΛΕΙΔΙΑ** (σαρώθηκε κάθε
  λέξη >4 χαρακτήρων της υποενότητας και συγκρίθηκαν οι φωνητικές μορφές):
  **βιομηχανίας ⊂ τσιμεντοβιομηχανίας** (η b4 ξανά), **δύναμη ⊂ αδυναμίες**
  (και οι δύο στην ΙΔΙΑ πρόταση, με αντίθετο νόημα), **πρόθεση ⊂ προθέσεις**,
  **εφαρμογή ⊂ εφαρμογής**. Και οι τέσσερις κλειδώνονται ΟΝΟΜΑΣΤΙΚΑ στο test —
  μια γενική δικλείδα δεν λέει ΠΟΙΑ παγίδα υπήρχε.
- ⭐ **ΚΑΜΙΑ ΑΠΟΣΤΡΟΦΟΣ ΣΕ ΟΛΗ ΤΗΝ ΥΠΟΕΝΟΤΗΤΑ**, μετρημένο και κλειδωμένο. Γι'
  αυτό, και μόνο γι' αυτό, οι παράγραφοι ζουν σε ΜΟΝΑ εισαγωγικά αντί για διπλά
  (b3b/b4/b5). Μένει άθικτο ένα τυπογραφικό του βιβλίου: **«-εδαφικά και
  πληθυσμιακά-» με ΕΝΩΤΙΚΑ (U+002D)**, όχι παύλες — βεβαίωση σε `charCodeAt`.
- **11 ΜΕΤΑΛΛΑΞΕΙΣ, ΟΛΕΣ ΔΑΓΚΩΣΑΝ** (ψηφίο στο κείμενο, λέξη στο κείμενο, γυμνό
  «βιομηχανίας», γυμνό «δύναμη», σβησμένη άγκυρα, τυπογραφικό εισαγωγικό, παύλα
  αντί ενωτικού, άγκυρα σε λεξιλόγιο, μακρύ `section`, σημείο με ΕΝΑ στοιχείο,
  άγκυρα εκτός βιβλίου). Καθαρό **0** επιζώντων.
- **ΤΟ RENDER ΔΙΑΒΑΣΤΗΚΕ** (harness που κρατάει το inline script και πετάει
  ΚΑΘΕ εξωτερικό `src`, με βεβαίωση ότι κανένα δεν επέζησε — σταθ. 8/19):
  η γραμμή ύλης βγαίνει **«6. Η βιομηχανία · 18 σημεία, 74 στοιχεία» (40 χαρ.,
  μπάτζετ 50)**, η κάρτα λέει «5 παράγραφοι · 74 στοιχεία», η αρίθμηση πήγε
  **«0 από 12 υποενότητες»**, η χρονογραμμή **28 χρονολογίες**. Το ίδιο render
  έπιασε το μόνο δικό μου λάθος που δεν έπιανε καμία βεβαίωση: **«αλήθινη»**.
- **Η §1.2 ΤΟΥ SPEC ΞΑΝΑΜΕΤΡΗΘΗΚΕ:** `units 12 · TOTAL points 172 ·
  unique-paragraph 172 · multi 0 · NONE 0`. Ο πίνακας §1.3 πήρε επιτέλους και
  τη `b5` (77) και τη `b6` (74).
- 🔴 **ΕΠΟΜΕΝΟ = «7. Τα δημόσια έργα» (β7).** Δηλωμένο ρητά στο `context` της
  b6 ως ΜΗ ανατεθειμένο, όπως έκανε και η b5 για τη βιομηχανία.
- 🔴 **Απρόβλητο στο κινητό του** — δεν έχει ανοίξει τη σελίδα ακόμη.

**2026-08-23 — `als-v504` — Η 2η ΕΝΟΤΗΤΑ ΤΩΝ ΑΡΧΑΙΩΝ, ΚΑΙ ΟΙ ΤΟΝΟΙ**
(`tests/arxaia-gnosto.test.js` **485/0** — ήταν 431· `smoke-test.sh` **SMOKE_OK**.
⚠️ `ladders.test.js` **98/4** — οι 4 ΠΡΟΫΠΑΡΧΟΥΝ στο HEAD, αφορούν την `istoria.html`.)

Δικά του λόγια: *«στη 1η ενότητα οι τόνοι των αρχαίων δεν είναι σωστοί»* και
*«θα φτιάξεις μόνο για τη μετάφραση όμως, όχι σαν την 1η ενότητα που έχει
χίλια πράγματα»*, *«ζητώ μόνο τη μετάφραση»*.

⭐⭐⭐ **ΟΙ ΤΟΝΟΙ ΔΕΝ ΗΤΑΝ ΠΟΤΕ ΣΤΑ ΔΕΔΟΜΕΝΑ — ΗΤΑΝ Η ΓΡΑΜΜΑΤΟΣΕΙΡΑ.** Το
test τα έλεγχε γράμμα προς γράμμα απέναντι στην πηγή και περνούσε όλη την
ώρα. Μετρήθηκε από τα cmap, δεν εικάστηκε (Greek Extended, U+1F00-1FFF):
**Instrument Serif → ΜΗΔΕΝ ελληνικά** (μόνο latin + latin-ext στο Google
Fonts CSS)· **Georgia → 74 ελληνικά, ΜΗΔΕΝ πολυτονικά**· **SF Pro → 2**·
**Times New Roman → 233**. Άρα στο «Διὰ γὰρ» το «Δι» το ζωγράφιζε η Georgia
και το «ὰ» ΑΛΛΗ γραμματοσειρά — μέσα στην ίδια λέξη. Λύση: `--ar-gr` =
**GFS Didot** (Ελληνική Εταιρεία Τυπογραφικών Στοιχείων, greek-ext
ολόκληρο) με Times New Roman από κάτω, σε **11 επιλογείς και στους δύο
κόσμους** — ο ΑΓΝΩΣΤΟΣ έπασχε το ίδιο και κανείς δεν το είχε δει.
⛔ **Η Georgia απαγορεύεται σε αυτή τη στοίβα, ακόμη και ως fallback**:
είναι η ΑΙΤΙΑ, οπότε ένα πεσμένο CDN θα επανέφερε το bug αυτούσιο.

⭐⭐⭐ **Η ΠΗΓΗ ΤΩΝ ΑΡΧΑΙΩΝ ΑΛΛΑΞΕ ΕΙΔΟΣ, ΚΑΙ ΕΙΝΑΙ ΑΝΑΒΑΘΜΙΣΗ.** Ο κανόνας
ήταν «μεταγράφω ΣΤΟ ΧΕΡΙ, φυλλάδιο φροντιστηρίου δεν υπάρχει online». Η 2η
ενότητα ΔΕΝ είναι φυλλάδιο — είναι ο **επίσημος Φάκελος Υλικού στο
ebooks.edu.gr** και κατεβαίνει. Άρα τα Αρχαία πήραν επιτέλους τη **ΓΕΙΩΣΗ
της Ιστορίας**: `tests/arxaia-enotita2.source.txt` κρατάει τις ΩΜΕΣ γραμμές
του PDF με καρφωμένο sha256, και ο αχυρώνας ΠΑΡΑΓΕΤΑΙ από αυτές μέσα στο
test. Δεν υπάρχει βήμα «το έγραψα εγώ» πουθενά στην αλυσίδα.

⚠️⚠️ **ΤΟ PDF ΓΡΑΦΕΙ ΣΕ OXIA, ΟΧΙ ΣΕ TONOS** — «ά» = U+1F71, όχι U+03AC.
ΦΑΙΝΟΝΤΑΙ ΟΛΟΙΔΙΑ. Ζουν στο Greek Extended, δηλαδή ΑΚΡΙΒΩΣ στο κενό που
γεννούσε τους σπασμένους τόνους, και καμία αναζήτηση δεν θα τα έβρισκε. Το
NFC είναι ο έλεγχος (κανονικοποιεί oxia→tonos και τίποτα άλλο στα ελληνικά)
και το test τα **απαγορεύει σε όλο το corpus**.

⚠️ **ΔΥΟ ΜΕΤΑΦΡΑΣΕΙΣ ΣΕ ΔΥΟ ΣΤΗΛΕΣ, ΚΑΙ ΜΠΛΕΚΟΝΤΑΙ ΜΕΣΑ ΣΤΗΝ ΙΔΙΑ ΛΕΞΗ**
αν βγάλεις το κείμενο μονοκόμματα. Ο διαχωρισμός έγινε **γεωμετρικά**
(PDFKit `selectionForRect`, x < 305pt), όχι με το μάτι. Κρατήθηκε ΜΟΝΟ ο
**Λ. Μπενάκης** — «από μετάφραση θέλω μόνο αυτήν που σου έστειλα στη φώτο».

⭐ **ΤΡΙΤΟ ΕΙΔΟΣ ΕΝΟΤΗΤΑΣ: `onlyAlign`.** Η `gk2` έχει ΕΝΑ στρώμα (13
περίοδοι, 99 ζευγάρια) και τίποτε άλλο: ούτε σκελετό, ούτε «με απλά λόγια»,
ούτε «κλειδί». Το test **γυρίζει** αντί να χαλαρώνει — απαιτεί να ΜΗΝ τα
έχει, ώστε να μη γεμίσει «για πληρότητα» κάποια μέρα.
⚠️ Το `un.skeleton.points.length` ήταν γραμμένο ΩΜΑ σε **4 σημεία**: μια
ενότητα χωρίς σκελετό δεν θα έδειχνε λάθος νούμερο, θα **μαύριζε ΟΛΟΚΛΗΡΟ
τον Γνωστό** με TypeError μέσα στο `renderHome`. Ίδιο σχήμα με το
περιστατικό του `elCount` (als-v499). Μπήκε το `bits()` + φρουρός.

⭐⭐ **ΚΑΙ ΠΑΛΙ ΤΟ RENDER ΒΡΗΚΕ Ο,ΤΙ ΔΕΝ ΒΡΗΚΑΝ 483 ΒΕΒΑΙΩΣΕΙΣ.** Η πινακίδα
`byHeart` έλεγε «αυτό που λες **στην ανάκληση**» — η ανάκληση έφυγε ολόκληρη
στην als-v499 — και «στο **κλειδί** πιο κάτω», που η `gk2` δεν έχει. Σωστός
κώδικας, ψεύτικο κείμενο· καμία assertion δεν το άγγιζε.

**2026-08-22 — `als-v503` — Η ΕΙΣΑΓΩΓΗ ΤΩΝ ΛΑΤΙΝΙΚΩΝ**
(`tests/latinika-eisagogi.test.js` **29/0** — νέο· `latinika-lectio` **275/0**·
`launcher` **77/0**· `smoke-test.sh` **SMOKE_OK**, 51 html · 509 links.)

Δικά του λόγια: *«μέσα στο λατινικά page εκτός από τις ενότητες έχουμε να
μάθουμε και την εισαγωγή … αυτό θα το κοιτάω και θα το διαβάζω και θα κάνω
επανάληψη μέσα στη χρονιά μέχρι να δώσω πανελλήνιες … κάνε το ΛΕΞΗ ΠΡΟΣ ΛΕΞΗ
ΟΛΟΣΩΣΤΟ»*.

1. ⭐⭐⭐ **ΟΙ ΦΩΤΟΓΡΑΦΙΕΣ ΔΕΝ ΗΤΑΝ ΤΟ ΚΕΙΜΕΝΟ (σταθ. 49).** Ήρθαν 10 φωτο
   **ΑΝΑΠΟΔΑ** — το είπε ο ίδιος, και η σειρά επιβεβαιώθηκε από αριθμούς
   σελίδας ΚΑΙ από τη ροή του κειμένου. Χωρίς αρχείο για `cp`, μετέγραψα με
   OCR· **η μεταγραφή είχε αληθινά λάθη**. Τα σήμανα ρητά ως αβέβαια αντί να
   τα περάσω για σίγουρα — και μετά έστειλε το πλήρες κείμενο, που
   **αντικατέστησε ΟΛΟΚΛΗΡΟ** το `DATA[]`. Η δομή προσαρμόστηκε στη δική του
   ιεραρχία: **οκτώ** κεφάλαια, τα 5 είδη ΜΕΣΑ στην Προκλασική, «Α.»/«Β.» στην
   Κλασική, και ο τίτλος «Η εξέλιξη της ρωμαϊκής λογοτεχνίας» ως χώρισμα.
2. ⭐⭐ **ΤΟ «ΛΕΞΗ ΠΡΟΣ ΛΕΞΗ» ΑΠΟΔΕΙΚΝΥΕΤΑΙ, ΔΕΝ ΔΗΛΩΝΕΤΑΙ.** Η πηγή ζει
   χωριστά στο `tests/latinika-eisagogi.source.txt` και το τεστ απαιτεί **και
   οι 75 παράγραφοι να υπάρχουν αυτολεξεί** μέσα στη σελίδα. Όταν κάποιος
   αλλάξει λέξη, το τεστ λέει **ποια πρόταση λείπει**.
3. ⭐⭐ **ΤΟ RENDER ΤΟ ΕΠΙΑΣΕ ΞΑΝΑ, ΚΑΙ ΗΤΑΝ Η ΣΤΑΘ. 39 ΧΩΡΙΣ ΠΟΡΤΑ.** Το πρώτο
   scroll-render βγήκε **εντελώς μαύρο**: τα κεφάλαια γεννιόνταν `opacity:0`
   περιμένοντας `IntersectionObserver` που στο harness δεν χτύπησε ποτέ.
   Διορθώθηκε στη ΡΙΖΑ, όχι στο harness — γεννιούνται ορατά, η είσοδος μπαίνει
   πίσω από `html.anim` που βάζει η ίδια η JS, + δίχτυ `setTimeout`.
4. ⭐ **ΜΙΑ ΚΛΑΣΗ, ΜΙΑ ΔΟΥΛΕΙΑ (σταθ. 50).** Το `.wk` ήταν ΚΑΙ ετικέτα κάρτας
   ΚΑΙ τίτλος έργου inline, κι έσπαγε τη γραμμή στα τρία. Ο inline είναι `.ttl`.
5. ⛔ **ΚΑΙ ΕΒΓΑΛΑ ΔΙΚΕΣ ΜΟΥ ΛΕΞΕΙΣ ΠΟΥ ΕΙΧΑ ΒΑΛΕΙ.** «ρήτορας · φιλόσοφος ·
   πολιτικός» στις κάρτες προσώπων — δεν υπάρχουν στις σημειώσεις. Το τεστ
   φρουρεί: ετικέτα = ΜΟΝΟ τίτλος έργου από την πηγή, και κανένα ` · ` δεν
   ενώνει δύο λεζάντες.
6. ⚠️ **Ο ΦΡΟΥΡΟΣ ΤΟΥ HARNESS ΟΥΡΛΙΑΞΕ ΓΙΑ ΣΧΟΛΙΟ (σταθ. 19, ζωντανή).** Ο
   έλεγχος «καμία δέσμη sync σε harness» χτύπησε στη λέξη «Supabase» μέσα σε
   **σχόλιο** της `latinika.html`. Το φίλτρο θέλει το **πλαίσιο `src=`** —
   ένας φρουρός που κράζει για σχόλια είναι φρουρός που κάποιος θα χαλαρώσει.
7. 🔴 **ΑΝΟΙΧΤΑ**: (α) οι **7 λεζάντες** εικόνων μπήκαν, οι **εικόνες όχι** —
   περιμένουν crops. (β) 🔴 αδοκίμαστο στο κινητό του· ⚠️ το headless Chrome
   **δεν πάει κάτω από 500px πλάτος**, οπότε «κομμένο» screenshot είναι crop,
   όχι overflow (μετρήθηκε: `VW=500 SW=500`). (γ) η παρατήρηση για το
   `als-profile.js` της als-v502 **ισχύει ακόμη**.
...
|---|---|---|---|---|
| `lat:v1` | 🔴 **ΚΑΝΕΙΣ — als-v502** (η μηχανή → `archive/latinika-drill.html`) | — | — | — |
...
- ⛔⛔ **ΤΟ «ΞΕΚΙΝΑ ΑΠΟ ΤΑ ΛΑΤΙΝΙΚΑ» ΠΕΘΑΝΕ ΣΤΗΝ als-v502, ΚΑΙ ΤΟ ΓΡΑΦΩ ΕΔΩ
  ΓΙΑΤΙ ΕΝΑ ΠΛΑΝΟ ΠΟΥ ΔΕΙΧΝΕΙ ΣΕ ΝΕΚΡΟ ΚΩΔΙΚΑ ΔΙΑΒΑΖΕΤΑΙ ΣΑΝ ΕΤΟΙΜΟ.** Η
  `latinika.html` έγινε **βιβλιοθήκη ενοτήτων** κατόπιν δικής του εντολής: δεν
  έχει πια `startSession`/`endSession`/`sess`, και **δεν γράφει καθόλου στο
  `lat:v1`**. Οι γραμμές 449/456/317 δείχνουν σε άλλο αρχείο τώρα.
  → Το σχήμα που έψαχνες ζει αυτούσιο στο `archive/latinika-drill.html`, αλλά
  **αντίγραψε τον γραφέα του `tonos.html`** ([[als_tonos_page]]), που είναι ο
  μόνος ΖΩΝΤΑΝΟΣ. ⚠️ Στο αρχειοθετημένο λέγεται `ok`, όχι `right`, και **δεν
  έχει μετρητή απαντημένων** — χρειάζεται το ίδιο `sess.done`.
  → Και αν κάποτε ξαναχρειαστεί σκάλα για τα Λατινικά, **η μονάδα είναι η
  ΕΝΟΤΗΤΑ, όχι το κελί** — νέο `kind` στο `ladders.js`, ⛔ ΠΟΤΕ αριθμός
  μαντεμένος από το «το ξέρω» (σταθ. 33).
...
⚠️ **Η φράση «`latinika.html`-shaped» είναι ΙΣΤΟΡΙΚΗ.** Από την als-v502 η
`latinika.html` είναι ΒΙΒΛΙΟΘΗΚΗ ΕΝΟΤΗΤΩΝ, όχι drill· το σχήμα που εννοούσε
ζει στο `archive/latinika-drill.html`.
...
⚠️ **ΙΣΤΟΡΙΚΟ als-v449 — η σελίδα άλλαξε είδος στην als-v502** (βιβλιοθήκη
ενοτήτων, όχι drill). Το *wiring* παρακάτω ισχύει ακόμη ολόκληρο, γιατί το
`lat:v1` **συνεχίζει να συγχρονίζεται από τη σελίδα**· ό,τι αφορά τη ΜΗΧΑΝΗ
(`latin-engine.js`, κελιά, «Κλίση ολόκληρη») ζει πλέον στο `archive/`.
...
# ⚠️ `tests/*.test.js` is only 47 of them. reinstall-safety.js and
...
⚠️ **`tests/ladders.test.js` has been RED since before als-v505** — three
"istoria.html is unchanged" assertions plus one homework count. It is the
σταθ.-3 trap in §7 (a before/after proof that points at `HEAD`), NOT something
the Αρχαία work broke; verified by stashing. Fix it or delete it, but **do not
let it become the failure everyone learns to ignore.**
...
### ⭐⭐⭐ ΓΙΑΤΙ ΒΓΑΙΝΕΙ ΠΡΑΣΙΝΟ ΚΑΙ ΕΙΝΑΙ ΛΑΘΟΣ (als-v505→507, τρεις απορρίψεις στη σειρά)

Σε μία συνεδρία μου είπε τρεις φορές «όχι» σε κάτι που περνούσε **κάθε** test:
*«οχι δικο του πατζε»* → *«ειναι μια λιστα… ξανα σκεψου το απο την αρχη»* →
*«παντα το κανεις σε μορφη κινητου και ασχημο»*. Κάθε φορά νόμιζα ότι
συζητάμε γούστο. **Κάθε φορά υπήρχε νούμερο από κάτω.**

**Η ΡΙΖΑ: ρωτάω «λειτουργεί;» και σταματάω.** Τα tests απαντούν σε αυτό και
μόνο σε αυτό. Δεν απαντούν σε **«σε τι πλάτος, με τι αντίθεση, σε τι μέγεθος,
σε ποιο σχήμα»** — και εκεί ζει ό,τι βλέπει αυτός.

**ΟΤΑΝ ΛΕΕΙ «ΔΕΝ ΕΙΝΑΙ ΩΡΑΙΟ», ΜΕΤΡΑ ΠΡΙΝ ΣΥΖΗΤΗΣΕΙΣ ΑΙΣΘΗΤΙΚΗ.** Στις τρεις
απορρίψεις οι αιτίες ήταν: 620px γονέας · `min-width` που άνοιξε στήλες 280px ·
κείμενο ανάγνωσης στο 2.98:1 · ένας renderer για τρία είδη ύλης. **Καμία δεν
ήταν γούστο.** Βρες το νούμερο, δείξ' του το, μετά διόρθωσε.

#### ΠΡΙΝ ΠΕΙΣ ΟΤΙ ΤΕΛΕΙΩΣΕ ΜΙΑ ΟΨΗ — έξι ερωτήσεις, όλες μετρήσιμες
1. **Πλάτος:** τι `max-width` έχει ο γονέας ΣΤΟ ΛΑΠΤΟΠ; (σταθ. 51)
2. **Θραύση:** υπάρχει `min-width` πουθενά; Αν ναι, **είναι bug**. (σταθ. 51)
3. **Αντίθεση:** ποιο κείμενο διαβάζεται και σε τι λόγο/μέγεθος; (σταθ. 52)
4. **Μέτρο:** η πρόζα είναι σε 70–78ch και οι πίνακες σε πλήρες πλάτος;
5. **Σχήμα:** το κάθε κομμάτι πήρε το σχήμα ΤΟΥ, ή τα πέρασα όλα από τον ίδιο
   βρόχο; (σταθ. 53)
6. **ΤΟ RENDER:** **τύπωσε την αληθινή έξοδο και ΔΙΑΒΑΣΕ ΤΗΝ.** Σε αυτό το
   project το render έχει βρει, κάθε φορά, ό,τι δεν βρήκαν εκατοντάδες
   βεβαιώσεις: «6 SHELFVES», «1 σελίδες», σειρά DOM πίσω από `position:absolute`,
   πινακίδα που έδειχνε σε νεκρές οθόνες, δυαδική ερώτηση που χάριζε τον βαθμό.

#### ΤΙ ΘΕΛΕΙ ΑΠΟ ΜΕΝΑ, ΜΕ ΤΑ ΔΙΚΑ ΤΟΥ ΛΟΓΙΑ
- **«πες μου τι θεωρεις το πιο ωραιο»** — θέλει **ΜΙΑ άποψη με γούστο**, όχι
  κατάλογο επιλογών. Ρώτα ΜΟΝΟ όταν η απάντηση αλλάζει τι θα χτιστεί, το πολύ
  δύο ερωτήσεις, και **πάντα με τη σύστασή μου πρώτη**. ⛔ Απέρριψε ρητά το
  `AskUserQuestion` picker.
- **«make it perfect, make no mistakes»** — ΔΕΝ σημαίνει «τρέξε τα tests».
  Σημαίνει: **οδήγησε την αληθινή σελίδα, διάβασε την έξοδο, μέτρα τα νούμερα.**
- **«ξανα σκεψου το απο την αρχη»** — μην μπαλώσεις. Πέτα το σχήμα και βρες
  ποιο είναι το σωστό ΓΙΑ ΑΥΤΟ ΤΟ ΥΛΙΚΟ.
- **Διαβάζει από λάπτοπ.** Το κινητό είναι για καταγραφή και για ειδοποιήσεις.
- **Το υλικό είναι ΙΕΡΟ:** αυτούσιο από την πηγή, ΚΑΝΕΝΑ εφευρεμένο παράδειγμα,
  και **τα κενά ΦΑΙΝΟΝΤΑΙ** (ό,τι δεν διάβασα το δηλώνω και δεν το ρωτάω ποτέ).
- **Είναι αριστερόχειρας**, δίνει διαγώνισμα Πανελλήνιες, και δεν θέλει
  βαθμολογίες εκεί που ο καθηγητής δεν βαθμολογεί.

⛔ **Και μη μεταφράζεις τη ματαίωσή του σε απολογίες.** Βρες το λάθος, πες το
με νούμερα, διόρθωσέ το, **γράψε το test που το κλειδώνει** — αυτό είναι η
απάντηση, όχι το «συγγνώμη».
