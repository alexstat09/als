...
2. **Bump `CACHE` in `sw.js:15` on every deploy.** Currently `als-v470`. Never
   move it backwards.
...
**2026-08-11 — ΤΟ ΧΡΕΟΣ ΕΙΝΑΙ ΧΤΙΣΜΕΝΟ. `als-v470`.**
Το `docs/HOMEWORK_SPEC.md` έγινε κώδικας: `ladders.js` + `homework.html` +
`add_homework`/`get_homework` στο υπάρχον `api/mcp.js` (**μηδέν νέα function**).
Το `docs/ISTORIA_SPEC.md` παραμένει ΑΧΤΙΣΤΟ, όπως και τα briefs για **Λατινικά ·
...
### 📋 `docs/HOMEWORK_SPEC.md` — ΤΟ COMMAND CENTER (`homework.html`, άχτιστο)
16 ενότητες αντί για τις 60 του brief του. Αντίγραφο και στο
`~/command center prompt.md`.
...
- 🔴 **Δικές του, ανοιχτές:** το όνομα «ΤΟ ΧΡΕΟΣ» (κρατιέται ή αλλάζει, μία
  γραμμή) και αν η **Φάση 0+1 φεύγει μόνη της** πριν υπάρξει σύλληψη (η σύστασή
  μου: ναι).
...
- ⭐⭐ **ΤΟ ΜΕΤΡΗΜΕΝΟ ΓΕΓΟΝΟΣ ΠΟΥ ΤΟ ΚΑΝΕΙ ΔΩΡΕΑΝ:** κάθε σημείο κουβαλάει ήδη
  ...
  όλο το corpus: **57 στα 57 σημεία λύνονται σε ΑΚΡΙΒΩΣ ΜΙΑ παράγραφο** (0
  διφορούμενα, 0 ορφανά), και είναι ήδη σε σειρά παραγράφου. **Άρα ένας
