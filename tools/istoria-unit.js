...
let ARTIFACTS = 0, PARENS = 0;
/* ⛔⛔ ΔΥΟ ΔΙΑΦΟΡΕΤΙΚΑ ΠΡΑΓΜΑΤΑ ΕΧΟΥΝ ΤΟ ΙΔΙΟ ΑΚΡΙΒΩΣ ΣΧΗΜΑ «λέξη- λέξη»:
   (α) η σπασμένη λέξη της τυπωμένης σελίδας («πληροφο- ρίαι») — ΕΝΩΝΕΤΑΙ·
   (β) η ΚΛΕΙΣΤΙΚΗ ΠΑΡΕΝΘΕΤΙΚΗ ΠΑΥΛΑ («…της -ο εκσυγχρονισμός του κράτους-
       αποτέλεσαν…») — ΔΕΝ ενώνεται ΠΟΤΕ: το «κράτους-αποτέλεσαν» δεν είναι λέξη.
   Η παλιά γραφή τα έκανε και τα δύο. Σαρώθηκαν και οι 24 σελίδες του βιβλίου:
   η αλλαγή γραμμής ΔΕΝ αφήνει ίχνος (όλα είναι σκέτο κενό, μηδέν \n), άρα το
   μόνο σήμα που ξεχωρίζει το (β) είναι η ΑΝΟΙΧΤΙΚΗ παύλα « -γράμμα» πριν από
   αυτό, μέσα στην ίδια παράγραφο. Γι' αυτό εδώ σαρώνουμε με κατάσταση αντί για
   σκέτο regex — και οι δύο μετρητές αναφέρονται, καμία σιωπηλή αλλαγή. */
const GRK = /[Α-Ωα-ωΆ-Ώά-ώϊϋΐΰ]/;
function deArtifact(s) {
  let out = '', open = false, i = 0;
  while (i < s.length) {
    if (s[i] === '-') {
      const prev = s[i - 1] || '', next = s[i + 1] || '';
      if (/\s/.test(prev) && GRK.test(next)) { open = true; out += '-'; i++; continue; }
      if (GRK.test(prev) && /\s/.test(next)) {
        let j = i + 1;
        while (j < s.length && /\s/.test(s[j])) j++;
        if (j < s.length && GRK.test(s[j])) {
          if (open) { open = false; PARENS++; out += '- '; i = j; continue; }
          ARTIFACTS++; out += '-'; i = j; continue;
        }
      }
    }
    out += s[i]; i++;
...
function stripChrome(seg) {
  return seg
    ...
    /* ⚠️ Η ΛΕΖΑΝΤΑ ΤΟΥ ΠΙΝΑΚΑ ΕΙΝΑΙ ΚΙ ΑΥΤΗ ΧΡΩΜΙΟ, ΚΑΙ ΖΕΙ ΕΞΩ ΑΠΟ ΤΟΝ
       ...
       περίοδο 1896-1935») κάθεται σε δικό του <div class="centered"> ΠΡΙΝ το
       <table>. Ο εξαγωγέας Α δεν τον έβλεπε (δεν είναι <p>), ο Β τον μετρούσε
       παράγραφο — 4 vs 5 στην k1-g3. Ελέγχθηκε: σε ΟΛΟ το κεφ. 1 αυτή η κλάση
       κρατάει ΜΟΝΟ λεζάντες πινάκων (Πίνακες 3-8) ή είναι κενή. */
    .replace(/<div class="centered">[\s\S]*?<\/div>/g, ' ')
    .replace(/<img[^>]*>/g, ' ')
...
(async () => {
  ...
  if (PARENS) e('   ℹ️  ' + PARENS + ' παρενθετικές παύλες ΕΜΕΙΝΑΝ ως έχουν — δεν είναι σπασμένες λέξεις.');
