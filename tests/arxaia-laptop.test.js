...
section('6 · Η ΑΝΤΙΣΤΟΙΧΙΑ ΕΙΝΑΙ ΣΕΛΙΔΑ, ΟΧΙ ΚΑΡΤΕΣ');
{
  ...
  ok('και δεν υπολείπεται του αρχαίου πάνω από 2.5px (λάπτοπ)',
     num(aW) !== null && num(mW) !== null && (num(aW) - num(mW)) <= 2.5);
  is('⛔ η μετάφραση ΔΕΝ είναι στο --au-dim — είναι το κύριο κείμενο',
     /--au-dim/.test(m) ? 'είναι' : '', '');
  ok('η μετάφραση είναι σε --au-ivory', /--au-ivory/.test(m));

  /* ⭐ ΜΙΑ ΟΙΚΟΓΕΝΕΙΑ ΓΙΑ ΤΙΣ ΔΥΟ ΣΤΗΛΕΣ: δύο διαφορετικές διαβάζονταν
     ως «κείμενο ‖ διεπαφή» αντί για ένα κείμενο σε δύο φωνές. */
  ok('το αρχαίο είναι σε --ar-gr',    /--ar-gr/.test(a));
  ok('ΚΑΙ η μετάφραση είναι σε --ar-gr', /--ar-gr/.test(m));
