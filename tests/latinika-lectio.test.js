/* ══════════════════════════════════════════════════════════════════════
   tests/latinika-lectio.test.js — ΟΙ ΔΥΟ ΕΝΟΤΗΤΕΣ ΤΩΝ ΛΑΤΙΝΙΚΩΝ
   ...
   Άρα αυτό το test φυλάει τέσσερα πράγματα:
     ...
     4. ⛔ ΟΤΙ ΤΑ ΚΛΕΙΔΙΑ ΤΩΝ ΕΝΟΤΗΤΩΝ ΔΕΝ ΣΥΓΧΡΟΝΙΖΟΝΤΑΙ. Το `lectio16:v1`
...
section('1 · τα δύο πακέτα, με hash');
...
PACKS.forEach(function (pk) {
  ...
  eq(crypto.createHash('sha256').update(stripped, 'utf8').digest('hex'), pk.sha,
...
S17.forEach(function (sc) {
  sc.s.forEach(function (sn, i) {
    var tag = 'XVII ' + sc.n + '/' + (i + 1);
    var la = {}, gr = {};
...
ok(HUB.indexOf('<b>' + (n16 + n17) + '</b> προτάσεις') > 0,
  'το hero λέει το ΑΘΡΟΙΣΜΑ των δύο πακέτων (' + (n16 + n17) + ')');
...
ok(HUB.indexOf('lectio16:v1') > 0 && HUB.indexOf('lectio17_known_v1') > 0,
  'η βιβλιοθήκη τα ΔΙΑΒΑΖΕΙ και τα δύο');
...
eq(A.els.ltN.textContent, '2 · 27 προτάσεις', 'ο μετρητής της ενότητας');
ok(A.els.ltHead.innerHTML.indexOf('<b>27</b>') >= 0, 'το hero δείχνει το σύνολο, όχι πρόοδο');
...
ok(B.els.ltHead.innerHTML.indexOf('<b>7</b> από <b>27</b>') >= 0, 'το hero αθροίζει τα δύο σχήματα');
ok(B.els.ltSub.innerHTML.indexOf('<b>20</b>') >= 0, 'και λέει πόσα μένουν');
