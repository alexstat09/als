...
(async () => {
  ...
  section('4 · Η καλωδίωση, σε κάθε σελίδα που κρατάει χάρτη εγγραφών');
  {
    ...
    /* Τα Λατινικά επιβίωναν ήδη, με άλλο σχήμα: πίνακας κελιών με `id`+`ts`,
       που το mergeArray κρατάει «ο νεότερος κερδίζει». Κλειδώνεται εδώ ώστε
       να μη «καθαριστεί» κάποτε το `c.ts` σαν αχρησιμοποίητο. */
    const lat = fs.readFileSync(path.join(ALS, 'latinika.html'), 'utf8');
    ok('latinika.html σφραγίζει ακόμη κάθε κελί με ts', /c\.ts = Date\.now\(\)/.test(lat));
  }
