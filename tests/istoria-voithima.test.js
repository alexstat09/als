...
if (G6) {
  ...
  /* ⭐⭐⭐ ΟΙ ΔΥΟ ΙΣΧΥΡΙΣΜΟΙ ΤΗΣ ΣΕΛΙΔΑΣ, ΒΓΑΛΜΕΝΟΙ ΑΠΟ ΤΟ ΥΛΙΚΟ */
  const sized = CHAPTERS.filter(c => /^k1-/.test(c.id))
    .map(c => ({ id: c.id, len: c.paragraphs.join('\n').length }))
    .sort((a, b) => a.len - b.len);
  is('⭐⭐⭐ Γ.6 · είναι όντως η ΠΙΟ ΜΙΚΡΗ ενότητα του corpus (μετρημένο, όχι καρφωμένο)',
     sized[0].id, 'k1-g6');
  ok('  ⭐ και τα ΔΥΟ νούμερα που γράφει η σελίδα συμφωνούν με τη μέτρηση',
     JSON.stringify(G6.facts).includes(String(sized[0].len).replace(/\B(?=(\d{3})+(?!\d))/g, '.'))
     && JSON.stringify(G6.facts).includes(String(sized[1].len).replace(/\B(?=(\d{3})+(?!\d))/g, '.')));
  ok('  ⚠️ …και η δεύτερη μικρότερη είναι όντως αυτή που ονομάζει η σελίδα (Β.9)',
     sized[1].id === 'k1-b9' && JSON.stringify(G6.facts).includes('Β.9'));
