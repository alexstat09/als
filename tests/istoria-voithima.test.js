...
/* ⭐⭐⭐ ΚΑΘΕ «Η ΠΙΟ ΜΙΚΡΗ» ΕΙΝΑΙ ΔΑΝΕΙΚΟ ΜΕΧΡΙ ΤΗΝ ΕΠΟΜΕΝΗ ΕΝΟΤΗΤΑ.
   Η Γ.6 έγραφε «η ΠΙΟ ΜΙΚΡΗ ενότητα ολόκληρου του βοηθήματος» και ο
   φρουρός της το επιβεβαίωνε ΜΕ ΜΕΤΡΗΣΗ. Η Γ.7 μπήκε με 1.138 και το
   ακύρωσε την επόμενη μέρα· ο φρουρός ΕΣΚΑΣΕ, όπως όφειλε, και ο
   ισχυρισμός ξαναγράφτηκε σε τρία σημεία.
   ⛔ Ο έλεγχος ΔΕΝ έχει καρφωμένο νούμερο: βγάζει την κατάταξη από το
   ΥΛΙΚΟ και απαιτεί η σελίδα να λέει το ίδιο. Αν κάποτε μπει μικρότερη
   ενότητα, θα ξανασκάσει — και αυτό είναι το ζητούμενο. */
{
  const sized = CHAPTERS.filter(c => /^k1-/.test(c.id))
    .map(c => ({ id: c.id, len: c.paragraphs.join('\n').length }))
    .sort((a, b) => a.len - b.len);
  const dot = n => String(n).replace(/\B(?=(\d{3})+(?!\d))/g, '.');
  is('⭐⭐⭐ η ΜΙΚΡΟΤΕΡΗ ενότητα του corpus είναι η Γ.7 (μετρημένο)', sized[0].id, 'k1-g7');
  is('  ⭐ και η δεύτερη είναι η Γ.6, που κρατούσε τον τίτλο ως χθες', sized[1].id, 'k1-g6');
  const G7f = JSON.stringify((CHAPTERS.find(c => c.id === 'k1-g7') || { facts: [] }).facts);
  const G6f = JSON.stringify((CHAPTERS.find(c => c.id === 'k1-g6') || { facts: [] }).facts);
  ok('  ⭐⭐ η Γ.7 γράφει το ΔΙΚΟ ΤΗΣ μέγεθος σωστά', G7f.includes(dot(sized[0].len)));
  ok('  ⭐⭐ η Γ.6 γράφει το ΔΙΚΟ ΤΗΣ μέγεθος σωστά', G6f.includes(dot(sized[1].len)));
  ok('  ⛔ …και η Γ.6 ΔΕΝ ισχυρίζεται πια ότι είναι η πιο μικρή',
     !/Η ΠΙΟ ΜΙΚΡΗ ενότητα ολόκληρου του βοηθήματος/.test(G6f));
  ok('  ⭐⭐⭐ …αλλά ΟΜΟΛΟΓΕΙ ότι το έλεγε και ότι ακυρώθηκε, αντί να το σβήσει σιωπηλά',
     G6f.includes('ΟΤΑΝ ΓΡΑΦΤΗΚΕ ΑΥΤΟ') && G6f.includes('Γ.7'));
}
